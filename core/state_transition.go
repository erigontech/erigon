...
func (st *StateTransition) TransitionDb(refunds bool, gasBailout bool) (*evmtypes.ExecutionResult, error) {
	...
	gas, floorGas7623, overflow := fixedgas.IntrinsicGas(st.data, uint64(len(accessTuples)), uint64(accessTuples.StorageKeys()), contractCreation, rules.IsHomestead, rules.IsIstanbul, isEIP3860, true, uint64(len(auths)))
	if overflow {
	...
	fmt.Println("rules.IsPrague: ", rules.IsPrague)
	...
	// st.state.AddBalance(sender.Address(), new(uint256.Int).SetFromBig(big), tracing.BalanceDecreaseTransactionFee)
