...
func TestDeleteRecreateSlots(t *testing.T) {
	...
	var got common.Hash
	statedb.GetState(aa, common.HexToHash("01"), &got)
	if exp := (common.Hash{}); got != exp {
		t.Errorf("got %x exp %x", got, exp)
	}
	statedb.GetState(aa, common.HexToHash("02"), &got)
	if exp := (common.Hash{}); got != exp {
		t.Errorf("got %x exp %x", got, exp)
	}
	// Also, 3 and 4 should be set
	statedb.GetState(aa, common.HexToHash("03"), &got)
	if exp := common.HexToHash("03"); got != exp {
		t.Fatalf("got %x exp %x", got, exp)
	}
	statedb.GetState(aa, common.HexToHash("04"), &got)
	...
	for i, block := range blocks {
		...
		if exp.exist {
			...
			for slot, val := range exp.values {
				...
				statedb.GetState(aa, asHash(slot), &gotValue)
