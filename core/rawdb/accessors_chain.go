...
func WriteReceiptsCache(tx kv.TemporalPutDel, blockNum uint64, blockHash common.Hash, receipts types.Receipts) error {
	...
	for txnIndex, receipt := range receipts {
		...
		if receipt != nil {
			if txnIndex != int(receipt.TransactionIndex) {
				panic(fmt.Sprintf("assert: txnIndex is wrong %d %d, blockNum=%d, txnIdx=%d", txnIndex, receipt.TransactionIndex, blockNum, txnIndex))
			}
