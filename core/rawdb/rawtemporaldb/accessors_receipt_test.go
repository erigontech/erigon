...
func TestAppendReceipt(t *testing.T) {
	...
	err = AppendReceipt(doms.AsPutDel(tx), &types.Receipt{CumulativeGasUsed: 11, FirstLogIndexWithinBlock: 1}, 0)
	require.NoError(err)

	doms.SetTxNum(2) // block1

	doms.SetTxNum(3) // block2
	err = AppendReceipt(doms.AsPutDel(tx), &types.Receipt{CumulativeGasUsed: 12, FirstLogIndexWithinBlock: 0}, 0)
	require.NoError(err)

	doms.SetTxNum(4) // block2
	err = AppendReceipt(doms.AsPutDel(tx), &types.Receipt{CumulativeGasUsed: 14, FirstLogIndexWithinBlock: 4}, 0)
	require.NoError(err)

	doms.SetTxNum(5) // block2

	err = doms.Flush(context.Background(), tx, 0)
	require.NoError(err)
