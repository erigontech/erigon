...
func CommitGenesisBlockWithOverride(db kv.RwDB, genesis *types.Genesis, ethCfg *ethconfig.Config, tmpDir string, logger log.Logger) (*chain.Config, *types.Block, error) {
	tx, err := db.BeginRw(context.Background())
	if err != nil {
		return nil, nil, err
	}
	defer tx.Rollback()
	c, b, err := WriteGenesisBlock(tx, genesis, ethCfg, tmpDir, logger)
	if err != nil {
		return c, b, err
	}
	err = tx.Commit()
...
func WriteGenesisBlock(tx kv.RwTx, genesis *types.Genesis, ethCfg *ethconfig.Config, tmpDir string, logger log.Logger) (*chain.Config, *types.Block, error) {
	...
	applyOverrides := func(config *chain.Config) {
		if ethCfg != nil {
			if ethCfg.OverridePragueTime != nil {
				config.PragueTime = ethCfg.OverridePragueTime
			}
			if ethCfg.PragueEOFTime != nil {
				config.PragueEOF = ethCfg.PragueEOFTime
			}
		}
	}
