...
func TestGenesisBlockHashesZkevm(t *testing.T) {
	...
	check := func(network string) {
		...
		defer tx.Rollback()
		_, block, err := core.WriteGenesisBlock(tx, genesis, nil, nil, nil, nil, nil, "/tmp/"+network, logger)
...
func TestCommitGenesisIdempotency2(t *testing.T) {
	...
	logger := log.New()
	_, _, err := core.WriteGenesisBlock(tx, genesis, nil, nil, nil, nil, nil, "/tmp/"+networkname.HermezMainnetChainName, logger)
	require.NoError(t, err)
