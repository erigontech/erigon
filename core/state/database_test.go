...
func TestReproduceCrash(t *testing.T) {
	...
	newSharedDomainsTx := state3.NewSharedDomainsTx(sd, tx)

	tsw := state.NewWriterV4(newSharedDomainsTx)
	tsr := state.NewReaderV3(newSharedDomainsTx)
	sd.SetTxNum(1)
	sd.SetBlockNum(1)
...
func TestChangeAccountCodeBetweenBlocks(t *testing.T) {
	...
	r, tsw := state.NewReaderV3(sharedDomainsTx), state.NewWriterV4(sharedDomainsTx)
...
func TestCacheCodeSizeSeparately(t *testing.T) {
	...
	sharedDomainsTx := state3.NewSharedDomainsTx(sd, tx)
	r, w := state.NewReaderV3(sharedDomainsTx), state.NewWriterV4(sharedDomainsTx)
