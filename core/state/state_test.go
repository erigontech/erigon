...
func (s *StateSuite) SetUpTest(c *checker.C) {
	...
	agg, err := stateLib.NewAggregator(context.Background(), datadir.New(""), 16, db, log.New(), false)
	if err != nil {
		panic(err)
	}
	defer agg.Close()
...
func NewTestTemporalDb(tb testing.TB) (kv.TemporalRwDB, kv.TemporalRwTx, *state.Aggregator) {
	...
	agg, err := state.NewAggregator(context.Background(), datadir.New(tb.TempDir()), 16, db, log.New(), false)
	if err != nil {
		tb.Fatal(err)
	}
