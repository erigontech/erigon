...
func (sdb *IntraBlockState) GetBalance(addr libcommon.Address) (*uint256.Int, error) {
	return versionedRead(sdb, SubpathKey(addr, BalancePath), false, u256.Num0,
...
func (sdb *IntraBlockState) GetNonce(addr libcommon.Address) (uint64, error) {
	return versionedRead(sdb, SubpathKey(addr, NoncePath), false, 0,
		...
		func(s *stateObject) ([]byte, error) {
			if s != nil && !s.deleted {
				...
				if sdb.trace || traceAccount(addr) {
					fmt.Printf("(%d) GetCode %x, returned %d\n", sdb.txIndex, addr, len(code))
				}
				return code, err
			}
			if sdb.trace || traceAccount(addr) {
				fmt.Printf("(%d) GetCode %x, returned nil\n", sdb.txIndex, addr)
			}
			return nil, nil
		})
}
...
func (sdb *IntraBlockState) GetCodeSize(addr libcommon.Address) (int, error) {
	...
	if sdb.trace || traceAccount(addr) {
		fmt.Printf("(%d) GetCodeSize %x: %d\n", sdb.txIndex, addr, size)
	}
...
func (sdb *IntraBlockState) GetCodeHash(addr libcommon.Address) (libcommon.Hash, error) {
	return versionedRead(sdb, SubpathKey(addr, CodeHashPath), false, libcommon.Hash{},
...
func (sdb *IntraBlockState) HasSelfdestructed(addr libcommon.Address) (bool, error) {
	return versionedRead(sdb, SubpathKey(addr, SelfDestructPath), false, false,
...
func (sdb *IntraBlockState) AddBalance(addr libcommon.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) error {
	if sdb.trace || traceAccount(addr) {
		...
		fmt.Printf("(%d) AddBalance %x, %d+%d=%d\n", sdb.txIndex, addr, bal, amount, (&uint256.Int{}).Add(bal, amount))
	}
...
func (sdb *IntraBlockState) SubBalance(addr libcommon.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) error {
	if sdb.trace || traceAccount(addr) {
		bal, _ := sdb.GetBalance(addr)
		fmt.Printf("(%d) SubBalance %x, %d-%d=%d\n", sdb.txIndex, addr, bal, amount, (&uint256.Int{}).Sub(bal, amount))
	}
...
func (sdb *IntraBlockState) SetNonce(addr libcommon.Address, nonce uint64) error {
	if sdb.trace || traceAccount(addr) {
		fmt.Printf("(%d) SetNonce %x, %d\n", sdb.txIndex, addr, nonce)
	}
...
func (sdb *IntraBlockState) SetCode(addr libcommon.Address, code []byte) error {
	if sdb.trace || traceAccount(addr) {
		...
		fmt.Printf("(%d) SetCode %x, %d: %x...\n", sdb.txIndex, addr, lenv, v)
	}
...
func (sdb *IntraBlockState) SetIncarnation(addr libcommon.Address, incarnation uint64) error {
	if sdb.trace || traceAccount(addr) {
		fmt.Printf("(%d) SetIncarnation %x, %d\n", sdb.txIndex, addr, incarnation)
	}
...
func (sdb *IntraBlockState) getStateObject(addr libcommon.Address) (*stateObject, error) {
	...
	if sdb.versionMap != nil {
		...
		if balance, _ := versionedRead[*uint256.Int](sdb, SubpathKey(addr, BalancePath), false, nil, nil, nil); balance != nil {
			account.Balance = *balance
		}

		if nonce, _ := versionedRead[uint64](sdb, SubpathKey(addr, NoncePath), false, 0, nil, nil); nonce > account.Nonce {
			account.Nonce = nonce
		}
	...
	if s.versionMap != nil {
		...
		if s.trace || traceAccount(k.GetAddress()) {
			...
			case []byte:
				...
				fmt.Printf("(%d) Version Written %x: %v: %d: %x...\n", s.txIndex, k.GetAddress(), s.Version(), lenv, v[0:40])
			default:
				fmt.Printf("(%d) Version Written %x: %v: %d\n", s.txIndex, k.GetAddress(), s.Version(), val)
			}
		}
