...
func (sdb *IntraBlockState) GetNonce(addr libcommon.Address) (uint64, error) {
	nonce, err := versionedRead(sdb, SubpathKey(&addr, NoncePath), false, 0,
...
func (sdb *IntraBlockState) GetCode(addr libcommon.Address) ([]byte, error) {
	code, err := versionedRead(sdb, SubpathKey(&addr, CodePath), false, nil,
		func(v []byte) []byte {
			return v
		},
...
func (sdb *IntraBlockState) getStateObject(addr libcommon.Address) (*stateObject, error) {
	...
	if sdb.versionMap != nil {
		...
		if balance, _ := versionedRead[uint256.Int](sdb, SubpathKey(&addr, BalancePath), false, account.Balance, nil, nil); balance.Cmp(&account.Balance) != 0 {
