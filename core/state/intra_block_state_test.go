...
func TestCopy(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		err = origTds.TrieStateWriter().UpdateAccountData(context.Background(), obj.address, &obj.data, new(accounts.Account), true)
		if err != nil {
			t.Log(err)
		}
	}
	...
	for i := byte(0); i < 255; i++ {
		...
		err = origTds.TrieStateWriter().UpdateAccountData(context.Background(), origObj.address, &origObj.data, new(accounts.Account), true)
		if err != nil {
			t.Log(err)
		}
		err = copyTds.TrieStateWriter().UpdateAccountData(context.Background(), copyObj.address, &copyObj.data, new(accounts.Account), true)
		if err != nil {
			t.Log(err)
		}
	}
