...
func (hr *HistoryReaderNoState) ReadAccountData(address common.Address) (*accounts.Account, error) {
	...
	if !noState {
		...
		if enc == nil {
			composite := make([]byte, 8+20)
			binary.BigEndian.PutUint64(composite, stateTxNum)
			copy(composite[8:], address.Bytes())
			enc, err = hr.tx.GetOne(kv.PlainStateR, composite)
			if err != nil {
				return nil, err
			}
			if enc == nil {
...
func (hr *HistoryReaderNoState) ReadAccountStorage(address common.Address, incarnation uint64, key *common.Hash) ([]byte, error) {
	...
	if !noState {
		...
		if enc == nil {
			composite := make([]byte, 8+20+8+32)
			binary.BigEndian.PutUint64(composite, stateTxNum)
			copy(composite[8:], address.Bytes())
			binary.BigEndian.PutUint64(composite[8+20:], 1)
			copy(composite[8+20+8:], key.Bytes())
			enc, err = hr.tx.GetOne(kv.PlainStateR, composite)
			if err != nil {
				return nil, err
			}
...
func (hr *HistoryReaderNoState) ReadAccountCode(address common.Address, incarnation uint64, codeHash common.Hash) ([]byte, error) {
	...
	if !noState {
		...
		if enc == nil {
			composite := make([]byte, 8+32)
			binary.BigEndian.PutUint64(composite, stateTxNum)
			copy(composite[8:], codeHash.Bytes())
			enc, err = hr.tx.GetOne(kv.CodeR, composite)
			if err != nil {
				return nil, err
			}
		}
