...
func (sdb *StateDB) SetStorageSize(addr common.Address, currentLocation common.Hash, newLocation common.Hash, val *big.Int) {
	fmt.Println("!!!! SetStorageSize", addr.Hex(), newLocation == nullLocation, val == nullValue, newLocation.Hex(), val.String())
	fmt.Println("!!!! SetStorageSize 1", nullLocation.String())
	fmt.Println("!!!! SetStorageSize 2", newLocation.String())

	switch {
	case currentLocation == nullLocation && val.Cmp(nullValue) != 0:
		// new value case
		fmt.Println("!!!! SetStorageSize inc currentLocation=nil", addr.Hex())
