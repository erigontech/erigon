...
func Execute(code, input []byte, cfg *Config, tempdir string) ([]byte, *state.IntraBlockState, error) {
	...
	if !externalState {
		...
		defer db.Close()
		agg, err := state3.NewAggregator(context.Background(), datadir.New(tempdir), config3.DefaultStepSize, db, log.New(), false)
		if err != nil {
			return nil, nil, err
		}
		defer agg.Close()
...
func Create(input []byte, cfg *Config, blockNr uint64) ([]byte, libcommon.Address, uint64, error) {
	...
	if !externalState {
		...
		defer db.Close()
		agg, err := state3.NewAggregator(context.Background(), datadir.New(tmp), config3.DefaultStepSize, db, log.New(), false)
		if err != nil {
