...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.Push(x)
		stack.Push(y)
		opFn(&pc, evmInterpreter, &ScopeContext{nil, stack, nil, nil, nil, 0})
		if len(stack.Data) != 1 {
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.Push(y)
		stack.Push(x)
		opAddmod(&pc, evmInterpreter, &ScopeContext{nil, stack, nil, nil, nil, 0})
		actual := stack.Pop()
		if actual.Cmp(expected) != 0 {
...
func opBenchmark(b *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < b.N; i++ {
		...
		op(&pc, evmInterpreter, &ScopeContext{nil, stack, nil, nil, nil, 0})
		stack.Pop()
	}
}
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evmInterpreter, &ScopeContext{mem, stack, nil, nil, nil, 0})
...
func TestOpTstore(t *testing.T) {
	...
	var (
		...
		scopeContext   = ScopeContext{mem, stack, contract, nil, nil, 0}
...
func BenchmarkOpKeccak256(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		opKeccak256(&pc, evmInterpreter, &ScopeContext{mem, stack, nil, nil, nil, 0})
	}
}
...
func TestOpMCopy(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		// Do the copy
		opMcopy(&pc, evmInterpreter, &ScopeContext{mem, stack, nil, nil, nil, 0})
