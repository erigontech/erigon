...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.Push(x)
		stack.Push(y)
		opFn(&pc, evmInterpreter, &callCtx{nil, stack, nil, nil, nil})
		if len(stack.Data) != 1 {
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.Push(y)
		stack.Push(x)
		opAddmod(&pc, evmInterpreter, &callCtx{nil, stack, nil, nil, nil})
		actual := stack.Pop()
		if actual.Cmp(expected) != 0 {
...
// 		stack.Push(x)
// 		stack.Push(y)
// 		opFn(&pc, interpreter, &callCtx{nil, stack, nil, nil, nil})
// 		actual := stack.Pop()
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, evmInterpreter, &callCtx{nil, stack, nil, nil, nil})
		stack.Pop()
	}
}
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evmInterpreter, &callCtx{mem, stack, nil, nil, nil})
...
func BenchmarkOpSHA3(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		opSha3(&pc, evmInterpreter, &callCtx{mem, stack, nil, nil, nil})
	}
}
