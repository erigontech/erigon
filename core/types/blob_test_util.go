...
func MakeBlobTxnRlp() ([]byte, []gokzg4844.KZGCommitment) {
	bodyRlp := hexutility.MustDecodeHex(testdata.BodyRlpHex)
	...
	copy(blob0[:], hexutility.MustDecodeHex(testdata.ValidBlob1Hex))
	copy(blob1[:], hexutility.MustDecodeHex(testdata.ValidBlob2Hex))
...
func MakeWrappedBlobTxn(chainId *uint256.Int) *BlobTxWrapper {
	...
	copy(wrappedTxn.Blobs[0][:], hexutility.MustDecodeHex(testdata.ValidBlob1Hex))
	copy(wrappedTxn.Blobs[1][:], hexutility.MustDecodeHex(testdata.ValidBlob2Hex))
