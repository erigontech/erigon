...
func TestCanEncodeAndDecodeRawBody(t *testing.T) {
	...
	require.Nil(t, signedBlobTx.EncodeRLP(&signedBlobTxBuf))
	...
	t.Log("Body:", string(expectedJson))
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(rawBody.Transactions) != 3 {
		t.Fatalf("expected there to be 3 transaction once decoded")
	}
	...
	if rawBody.Transactions[2][3] != BlobTxType {
...
func TestExcessDataGasEncoding(t *testing.T) {
	...
	withdrawals[0] = &Withdrawal{
		...
		Validator: 89,
		Address:   libcommon.HexToAddress("0x690b9a9e9aa1c9db991c7721a92d351db4fac990"),
		Amount:    2 * params.Ether,
	}
	withdrawals[1] = &Withdrawal{
		Index:     44555667,
		Validator: 37,
		Address:   libcommon.HexToAddress("0x95222290dd7278aa3ddd389cc1e1d165cc4bafe5"),
		Amount:    5 * params.Ether,
	}
