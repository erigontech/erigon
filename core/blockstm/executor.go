...
func (ev *ExecVersionView) Execute(logger log.Logger) (er ExecResult) {
	er.ver = ev.ver
	if er.err = ev.et.Execute(ev.mvh, ev.ver.Incarnation, logger); er.err != nil {
		return
	}
...
// nolint: gocognit
func (pe *ParallelExecutor) Prepare(logger log.Logger) error {
...
func executeParallelWithCheck(interruptCtx context.Context, tasks []ExecTask, profile bool, check PropertyCheck, metadata bool, logger log.Logger) (result ParallelExecutionResult, err error) {
	if len(tasks) == 0 {
...
func ExecuteParallel(ctx context.Context, tasks []ExecTask, profile bool, metadata bool, logger log.Logger) (result ParallelExecutionResult, err error) {
	return executeParallelWithCheck(ctx, tasks, profile, nil, metadata, logger)
}
