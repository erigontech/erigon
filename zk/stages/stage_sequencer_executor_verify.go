			...
			if cfg.cfgZk.Limbo {
				...
				for _, blockNumber := range blockNumbers {
					...
					for i, transaction := range block.Transactions() {
						...
						signer := types.MakeSigner(cfg.chainConfig, blockNumber, 0)
				...
				u.UnwindTo(lowestBlock.NumberU64()-1, stagedsync.BadBlock(lowestBlock.Hash(), fmt.Errorf("invalid batch %d", response.BatchNumber)))
