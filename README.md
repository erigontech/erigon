...
* For an Archive node of Ethereum Mainnet we recommend >=3.5TB storage space: 2.3TB state (as of March 2024),
  642GB snapshots (can symlink or mount folder `<datadir>/snapshots` to another disk), 200GB temp files (can symlink or mount folder `<datadir>/temp` to another disk). Ethereum Mainnet Full node (
  see `--prune*` flags): 1.1TB (March 2024).

* Goerli Full node (see `--prune*` flags): 189GB on Beta, 114GB on Alpha (April 2022).

* Gnosis Chain Archive: 1.7TB (March 2024). Gnosis Chain Full node (`--prune=hrtc` flag): 530GB (March 2024).
