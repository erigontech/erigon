...
pruning options; enabling receipts or commitment history adds a lot on top —
see [Erigon3 datadir size](#erigon3-datadir-size). The same figures are published, with more detail, on the
...
headers, bodies and transactions — that sit directly in `snapshots/`; the rows below it are its sub-folders. Each column
is one node, so totals can differ a few percent from the table above, which is measured on freshly synced nodes.
...
Caplin numbers are the backfilled beacon blocks, blob sidecars and beacon state snapshots. They exclude the beacon chain
db that every node with the embedded Consensus Layer keeps anyway. Blob sidecars dominate: on mainnet they are 2.17 TB
of the 2.48 TB total.
...
- **Store most of data in immutable files (segments/snapshots):**
  ...
  - `chaindata` is tens of gb (22gb on an archive mainnet node). It's ok to `rm -rf chaindata`. (to prevent grow:
    recommend `--batchSize <= 1G`)
