...
func TestChapelHertzFixOverrides(t *testing.T) {
	...
	require.Len(t, p.StorageOverrides(), 2)
	at := overridesAt(p)
	...
	for _, tt := range []struct {
		...
		}}, at(tt.blockNum, tt.txIndex))

		require.Nil(t, at(tt.blockNum, tt.txIndex-1))
		require.Nil(t, at(tt.blockNum, tt.txIndex+1))
		require.Nil(t, at(tt.blockNum+1, tt.txIndex))
	}

	// Mainnet's entries must not fire on Chapel.
	require.Nil(t, at(33851236, 89))
	require.Nil(t, at(33851236, 90))
}
...
func TestMainnetHertzFixOverrides(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		overrides := at(33851236, tt.txIndex)
	...
	require.Nil(t, at(33851236, 88))
	require.Nil(t, at(33851236, 91))

	// Chapel's entries must not fire on mainnet.
	require.Nil(t, at(35547779, 196))
}

func overridesAt(p *Parlia) func(blockNum uint64, txIndex int) []state.StorageOverride {
	table := p.StorageOverrides()
	return func(blockNum uint64, txIndex int) []state.StorageOverride {
		return table[state.StorageOverridePosition{BlockNum: blockNum, TxIndex: txIndex}]
	}
}
