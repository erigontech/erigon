...
func CheckCommitmentHistAtBlk(ctx context.Context, db kv.TemporalRoDB, br services.FullBlockReader, blockNum uint64, lvl log.Lvl, logger log.Logger) error {
	...
	defer tx.Rollback()
	sd, err := execctx.NewSharedDomains(ctx, tx, logger, commitment.TrieConfig{Variant: commitment.VariantHexPatriciaTrie, DeferBranchUpdates: false})
	if err != nil {
		return err
	}
	defer sd.Close()
...
func CheckCommitmentHistAtBlkRange(ctx context.Context, sc SamplerCfg, db kv.TemporalRoDB, br services.FullBlockReader, from, to uint64, logger log.Logger) error {
	...
	for windowStart := from; windowStart < to; windowStart += checkCommitmentHistWindowSize {
		...
		g.Go(func() error {
			...
			defer tx.Rollback()
			sd, err := execctx.NewSharedDomains(wCtx, tx, logger, commitment.TrieConfig{Variant: commitment.VariantHexPatriciaTrie, DeferBranchUpdates: false})
			if err != nil {
				return err
			}
			defer sd.Close()
