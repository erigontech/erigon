...
func (w *BlockWriter) TruncateBodies(tx kv.RwTx, from uint64) error {
	...
	if err := backup.ClearTableInTx(tx, kv.EthTx); err != nil {
		return err
	}
	if err := backup.ClearTableInTx(tx, kv.MaxTxNum); err != nil {
		return err
	}
