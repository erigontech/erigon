...
func TestBlockAccessListBytes_CacheHitShortCircuits(t *testing.T) {
	...
	data := []byte{0xc1, 0x00}
	rawdb.CacheBlockAccessList(hash, data)
...
func TestCacheBlockAccessList_EmptyIsNoOp(t *testing.T) {
	...
	rawdb.CacheBlockAccessList(hash, nil)
	rawdb.CacheBlockAccessList(hash, []byte{})
	_, ok := rawdb.CachedBlockAccessList(hash)
...
func TestCacheBlockAccessList_CopiesBytes(t *testing.T) {
	...
	rawdb.CacheBlockAccessList(hash, src)
	src[0] = 0xff // mutate caller's slice — cache must hold its own copy

	got, ok := rawdb.CachedBlockAccessList(hash)
	require.True(t, ok)
