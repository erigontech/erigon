...
func TestGetMinimumBlocksToDownload_ThreeCutoffs(t *testing.T) {
	...
	minBlock, historyStep, commitmentStep, receiptsStep, err := getMinimumBlocksToDownload(context.Background(), br, 150, stepSize, 100, 300, 200)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetMinimumBlocksToDownload_MinBlock(t *testing.T) {
	...
	minBlock, historyStep, commitmentStep, receiptsStep, err := getMinimumBlocksToDownload(context.Background(), br, 150, stepSize, 200, 200, 100)
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, uint64(100), minBlock)
