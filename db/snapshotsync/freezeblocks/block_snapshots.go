...
func (br *BlockRetire) BuildFilesInBackground(
	...
	started := br.background.TryGo(func() {
		...
		// Cancel on Close too (br.ctx). retireCancel already cancels on
		// caller ctx via WithCancel above; the AfterFunc adds the Close path.
		stopOnClose := context.AfterFunc(br.ctx, retireCancel)
		defer stopOnClose()
...
func DumpHeadersRaw(ctx context.Context, db kv.RoDB, _ *chain.Config, blockFrom, blockTo uint64, _ firstKeyGetter, collect func([]byte) error, workers int, lvl log.Lvl, logger log.Logger, test bool, blockReader dbservices.FullBlockReader) (uint64, error) {
	logEvery := time.NewTicker(20 * time.Second)
	defer logEvery.Stop()
