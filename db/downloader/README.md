...
1. Determine the pprof or debug port for your node. It can be exposed by passing `--pprof` to `erigon` or setting [
   `GOPPROF=http` (see http)](https://github.com/anacrolix/envpprof?tab=readme-ov-file#envpprof) in `erigon`'s
   environment. This will give you a `host:port` combo of something like `localhost:6061`, where the host is the route
   to your `erigon` node.
    ```
    set hostport=localhost:6061
    ```
2. Check `http://$hostport/downloader/torrentClientStatus`. It shows current torrent client activity, including number
   of torrents `# Torrents:`, and on the same line, how many remain to be completed. The torrents are listed, incomplete
   first following that.
3. Check the goroutine listing at `http://$hostport/debug/pprof/goroutine?debug=1`. See `http://$hostport/debug/pprof/`
   for a directory listing of useful profiles.
4. Examine the CPU profile: `go tool pprof http://$hostport/debug/pprof/profile`.
5. If you fail to determine the cause, provide the above information to the developers. This snippet describes one way
