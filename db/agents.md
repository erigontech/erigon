...
- Blob sidecar segments and their index do not share a name: the segment is `<range>-blobsidecars.seg`, the index is `<range>-blocksidecars.idx` (`snaptype/type.go` registers `blocksidecars` as `BlobSidecarSlot`'s index name). Any glob, cleanup or existence check must cover both names or it silently misses half the pair
- Blob snapshots rest up to one `CaplinMergeLimit` behind beacon snapshots by design: the antiquary will not dump until a whole range is available (`cl/antiquary/antiquary.go`), so a resting gap of `CaplinMergeLimit - 1` is healthy, not a stall. Coverage checks encode that tolerance; one that requires the blob and beacon frontiers to match rejects every healthy node
