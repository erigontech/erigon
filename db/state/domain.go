...
func (dt *DomainRoTx) getLatestFromDb(key []byte, roTx kv.Tx, maxStep kv.Step) ([]byte, kv.Step, bool, error) {
	...
	if dt.d.LargeValues {
		// SeekBothRange at the smallest invStep returns the dup with the smallest
		// invStep = highest actual step = latest write; a maxStep bound raises the
		// floor to the newest step at or below it.
		var dupSeek [8]byte
		if maxStep != kv.NoStepBound {
			binary.BigEndian.PutUint64(dupSeek[:], ^uint64(maxStep))
		}
