...
func (dt *DomainRoTx) getLatestFromDb(key []byte, roTx kv.Tx) ([]byte, kv.Step, bool, error) {
	...
	// Published snapshot files may be missing deletion entries due to the
	// collation/pruning race (#20169) on the node that built them. Without
	// this guard, getLatestFromDb discards the DB deletion (step too old)
	// and falls through to getLatestFromFiles, which returns a stale
	// pre-deletion value from the corrupt file.
	if len(v) == 0 {
...
// GetLatest returns value, step in which the value last changed, and a bool
// indicating whether an authoritative answer was found. found=true with
// len(v)==0 means the key was explicitly deleted (tombstone); found=false
