...
func testDbAndInvertedIndex(tb testing.TB, aggStep uint64, logger log.Logger) (kv.RwDB, *InvertedIndex) {
	...
	ii, err := NewInvertedIndex(cfg, aggStep, config3.DefaultFrozenStepsThreshold, dirs, logger)
...
func mergeInverted(tb testing.TB, db kv.RwDB, ii *InvertedIndex, txs uint64) {
	...
	for step := kv.Step(0); step < kv.Step(txs/ii.stepSize)-1; step++ {
		func() {
			...
			maxSpan := ii.stepSize * config3.DefaultFrozenStepsThreshold
...
func TestInvIndexScanFiles(t *testing.T) {
	...
	var err error
	ii, err = NewInvertedIndex(cfg, 16, config3.DefaultFrozenStepsThreshold, ii.dirs, logger)
	require.NoError(err)
	defer ii.Close()
	ii.salt.Store(&salt)
