...
func testDbAndDomainOfStep(t *testing.T, aggStep uint64, logger log.Logger) (kv.RwDB, *Domain) {
	...
	d, err := NewDomain(cfg, aggStep, config3.DefaultMaxStepsInFrozenFile, dirs, logger)
	d.salt.Store(&salt)
...
func collateAndMerge(t *testing.T, db kv.RwDB, tx kv.RwTx, d *Domain, txs uint64) {
	...
	var r DomainRanges
	maxSpan := d.stepSize * config3.DefaultMaxStepsInFrozenFile
...
func TestDomain_Prune_AfterAllWrites(t *testing.T) {
	...
	maxFrozenFiles := (txCount / dom.stepSize) / config3.DefaultMaxStepsInFrozenFile
...
func emptyTestDomain(aggStep uint64) *Domain {
	...
	d, err := NewDomain(cfg, aggStep, config3.DefaultMaxStepsInFrozenFile, dirs, log.New())
	if err != nil {
		panic(err)
	}
	d.salt.Store(&salt)
...
func filledDomainFixedSize(t *testing.T, keysCount, txCount, aggStep uint64, logger log.Logger) (kv.RwDB, *Domain, map[uint64][]bool) {
	...
	var k [8]byte
	var v [8]byte
	maxFrozenFiles := (txCount / d.stepSize) / config3.DefaultMaxStepsInFrozenFile
	prev := map[string]string{}
