...
// TestDomain_DeletedKeyNotResurrectedByFiles verifies that a deleted storage
// slot is not resurrected by stale data in snapshot files.
//
// Scenario: write a value at step 0, delete it at step 1, build files covering
// both steps WITHOUT pruning the DB. The deletion entry stays in the DB at
// step 1, but files.EndTxNum() advances past step 1. Before the fix,
// getLatestFromDb discarded the deletion entry because its step was "old"
// relative to files, then getLatestFromFiles returned the stale pre-deletion
// value from the step-0 file.
func TestDomain_DeletedKeyNotResurrectedByFiles(t *testing.T) {
	...
	logger := log.New()
	db, d := testDbAndDomain(t, logger) // stepSize=16
	ctx, require := context.Background(), require.New(t)
	tx, err := db.BeginRw(ctx)
	require.NoError(err)
	defer tx.Rollback()
	...
	// Step 0 (txNum 0-15): write value
	require.NoError(writer.PutWithPrev(key, value, 0, nil))
	// Fill the rest of step 0 with writes to a different key so the step is complete
	for txNum := uint64(1); txNum < d.stepSize; txNum++ {
		require.NoError(writer.PutWithPrev([]byte("filler"), []byte("x"), txNum, []byte("x")))
	}
	// Step 1 (txNum 16-31): delete key
	require.NoError(writer.DeleteWithPrev(key, d.stepSize, value))
	for txNum := d.stepSize + 1; txNum < d.stepSize*2; txNum++ {
		require.NoError(writer.PutWithPrev([]byte("filler"), []byte("x"), txNum, []byte("x")))
	}
	// Steps 2-3 (txNum 32-63): filler to produce enough steps for collation
	for txNum := d.stepSize * 2; txNum < d.stepSize*4; txNum++ {
		require.NoError(writer.PutWithPrev([]byte("filler"), []byte("x"), txNum, []byte("x")))
	}
	...
	// Build files for steps 0,1,2 WITHOUT pruning.
	// This advances files.EndTxNum() past the deletion step while leaving
	// the deletion entry in the DB.
	for step := kv.Step(0); step <= 2; step++ {
		collateAndMergeOnce(t, d, tx, step, false)
	}

	// Re-open domain view — files now cover steps 0-2
	domainRoTx = d.BeginFilesRo()
	defer domainRoTx.Close()

	// files.EndTxNum() should be > lastTxNumOfStep(1, 16) = 31
...
// TestDomain_DeletionSurvivesPruning verifies that when a key is deleted
// (empty value) and the deletion's step is subsequently collated into a file
// and pruned, the deletion entry is preserved in the DB by the
// IsDeletionEntry callback. Without this, the deletion marker is removed and
// getLatestFromFiles returns a stale pre-deletion value from an older file.
func TestDomain_DeletionSurvivesPruning(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	t.Parallel()
	...
	// Build files for steps 0-2 WITH pruning via the scanning prune path
	// (which uses the IsDeletionEntry callback).
	for step := kv.Step(0); step <= 2; step++ {
...
// value written at a step is NOT affected by the deletion-preservation logic —
// it is still pruned normally after being collated into a file.
func TestDomain_NonDeletionEntrySurvivesPruning(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	t.Parallel()
	...
	// Steps 1-2: filler only (key1 not touched)
	for txNum := d.stepSize; txNum < d.stepSize*3; txNum++ {
		require.NoError(writer.PutWithPrev([]byte("filler"), []byte("x"), txNum, []byte("x")))
	}
	require.NoError(writer.Flush(ctx, tx))
	writer.Close()
	...
	// Build and prune steps 0-1. The non-deletion entry at step 0 should
	// be pruned from the DB (it's in the file now).
	...
	// The value should still be readable (from the file).
	v, _, found, err := domainRoTx.GetLatest(key, tx)
	require.NoError(err)
	require.True(found)
	require.Equal(value, v)

	// But the DB entry at step 0 should have been pruned — verify by
	// checking that getLatestFromDb returns found=false.
	_, _, dbFound, err := domainRoTx.getLatestFromDb(key, tx)
	require.NoError(err)
	require.False(dbFound, "non-deletion DB entry should be pruned after collation")
}
