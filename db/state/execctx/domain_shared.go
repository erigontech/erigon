...
type SharedDomains struct {
	...
	// See origin/mh/parallel-exec-fixes for the full rationale: parallel
	// commitment calculator swaps the global accumulator pointer to route
	// its own per-block branch writes into block N's CS; concurrent DomainPut
	// calls for block N+1 would otherwise land in N's CS and corrupt unwind.
	// Band-aid until the accumulator is moved out of the execution path.
