...
type SharedDomains struct {
	...
	//
	// versionedIOBlock stamps which block the carried tracking belongs to. It is
	// load-bearing: staged sync reuses one SharedDomains across every block of a
	// batch, so without the stamp block N's version map would be adopted by block
	// N+1 and stale versioned reads would corrupt nonces and the state root. Only
	// a flashblock continuation — same block number — may adopt it.
	versionedIO      VersionedIO
	versionMap       VersionMap
	versionedIOBlock uint64
...
// SetParallelExecBlock stamps the carried tracking with the block it belongs to.
// SetVersionedIO/SetVersionMap callers must set this too, or the state will never
// be adopted.
func (sd *SharedDomains) SetParallelExecBlock(blockNum uint64) { sd.versionedIOBlock = blockNum }

// ParallelExecBlock reports the block the carried tracking belongs to.
func (sd *SharedDomains) ParallelExecBlock() uint64 { return sd.versionedIOBlock }

func (sd *SharedDomains) ResetParallelExecState() {
	sd.versionedIO = nil
	sd.versionMap = nil
	sd.versionedIOBlock = 0
}
