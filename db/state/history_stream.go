...
type HistoryChangesIterDB struct {
	...
	// For vlog dereferencing in large values mode
	ht *HistoryRoTx
...
func (hi *HistoryChangesIterDB) advanceLargeVals() error {
	...
	for k, v, err := hi.valsC.Seek(seek); k != nil; k, v, err = hi.valsC.Seek(seek) {
		...
		// Dereference vlog if large values mode and value is a reference
		// - len(v) == 0: empty value
		// - len(v) == 8: vlog reference (need to dereference)
		// - other: inline value (backward compatibility)
		if len(v) == 0 {
			// Empty value
			hi.nextVal = v
		} else if len(v) == 8 && hi.ht != nil {
