...
type HistCfg struct {
	...
	// ValuesTable is the InvIndexTable (DupSort): key → txNum
	// The DataTable (non-DupSort, txNum+key → prev_value) is IiCfg.KeysTable
	ValuesTable string // bucket for history inverted index; key -> txNum (DupSort, no embedded value)
