...
func emptyTestInvertedIndex(aggStep uint64) *InvertedIndex {
	...
	ii, err := NewInvertedIndex(cfg, aggStep, config3.DefaultMaxStepsInFrozenFile, dirs, log.New())
	ii.Accessors = 0
	ii.salt.Store(&salt)
	if err != nil {
		panic(err)
...
func TestMergeFilesWithDependency(t *testing.T) {
	...
	newTestDomain := func(dom kv.Domain) *Domain {
		...
		d, err := NewDomain(cfg, 1, config3.DefaultMaxStepsInFrozenFile, dirs, log.New())
		if err != nil {
			panic(err)
		}
		d.salt.Store(&salt)
