...
func stateRootsDiffer(a, b []byte) bool {
	...
	_, _, rootA, errA := v4.DecodeState(a)
	_, _, rootB, errB := v4.DecodeState(b)
				...
				for c, span := range batch.spans {
					...
					if commitment.IsCommitmentStateKey(legacyKey) {
						for i, txNum := range txNums {
							...
							if emitErr := emit(v4.StateKey(), txNum, before, v4.LegacyDirect); emitErr != nil {
								return emitErr
							}
							next, err := after(i)
							if err != nil {
...
func verifyCommitmentV3History(ctx context.Context, a *Aggregator, ranges []commitmentHistoryRange, logger log.Logger) error {
	...
	var samples []uint64
	for _, r := range ranges {
		txNums, err := appendFileTxNums(nil, iit, uint64(r.fromStep)*stepSize, uint64(r.toStep)*stepSize, v4.StateKey())
		if err != nil {
			at.Close()
			return err
		}
