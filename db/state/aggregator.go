...
type Aggregator struct {
	...
	// set via SetAwaiter by production wiring (storage.Provider). nil
	// for tools/tests.
	awaiterMu sync.RWMutex
	awaiter   views.Awaiter
}

// SetAwaiter wires the wait-on-miss dependency. Safe to call concurrently
// with reads.
...
func (at *AggregatorRoTx) DebugGetLatestFromFilesUpToStep(domain kv.Domain, k []byte, maxStep kv.Step) (v []byte, fileEndStep kv.Step, found bool, err error) {
	...
	if domain == kv.CommitmentDomain {
		...
		for _, v := range slices.Backward(at.d[domain].files) {
			if v.endTxNum == fileEndTxNum {
				fileStartTxNum = v.startTxNum
				break
			}
		}
		v, err = at.replaceShortenedKeysInBranch(k, commitment.BranchData(v), fileStartTxNum, fileEndTxNum)
