...
func RebuildCommitmentFiles(ctx context.Context, rwDb kv.TemporalRwDB, txNumsReader *rawdbv3.TxNumsReader, logger log.Logger, squeeze bool) (latestRoot []byte, err error) {
	...
	// Disable ReplaceKeysInValues during the rebuild loop. The merge loop after each
	// range would otherwise shorten keys in commitment branch data, embedding file-range
	// references (e.g. storage.0-16) that become invalid once those files are merged into
	// larger ranges (storage.0-32). The squeeze pass at the end re-enables this and applies
	// the shortening in one shot when all files are finalized.
	a.ForTestReplaceKeysInValues(kv.CommitmentDomain, false)
