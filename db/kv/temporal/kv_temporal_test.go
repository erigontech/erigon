...
func TestTemporalTx_HasPrefix_StorageDomain(t *testing.T) {
	...
	{
		// write to storage
		err = sd.PutStorage(ctx, accounts.InternAddress(acc1), accounts.InternKey(acc1slot1), u256.Num1, rwTtx1, 1)
		require.NoError(t, err)
	...
	// --- check 3: storage exists in files only - TemporalTx.HasPrefix should catch this
	{
		...
		err = sd.PutStorage(ctx, accounts.InternAddress(acc2), accounts.InternKey(acc2slot2), u256.Num2, rwTtx2, 2)
		require.NoError(t, err)
	...
	// --- check 5: write to it again after deletion - TemporalTx.HasPrefix should catch
	{
		...
		err = sd.PutStorage(ctx, accounts.InternAddress(acc1), accounts.InternKey(acc1slot1), u256.U64(3), rwTtx5, 4)
		require.NoError(t, err)
...
func TestTemporalTx_RangeAsOf_StorageDomain(t *testing.T) {
	...
	err = sd.PutStorage(ctx, accounts.InternAddress(acc1), accounts.InternKey(acc1slot1), u256.U64(1), rwTtx1, 1)
	require.NoError(t, err)
	err = sd.Flush(ctx, rwTtx1)
	require.NoError(t, err)
	err = rwTtx1.Commit()
	...
	require.NoError(t, err)
	defer rwTtx2.Rollback()
	err = sd.PutStorage(ctx, accounts.InternAddress(acc1), accounts.InternKey(acc1slot1), u256.U64(2), rwTtx2, 2)
	require.NoError(t, err)
	err = sd.Flush(ctx, rwTtx2)
	require.NoError(t, err)
	err = rwTtx2.Commit()
	...
	require.NoError(t, err)
	defer rwTtx3.Rollback()
	err = sd.DelStorage(ctx, accounts.InternAddress(acc1), accounts.NilKey, rwTtx3, 3)
	require.NoError(t, err)
	...
	err = sd.PutStorage(ctx, accounts.InternAddress(acc1), accounts.InternKey(acc1slot1), u256.U64(3), rwTtx4, 4)
	require.NoError(t, err)
	err = sd.Flush(ctx, rwTtx4)
	require.NoError(t, err)
	err = rwTtx4.Commit()
