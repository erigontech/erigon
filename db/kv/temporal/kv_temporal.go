...
func (tx *RwTx) HistoryKeyTrace(domain kv.Domain, k []byte, fromTxNum, toTxNum uint64) (stream.U64V, error) {
	return tx.aggtx.DebugHistoryKeyTrace(tx.ctx, domain, k, fromTxNum, toTxNum, tx.RwTx)
}
...
func (tx *Tx) HistoryKeyTrace(domain kv.Domain, k []byte, fromTxNum, toTxNum uint64) (stream.U64V, error) {
	return tx.aggtx.DebugHistoryKeyTrace(tx.ctx, domain, k, fromTxNum, toTxNum, tx.Tx)
}
