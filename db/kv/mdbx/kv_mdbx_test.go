...
func BaseCaseDB(t *testing.T) kv.RwDB {
	...
	logger := log.New()
	table := "Table"
	db := New(dbcfg.ChainDB, logger).InMem(path, false).WithTableCfg(func(defaultBuckets kv.TableCfg) kv.TableCfg {
		return kv.TableCfg{
...
func TestBeginRoAfterClose(t *testing.T) {
	db := New(dbcfg.ChainDB, log.New()).InMem(t.TempDir(), false).MustOpen()
	db.Close()
...
func TestAutoRemove(t *testing.T) {
	...
	t.Run("autoRemove enabled", func(t *testing.T) {
		db := New(dbcfg.TemporaryDB, logger).InMem(t.TempDir(), true).AutoRemove(true).MustOpen()
		mdbxDB := db.(*MdbxKV)
		dbPath := mdbxDB.Path()
	...
	t.Run("autoRemove disabled", func(t *testing.T) {
		db := New(dbcfg.TemporaryDB, logger).InMem(t.TempDir(), true).AutoRemove(false).MustOpen()
		mdbxDB := db.(*MdbxKV)
		dbPath := mdbxDB.Path()
