...
type Index struct {
	...
	existenceV2        *fusefilter.ShardedReader
...
func (idx *Index) init() (err error) {
	...
	if idx.lessFalsePositives && idx.keyCount > 0 {
		...
		case 2:
			var sz int
			idx.existenceV2, sz, err = fusefilter.NewShardedReaderOnBytes(idx.data[offset:], idx.fileName)
			if err != nil {
				return fmt.Errorf("NewShardedReaderOnBytes: %w, %s", err, idx.fileName)
			}
			if fusefilter.MadvWillNeedByDefault {
