...
func NewRecSplit(args RecSplitArgs, logger log.Logger) (*RecSplit, error) {
	...
	// Size count buffer for 8-way parallel salt testing in findSplit.
	// Needs at least 8*maxFanout slots where maxFanout = primaryAggrBound/leafSize.
	maxFanout := rs.primaryAggrBound / rs.leafSize
	countSize := rs.secondaryAggrBound
	if need := 8 * maxFanout; need > countSize {
		countSize = need
	}
	rs.count = make([]uint16, countSize)
	if args.NoFsync {
		rs.DisableFsync()
	}
	closeFiles = false
