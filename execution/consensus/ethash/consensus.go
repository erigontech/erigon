...
func (ethash *Ethash) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parentHeader *types.Header) *big.Int {
	return CalcDifficulty(chain.Config(), time, parentHeader.Time, parentHeader.Difficulty, parentHeader.Number.Uint64(), parentHeader.UncleHash)
}
