...
func TestEIP8246FundedThenSelfdestructedAccount(t *testing.T) {
	for _, mode := range []struct {
		...
		t.Run(mode.name, func(t *testing.T) {
			...
			chainPack, err := m.GenerateChain(1, func(_ int, block *blockgen.BlockGen) {
				...
				block.AddTx(sign(types.NewTransaction(block.TxNonce(sender), contract, uint256.NewInt(1), 300_000, gasPrice, nil)))
			...
			require.NoError(t, m.DB.ViewTemporal(t.Context(), func(tx kv.TemporalTx) error {
				...
				require.NoError(t, err)
				require.Equal(t, *uint256.NewInt(2), balance)
				return nil
			}))
		})
	}
