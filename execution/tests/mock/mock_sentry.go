...
func MockWithEverything(tb testing.TB, gspec *types.Genesis, key *ecdsa.PrivateKey, prune prune.Mode, engine rules.Engine, blockBufferSize int, withTxPool, withPosDownloader bool) *MockSentry {
	...
	if !cfg.TxPool.Disable {
		...
		mock.TxPool, mock.TxPoolGrpcServer, err = txpool.Assemble(
			...
			txpool.WithPoolDBInitializer(func(_ context.Context, _ txpoolcfg.Config, _ log.Logger) (kv.RwDB, error) {
				return mdbx.New(dbcfg.TxPoolDB, logger).InMem(tmpdir, false).MustOpen(), nil
			}),
		)
		if err != nil {
			tb.Fatal(err)
	...
	inMemoryExecution := func(sd *state.ExecutionContext, tx kv.TemporalRwTx, unwindPoint uint64, headersChain []*types.Header, bodiesChain []*types.RawBody,
