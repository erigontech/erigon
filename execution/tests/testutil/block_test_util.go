...
// RunWithTester is like Run but returns the underlying ExecModuleTester so
// callers that need RPC-level access to the test DB (e.g. witness comparison
// tests calling debug_executionWitness) can use it after block import. The
// tester's lifetime is bound to t via execmoduletester.New's t.Cleanup; callers
