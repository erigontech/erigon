...
func (st *StateTransition) TransitionDb(refunds bool, gasBailout bool) (result *evmtypes.ExecutionResult, err error) {
	...
	if st.state.BlockNumber() == 2451926 {
		fmt.Printf("[DEBUG-2451926] txIdx=%d from=%x gasLimit=%d txnGasUsed=%d txnGasUsedB4Refunds=%d floorGasCost=%d imdGasRegular=%d vmerr=%v\n",
