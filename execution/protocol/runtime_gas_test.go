...
func TestFrameV2RuntimeFailure(t *testing.T) {
	...
	HandleRuntimeFailure(evm, vm.CALL, accounts.ZeroAddress, accounts.InternAddress(common.HexToAddress("0x04")), nil, initial, &remaining, uint256.Int{}, vm.ErrRuntimeOutOfGas)
	require.Equal(t, []mdgas.MdGas{initial}, entered)
	require.Equal(t, []mdgas.MdGas{{State: initial.State}}, exited)
	require.Equal(t, exited[0], remaining)
}
