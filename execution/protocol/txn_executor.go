...
func (st *TxnExecutor) Execute(refunds bool, gasBailout bool) (result *evmtypes.ExecutionResult, err error) {
	...
	if th != nil {
		effectiveRefund := st.txnGasUsedB4Refunds - st.txnGasUsed
		th.HashGasAccounting(st.txnGasUsed, st.state.GetRefund().Regular, effectiveRefund, st.gasRemaining.Regular, st.blockRegularGasUsed)
	}
	...
	if th != nil {
		gasReturnValue := u256.Mul(u256.U64(st.gasRemaining.Regular), *st.gasPrice)
