...
func (cw *CachedWriter) UpdateAccountCode(address common.Address, incarnation uint64, codeHash common.Hash, code []byte) error {
	if err := cw.w.UpdateAccountCode(address, 0, codeHash, code); err != nil {
		return err
	}
	cw.cache.SetCodeWrite(address.Bytes(), 0, code)
	return nil
}
...
func (cw *CachedWriter) WriteAccountStorage(address common.Address, incarnation uint64, key common.Hash, original, value uint256.Int) error {
	if err := cw.w.WriteAccountStorage(address, 0, key, original, value); err != nil {
		return err
	}
	if original == value {
		return nil
	...
	if value.IsZero() {
		cw.cache.SetStorageDelete(address.Bytes(), 0, key[:])
	} else {
		cw.cache.SetStorageWrite(address.Bytes(), 0, key[:], value.Bytes())
	}
	return nil
}
