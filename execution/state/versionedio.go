...
type accountState struct {
	...
	selfDestructed          bool                                //
	selfDestructedAt        uint64                              // access index of the selfdestruct
...
func (ft *fieldTracker[T]) recordWrite(idx uint64, value T, copyFn func(T) T, equal func(T, T) bool) {
...
func applyToBalance(bt *fieldTracker[uint256.Int], ac *types.AccountChanges, initialBalance *uint256.Int) {
	...
	firstFiltered := false
	bt.changes.apply(func(idx uint64, value uint256.Int) {
		if !firstFiltered {
			firstFiltered = true
...
func applyToNonce(nt *fieldTracker[uint64], ac *types.AccountChanges) {
	nt.changes.apply(func(idx uint64, value uint64) {
...
func applyToCode(ct *fieldTracker[[]byte], ac *types.AccountChanges) {
	ct.changes.apply(func(idx uint64, value []byte) {
...
func (ct *changeTracker[T]) recordWrite(idx uint64, value T, copyFn func(T) T, equal func(T, T) bool) {
	if ct.entries == nil {
		ct.entries = make(map[uint64]T)
		ct.equal = equal
	}
	ct.entries[idx] = copyFn(value)
}

func (ct *changeTracker[T]) apply(applyFn func(uint64, T)) {
	if len(ct.entries) == 0 {
		return
	}

	indices := make([]uint64, 0, len(ct.entries))
...
func (account *accountState) updateWrite(vw *VersionedWrite, accessIndex uint64) {
	switch vw.Path {
	case StoragePath:
	...
	if vr != nil {
		...
		case StoragePath:
			...
			if val, ok := vr.Val.(uint256.Int); ok {
				...
				if _, exists := account.storageReadValues[vr.Key]; !exists {
					account.storageReadValues[vr.Key] = val
				}
			}
...
func addStorageUpdate(ac *types.AccountChanges, vw *VersionedWrite, txIndex uint64) {
