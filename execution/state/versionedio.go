...
type ReadSet map[Address]map[AccountKey]*VersionedRead
...
func (s ReadSet) Delete(addr Address, key AccountKey) {
...
type WriteSet map[Address]map[AccountKey]*VersionedWrite
...
func (s WriteSet) Delete(addr Address, key AccountKey) {
...
func (vr *versionedStateReader) ReadAccountData(address Address) (*accounts.Account, error) {
...
func versionedUpdate[T any](versionMap *VersionMap, addr Address, path AccountPath, key StorageKey, txIndex int) (T, bool) {
...
func (vr versionedStateReader) applyVersionedUpdates(address Address, account accounts.Account) accounts.Account {
...
func (vr versionedStateReader) ReadAccountDataForDebug(address Address) (*accounts.Account, error) {
	if r, ok := vr.reads[address][AccountKey{Path: AddressPath}]; ok && r.Val != nil {
		if account, ok := r.Val.(*accounts.Account); ok {
			updated := vr.applyVersionedUpdates(address, *account)
			return &updated, nil
...
func (vr versionedStateReader) ReadAccountStorage(address Address, key StorageKey) (uint256.Int, bool, error) {
...
func (vr versionedStateReader) HasStorage(address Address) (bool, error) {
...
func (vr versionedStateReader) ReadAccountCode(address Address) ([]byte, error) {
...
func (vr versionedStateReader) ReadAccountCodeSize(address Address) (int, error) {
	if r, ok := vr.reads[address][AccountKey{Path: CodePath}]; ok && r.Val != nil {
		if code, ok := r.Val.([]byte); ok {
			return len(code), nil
		}
...
func (vr versionedStateReader) ReadAccountIncarnation(address Address) (uint64, error) {
	if r, ok := vr.reads[address][AccountKey{Path: AddressPath}]; ok && r.Val != nil {
...
func (writes VersionedWrites) HasNewWrite(cmpSet []*VersionedWrite) bool {
	...
	cmpMap := map[Address]map[AccountKey]struct{}{}
...
func versionedRead[T any](s *IntraBlockState, addr Address, path AccountPath, key StorageKey, commited bool, defaultV T, copyV func(T) T, readStorage func(sdb *stateObject) (T, error)) (T, ReadSource, Version, error) {
	if s.versionMap == nil {
...
type VersionedIO struct {
	...
	outputsSet []map[Address]map[AccountKey]struct{}
}
...
func (io *VersionedIO) HasWritten(txnIdx int, addr Address, path AccountPath, key StorageKey) bool {
...
func NewVersionedIO(numTx int) *VersionedIO {
	return &VersionedIO{
		...
		outputsSet: make([]map[Address]map[AccountKey]struct{}, numTx+1),
	}
}
...
func (io *VersionedIO) RecordWrites(txVersion Version, output VersionedWrites) {
	...
	if len(io.outputsSet) <= txId+1 {
		io.outputsSet = append(io.outputsSet, make([]map[Address]map[AccountKey]struct{}, txId+2-len(io.outputsSet))...)
	}
	io.outputsSet[txId+1] = map[Address]map[AccountKey]struct{}{}
