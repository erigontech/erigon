...
func TestGenesisBlockRoots(t *testing.T) {
	...
	block, _, err := genesiswrite.GenesisToBlock(chainspec.MainnetGenesisBlock(), datadir.New(t.TempDir()), false, log.Root())
	require.NoError(err)
	...
	for _, netw := range []string{
		...
		block, _, err = genesiswrite.GenesisToBlock(spec.Genesis, datadir.New(t.TempDir()), false, log.Root())
		require.NoError(err)
