...
func (writes *WriteSet) Normalize(vm *VersionMap, txIndex int, incarnation int, stateReader StateReader, domainStorageKeys func(addr accounts.Address) []accounts.StorageKey, emptyRemoval bool, isAura bool, eip8246 bool) (*WriteSet, error) {
	...
	// write it would otherwise have to look up again by address.
	//
	// SD'd addresses lose their account-field and raw storage writes here, so
	// applyVersionedWrites takes the pure-delete branch instead of
	// cleanup-before-recreate; the self-destruct loop re-emits an explicit
	// StoragePath=0 delete for every slot via sdStorageSlots.
	...
	// Code size is derived from the code bytes and isn't a domain field, so it
	// is intentionally not carried into the calc/apply write set (as on
	// serial). Cross-tx ReadCodeSize is served from the versionMap, which the
	// worker populates directly — independent of this pass. The address still
	// counts as dirty for the fill loop below.
	for addr := range writes.codeSize {
		allAddresses[addr] = true
	}

	// writes.address is not walked: AddressPath is record-level — skip for
	// field-level consumers. It carries no field of its own, so it neither
	...
	for _, inner := range writes.storage {
		...
		for _, sw := range inner {
			...
			originVal, origin, originOK := vm.ReadStorage(sw.Address, sw.Key, txIndex)
			...
			case stateReader != nil:
				...
				if revived {
					...
					preVal, found, err := stateReader.ReadAccountStorage(sw.Address, sw.Key)
					if err != nil {
						return nil, err
					}
			...
			filtered.SetStorage(sw.Address, sw.Key, sw)
		}
	}
