...
// SetVersionedReads replaces the tracked versioned reads. Callers implementing
// snapshot/restore semantics MUST capture the prior state via ReadSet.Clone() —
// VersionedReads() returns a live reference, and subsequent versionedRead.Set()
// calls would mutate a non-cloned snapshot in place, making the restore a no-op.
// Used to discard synthetic reads generated by operations the caller does not
// want in the final BAL (e.g. the parallel executor's finalize-path
// postApplyMessageFunc / FinalizeTx re-invocation on a reconstructed IBS).
