...
func TestChangeAccountCodeBetweenBlocks(t *testing.T) {
	...
	rh1, err := sd.ComputeCommitment(context.Background(), tx, nil, true, blockNum, txNum, "", nil, 0)
	require.NoError(t, err)
	...
	rh2, err := sd.ComputeCommitment(context.Background(), tx, nil, true, blockNum, txNum, "", nil, 0)
	require.NoError(t, err)
...
func TestCacheCodeSizeInTrie(t *testing.T) {
	...
	r2, err := sd.ComputeCommitment(context.Background(), tx, nil, true, blockNum, txNum, "", nil, 0)
	require.NoError(t, err)
	...
	r2, err = sd.ComputeCommitment(context.Background(), tx, nil, true, 1, 2, "", nil, 0)
	require.NoError(t, err)
	require.Equal(t, root, common.CastToHash(r2))
}
