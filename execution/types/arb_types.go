...
func (t *ArbitrumRetryTx) Sender(signer Signer) (accounts.Address, error) {
	return accounts.InternAddress(t.From), nil
}
...
func (tx *ArbitrumSubmitRetryableTx) AsMessage(s Signer, baseFee *big.Int, rules *chain.Rules) (*Message, error) {
	msg := &Message{
		...
		accessList: tx.GetAccessList(),
		from:       accounts.InternAddress(tx.From),
		to:         accounts.InternAddress(ArbRetryableTxAddress),
		data:       tx.GetData(),
		amount:     *tx.GetValue(),
		checkNonce: !skipAccountChecks[tx.Type()],
...
func (tx *ArbitrumInternalTx) AsMessage(s Signer, baseFee *big.Int, rules *chain.Rules) (*Message, error) {
	msg := &Message{
		...
		nonce:        tx.GetNonce(),
		accessList:   tx.GetAccessList(),
		from:         accounts.InternAddress(ArbosAddress),
		to:           accounts.InternAddress(ArbosAddress),
		data:         tx.GetData(),
		amount:       *tx.GetValue(),
		checkNonce:   !skipAccountChecks[tx.Type()],
		Tx:           tx,
