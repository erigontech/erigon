...
func TestReceiptsEncodeRLP69(t *testing.T) {
	t.Run("ProperRLPListEncoding", func(t *testing.T) {
		receipts := Receipts{
			&Receipt{
				Type:              LegacyTxType,
			...
			&Receipt{
				...
				Logs: []*Log{
					{
						Address: common.BytesToAddress([]byte{0x22}),
						Topics:  []common.Hash{common.HexToHash("beef")},
						Data:    []byte{0x03, 0x04},
					},
				},
			},
		...
		var decoded Receipts
		err = rlp.DecodeBytes(encoded, &decoded)
		require.NoError(t, err)

		require.Equal(t, len(receipts), len(decoded))
		for i := range receipts {
			require.Equal(t, receipts[i].Type, decoded[i].Type)
			require.Equal(t, receipts[i].Status, decoded[i].Status)
			require.Equal(t, receipts[i].CumulativeGasUsed, decoded[i].CumulativeGasUsed)
			require.Equal(t, len(receipts[i].Logs), len(decoded[i].Logs))
		}

		// should produce the same result as standard RLP encoding
		standardEncoded, err := rlp.EncodeToBytes(receipts)
		require.NoError(t, err)
		require.Equal(t, standardEncoded, encoded, "EncodeRLP69 should produce same result as rlp.Encode")
	})

	t.Run("EmptyReceipts", func(t *testing.T) {
		// Test with empty receipts slice
		emptyReceipts := Receipts{}
		var buf bytes.Buffer
		err := emptyReceipts.EncodeRLP69(&buf)
		require.NoError(t, err)
		emptyEncoded := buf.Bytes()

		var emptyDecoded Receipts
		err = rlp.DecodeBytes(emptyEncoded, &emptyDecoded)
		require.NoError(t, err)
		require.Empty(t, emptyDecoded)

		// Should produce same result as standard RLP encoding
		standardEncoded, err := rlp.EncodeToBytes(emptyReceipts)
		require.NoError(t, err)
		require.Equal(t, standardEncoded, emptyEncoded, "Empty receipts EncodeRLP69 should produce same result as rlp.Encode")
	})
}
