...
func TestCheckCompatible(t *testing.T) {
	...
	tests := []test{
		...
		{
			stored:  &chain.Config{TangerineWhistleBlock: newUint64(10)},
			new:     &chain.Config{TangerineWhistleBlock: newUint64(20)},
			head:    9,
			wantErr: nil,
		},
		{
		...
		{
			stored: chain.AllProtocolChanges,
			new:    &chain.Config{HomesteadBlock: newUint64(1)},
			head:   3,
			wantErr: &chain.ConfigCompatError{
				What:         "Homestead fork block",
				StoredConfig: newUint64(0),
				NewConfig:    newUint64(1),
				RewindTo:     0,
			},
		},
		{
			stored: &chain.Config{HomesteadBlock: newUint64(30), TangerineWhistleBlock: newUint64(10)},
			new:    &chain.Config{HomesteadBlock: newUint64(25), TangerineWhistleBlock: newUint64(20)},
			head:   25,
			wantErr: &chain.ConfigCompatError{
				What:         "Tangerine Whistle fork block",
				StoredConfig: newUint64(10),
				NewConfig:    newUint64(20),
				RewindTo:     9,
			},
		},
		{
			stored:  &chain.Config{ConstantinopleBlock: newUint64(30)},
			new:     &chain.Config{ConstantinopleBlock: newUint64(30), PetersburgBlock: newUint64(30)},
			head:    40,
			wantErr: nil,
		},
		{
			stored: &chain.Config{ConstantinopleBlock: newUint64(30)},
			new:    &chain.Config{ConstantinopleBlock: newUint64(30), PetersburgBlock: newUint64(31)},
			head:   40,
			wantErr: &chain.ConfigCompatError{
				What:         "Petersburg fork block",
				StoredConfig: nil,
				NewConfig:    newUint64(31),
				RewindTo:     30,
			},
		},
	}
