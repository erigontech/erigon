...
func NewDefaultStages(ctx context.Context,
	...
	return stagedsync.DefaultStages(
		...
		stagedsync.StageExecuteBlocksCfg(db, cfg.Prune, cfg.BatchSize, controlServer.ChainConfig, controlServer.Engine, &vm.Config{Tracer: tracingHooks, UseGevm: cfg.UseGevm}, notifications, cfg.StateStream, false /* badBlockHalt */, dirs, blockReader, controlServer.Hd, cfg.Genesis, cfg.Sync, SilkwormForExecutionStage(silkworm, cfg), cfg.ExperimentalBAL),
...
func NewPipelineStages(ctx context.Context,
	...
	return stagedsync.PipelineStages(ctx,
		...
		stagedsync.StageBlockHashesCfg(dirs.Tmp, blockWriter),
		stagedsync.StageSendersCfg(controlServer.ChainConfig, cfg.Sync, false /* badBlockHalt */, dirs.Tmp, cfg.Prune, blockReader, controlServer.Hd),
		stagedsync.StageExecuteBlocksCfg(db, cfg.Prune, cfg.BatchSize, controlServer.ChainConfig, controlServer.Engine, &vm.Config{Tracer: tracingHooks, UseGevm: cfg.UseGevm}, notifications, cfg.StateStream, false, dirs, blockReader, controlServer.Hd, cfg.Genesis, cfg.Sync, SilkwormForExecutionStage(silkworm, cfg), cfg.ExperimentalBAL),
		stagedsync.StageTxLookupCfg(cfg.Prune, dirs.Tmp, blockReader),
		stagedsync.StageFinishCfg(forkValidator),
...
func NewInMemoryExecution(
	...
	return stagedsync.New(
		...
		stagedsync.StateStages(ctx,
			...
			stagedsync.StageExecuteBlocksCfg(db, cfg.Prune, cfg.BatchSize, controlServer.ChainConfig, controlServer.Engine, &vm.Config{UseGevm: cfg.UseGevm}, notifications, cfg.StateStream, true, cfg.Dirs, blockReader, controlServer.Hd, cfg.Genesis, cfg.Sync, SilkwormForExecutionStage(silkworm, cfg), cfg.ExperimentalBAL),
		),
