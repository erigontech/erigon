...
type parallelExecutor struct {
	...
	// calculator is the commitment calculator this batch runs against; the
	// exec loop only reads it, for the COMMITMENT_AFTER_EXEC barrier.
...
func (pe *parallelExecutor) completeBlock(ctx context.Context, blockResult *blockResult, sizeCutPending *bool) (exit bool, err error) {
	...
	if ok {
		...
		if dbg.CommitmentAfterExec && pe.calculator != nil {
			if err := pe.calculator.WaitProcessed(ctx, blockNum); err != nil {
				return false, err
			}
		}
