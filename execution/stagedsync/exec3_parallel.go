...
func (pe *parallelExecutor) execImpl(ctx context.Context, execStage *StageState, u Unwinder,
	...
	// || KeepExecutionProofs`): blocks from the changeset window (perBlockFrom)
	// onward compute per-block — otherwise batch-mode dedupes branch updates
	// across the batch and flushes them into one block's changeset, which fails
	// on subsequent reorgs. The BAL-driven fold path feeds via blockRequests.
