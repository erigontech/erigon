...
func (pe *parallelExecutor) rwLoop(ctx context.Context, maxTxNum uint64, logger log.Logger) error {
	...
	for pe.outputTxNum.Load() <= maxTxNum {
		...
		case <-pe.logEvery.C:
			...
			pe.progress.Log("", pe.rs, pe.in, pe.rws, pe.rs.DoneCount(), 0 /* TODO logGas*/, pe.inputBlockNum.Load(), pe.outputBlockNum.GetValueUint64(), pe.outputTxNum.Load(), mxExecRepeats.GetValueUint64(), stepsInDB, pe.shouldGenerateChangeSets() || pe.cfg.syncCfg.KeepExecutionProofs, pe.inMemExec, 0)
