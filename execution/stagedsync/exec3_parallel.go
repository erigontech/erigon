		...
		if valid {
			if cntInvalid == 0 {
...

				{
					...
					txResult.writes = rawWrites.Normalize(be.versionMap, txVersion.TxIndex, resultIncarnation, stateReader, domainStorageKeys, emptyRemoval, pe.cfg.chainConfig.Aura != nil)
				}
	...
	if be.publishTasks.countComplete() == len(be.tasks) && be.execTasks.countComplete() == len(be.tasks) {
		...
		if be.blockNum > 0 {
			...
			if tt, ok := lastResult.Task.(*taskVersion).Task.(*exec.TxTask); ok {
				...
				emptyRemoval := be.blockNum != 0 && pe.cfg.chainConfig.IsEIP161Enabled(be.blockNum)
				finalizeWrites = ivw.Normalize(be.versionMap, finalVersion.TxIndex, finalVersion.Incarnation, reader, domainStorageKeys, emptyRemoval, pe.cfg.chainConfig.Aura != nil)
