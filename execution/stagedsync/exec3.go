...
func ExecV3(ctx context.Context,
	...
	if parallel {
		...
		pe.doms.SetWarmupConfig(commitment.WarmupConfig{
			Enabled:    true,
			NumWorkers: 16,
			MaxDepth:   commitment.WarmupMaxDepth,
			LogPrefix:  pe.logPrefix,
		})
		...
		se.doms.SetWarmupConfig(commitment.WarmupConfig{
			Enabled:    true,
			NumWorkers: 16,
			MaxDepth:   commitment.WarmupMaxDepth,
			LogPrefix:  se.logPrefix,
		})
...
func flushAndCheckCommitmentV3(ctx context.Context, header *types.Header, applyTx kv.TemporalRwTx, doms *execctx.SharedDomains, cfg ExecuteBlockCfg, e *StageState, parallel bool, logger log.Logger, u Unwinder, inMemExec bool) (ok bool, times FlushAndComputeCommitmentTimes, err error) {
	...
	doms.SetWarmupConfig(commitment.WarmupConfig{
		Enabled:    true,
		NumWorkers: 16,
		MaxDepth:   commitment.WarmupMaxDepth,
	})
