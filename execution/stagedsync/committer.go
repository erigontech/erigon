...
func (cc *commitmentCalculator) handleMessage(ctx context.Context, msg applyResult) {
	...
	case *blockResult:
		...
		// foldedAhead: block N was already computed by foldBlockFromBAL ahead
		// of this boundary; in shadow mode recompute incrementally and
		// cross-check, otherwise the verified BAL-driven result already stands.
		// Otherwise per-block mode: forcePerBlockCompute mirrors serial's gate
		// (exec3_serial.go: !dbg.BatchCommitments || shouldGenerateChangesets ||
		// KeepExecutionProofs); blocks from the changeset window (perBlockFrom)
		// onward also compute per-block — per-block branch deltas are required
		// for reorg support / KeepExecutionProofs.
		if cc.foldedAhead[r.BlockNum] {
			if dbg.BALShadowCompute {
				cc.shadowCrossCheck(ctx, r)
			}
		} else if !dbg.BatchCommitments || cc.forcePerBlockCompute || r.BlockNum >= cc.perBlockFrom {
