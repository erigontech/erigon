...
func (evm *EVM) handleFrameRevert(gasRemaining *mdgas.MdGas, err error, snapshot int, stateGasUsed int64, stateGasSpill uint64) {
	// 1. Revert state changes.
	evm.intraBlockState.RevertToSnapshot(snapshot, err)
	// 2. EIP-8037: a reverted frame refunds the state gas it charged.
	if evm.chainRules.IsAmsterdam {
		gasRemaining.Regular += stateGasSpill
		gasRemaining.State = uint64(int64(gasRemaining.State) + stateGasUsed - int64(stateGasSpill))
	}
...
func (evm *EVM) call(typ OpCode, caller accounts.Address, callerAddress accounts.Address, addr accounts.Address, input []byte, gas mdgas.MdGas, value uint256.Int, bailout bool) (ret []byte, gasRemaining mdgas.MdGas, gasUsed mdgas.MdGasUsage, err error) {
	...
	if err != nil || evm.config.RestoreState {
		evm.handleFrameRevert(&gasRemaining, err, snapshot, gasUsed.State, gasUsed.StateSpill)
	}
