		...
		if original.Eq(&current) {
			if original.IsZero() { // create slot (2.1.1)
				if rules.IsAmsterdam {
					return mdgas.MdGas{Regular: cost + params.SstoreSetGasEIP8037, State: params.StateBytesStorageSet * evm.Context.CostPerStateByte}, nil
				} else {
		...
		if original.Eq(&value) {
			if original.IsZero() { // reset to original inexistent slot (2.2.2.1)
				...
				if rules.IsAmsterdam {
					...
					evm.IntraBlockState().AddStateRefund(params.StateBytesStorageSet * evm.Context.CostPerStateByte)
				} else {
