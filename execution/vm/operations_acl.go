...
func makeGasSStoreFunc(clearingRefund uint64) gasFunc {
	return func(evm *EVM, callContext *CallContext, scopeGas uint64, memorySize uint64) (uint64, error) {
		...
		var (
			...
			slot    = types.InternKey(x.Bytes32())
			current uint256.Int
			cost    = uint64(0)
		)
		...
		slotCommited := types.InternKey(x.Bytes32())
...
func gasSLoadEIP2929(evm *EVM, callContext *CallContext, scopeGas uint64, memorySize uint64) (uint64, error) {
	...
	if _, slotMod := evm.IntraBlockState().AddSlotToAccessList(callContext.Address(), types.InternKey(loc.Bytes32())); slotMod {
...
func gasExtCodeCopyEIP2929(evm *EVM, callContext *CallContext, scopeGas uint64, memorySize uint64) (uint64, error) {
	...
	addr := types.InternAddress(callContext.Stack.peek().Bytes20())
...
func makeCallVariantGasCallEIP2929(oldCalculator gasFunc) gasFunc {
	return func(evm *EVM, callContext *CallContext, scopeGas uint64, memorySize uint64) (uint64, error) {
		addr := types.InternAddress(callContext.Stack.Back(1).Bytes20())
...
func makeSelfdestructGasFn(refundsEnabled bool) gasFunc {
	gasFunc := func(evm *EVM, callContext *CallContext, scopeGas uint64, memorySize uint64) (uint64, error) {
		var (
			gas     uint64
			address = types.InternAddress(callContext.Stack.peek().Bytes20())
		)
		// If the caller cannot afford the cost, this change will be rolled back
