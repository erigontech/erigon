...
func jumpTable(chainRules *chain.Rules, cfg Config) *JumpTable {
	var jt *JumpTable
	switch {
	case chainRules.IsAmsterdam && chainRules.IsEIP8037:
		jt = &amsterdamInstructionSet
	case chainRules.IsAmsterdam: // Amsterdam without EIP-8037
		jt = &amsterdamWithoutEIP8037InstructionSet
	...
	for {
		...
		if operation.dynamicGas != nil {
			...
			if dynamicCost.State > 0 {
				...
				if dbg.TraceGas {
					log.Warn("[interp state] pre", "block", blockNum, "tx", txIndex, "depth", evm.depth,
						"op", op, "dynamicState", dynamicCost.State, "preConsumed", evm.StateGasConsumed())
				}
				...
				if dbg.TraceGas {
					log.Warn("[interp state] post", "block", blockNum, "tx", txIndex, "depth", evm.depth,
						"op", op, "postConsumed", evm.StateGasConsumed())
				}
			}
		}
