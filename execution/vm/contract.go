	...
	if !c.CodeHash.IsZero() {
		...
		if evicted {
			...
			if evictedN%10_000 == 0 {
				log.Warn("[dbg] JumpDestCache", "hit_m", hit/1_000_000, "total_m", (hit+miss)/1_000_000, "limit", 256, "ratio", fmt.Sprintf("%.2f", float64(hit)/float64(hit+miss)), "evictedN", evictedN)
			}
		}
	}
