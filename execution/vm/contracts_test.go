...
func testPrecompiled(t *testing.T, addr string, test precompiledTest) {
	...
	t.Run(fmt.Sprintf("%s-Gas=%d", test.Name, gas), func(t *testing.T) {
		t.Parallel()
		if res, _, err := RunPrecompiledContract(nil, p, accounts.InternAddress(common.HexToAddress(addr)), in, gas, nil); err != nil {
			t.Error(err)
...
func testPrecompiledOOG(t *testing.T, addr string, test precompiledTest) {
	...
	t.Run(fmt.Sprintf("%s-Gas=%d", test.Name, gas), func(t *testing.T) {
		t.Parallel()
		_, _, err := RunPrecompiledContract(nil, p, accounts.InternAddress(common.HexToAddress(addr)), in, gas, nil)
...
func benchmarkPrecompiled(b *testing.B, addr string, test precompiledTest) {
	...
	b.Run(fmt.Sprintf("%s-Gas=%d", test.Name, reqGas), func(bench *testing.B) {
		...
		for i := 0; i < bench.N; i++ {
			copy(data, in)
			res, _, err = RunPrecompiledContract(nil, p, accounts.InternAddress(common.HexToAddress(addr)), data, reqGas, nil)
		}
		bench.StopTimer()
...
func TestPrecompiledModExpPotentialOutOfRange(t *testing.T) {
	...
	_, _, err := RunPrecompiledContract(nil, modExpContract, accounts.InternAddress(common.BytesToAddress([]byte{0xa5})), input, maxGas, nil)
...
func TestPrecompiledModExpInputEip7823(t *testing.T) {
	...
	res, _, err := RunPrecompiledContract(nil, pragueModExp, accounts.InternAddress(common.BytesToAddress([]byte{0xa5})), in, gas, nil)
	require.NoError(t, err)
	assert.Equal(t, "", common.Bytes2Hex(res))
	gas = osakaModExp.RequiredGas(in)
	_, _, err = RunPrecompiledContract(nil, osakaModExp, accounts.InternAddress(common.BytesToAddress([]byte{0xb5})), in, gas, nil)
	require.NoError(t, err)
	assert.Equal(t, "", common.Bytes2Hex(res))
