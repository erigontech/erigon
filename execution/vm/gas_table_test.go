...
// Old implementation for comparison (with ToWordSize call + divisions)
func toWordSizeOld(size uint64) uint64 {
...
func memoryGasCostOld(lastGasCost *uint64, memLen int, newMemSize uint64) (uint64, error) {
	if newMemSize == 0 {
		return 0, nil
	}
	...
	newMemSizeWords := toWordSizeOld(newMemSize) // function call + division
	newMemSize = newMemSizeWords * 32            // multiplication

	if newMemSize > uint64(memLen) {
		square := newMemSizeWords * newMemSizeWords
		linCoef := newMemSizeWords * 3
		quadCoef := square / 512 // division
		newTotalFee := linCoef + quadCoef

		fee := newTotalFee - *lastGasCost
		*lastGasCost = newTotalFee
...
// New implementation (inlined ToWordSize + shifts)
func memoryGasCostNew(lastGasCost *uint64, memLen int, newMemSize uint64) (uint64, error) {
	if newMemSize == 0 {
		return 0, nil
	}
	if newMemSize > 0x1FFFFFFFE0 {
	...
	newMemSizeWords := (newMemSize + 31) >> 5 // shift
	newMemSize = newMemSizeWords << 5         // shift

	if newMemSize > uint64(memLen) {
		square := newMemSizeWords * newMemSizeWords
		newTotalFee := newMemSizeWords*3 + square>>9 // shift
...
func BenchmarkMemoryGasCost(b *testing.B) {
	sizes := []uint64{32, 256, 1024, 4096, 32768}

	for _, size := range sizes {
		b.Run("new/"+strconv.FormatUint(size, 10), func(b *testing.B) {
			for b.Loop() {
				var lastGasCost uint64
				_, _ = memoryGasCostNew(&lastGasCost, 0, size)
			}
		})
		b.Run("old/"+strconv.FormatUint(size, 10), func(b *testing.B) {
			for b.Loop() {
				var lastGasCost uint64
				_, _ = memoryGasCostOld(&lastGasCost, 0, size)
			}
		})
	}
}
