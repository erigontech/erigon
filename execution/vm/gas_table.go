...
func memoryCopierGas(stackpos int) gasFunc {
	return func(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
func gasSStore(evm *EVM, contractAddress common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	value, x := stack.Back(1), stack.Back(0)
	key := common.Hash(x.Bytes32())
	var current uint256.Int
	evm.IntraBlockState().GetState(contractAddress, key, &current)
	...
	evm.IntraBlockState().GetCommittedState(contractAddress, key, &original)
	if original == current {
...
func gasSStoreEIP2200(evm *EVM, contractAddress common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
func gasKeccak256(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
// expansion
func pureMemoryGascost(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
func gasCreate2(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasCreateEip3860(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasCreate2Eip3860(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasExpFrontier(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
func gasExpEIP160(_ *EVM, _ common.Address, _ uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	expByteLen := uint64(common.BitLenToByteLen(stack.data[stack.len()-2].BitLen()))
...
func gasCall(evm *EVM, _ common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		gas            uint64
	...
	callGasTemp, err = callGas(evm.ChainRules().IsTangerineWhistle, scopeGas, gas, stack.Back(0))
...
func gasCallCode(evm *EVM, _ common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	memoryGas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasDelegateCall(evm *EVM, _ common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasStaticCall(evm *EVM, _ common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	gas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
...
func gasSelfdestruct(evm *EVM, contractAddress common.Address, scopeGas uint64, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var gas uint64
	...
	if evm.ChainRules().IsTangerineWhistle {
		...
		if evm.ChainRules().IsSpuriousDragon {
			...
			balance, err := evm.IntraBlockState().GetBalance(contractAddress)
			if err != nil {
				return 0, err
			}
	...
	hasSelfdestructed, err := evm.IntraBlockState().HasSelfdestructed(contractAddress)
	if err != nil {
		return 0, err
	}
