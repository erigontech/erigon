...
// getDeferredUpdate clones so pooled objects never keep their backing arrays.
...
type PendingCommitmentUpdate struct {
	BlockNum uint64
	// BlockHash disambiguates same-block-number changesets in GetChangesetByBlockNum.
...
// EncodeBranch result must be copied before the next call; the underlying slice is reused.
...
// HasShortenedKeys treats a malformed branch as true: never under-report referenced keys.
...
// ReplacePlainKeys keeps the original key where fn returns nil; the no-op case makes zero copies.
...
// Collect must not be called with the root node's stat as other.
...
// streamingSink: plainKey and update must stay valid until the committer's Process call.
...
type Updates struct {
	...
	etl    *etl.Collector
	// Process's fold/unfold requires hashedKey-sorted traversal for adjacent trie paths;
	// plainKey order diverges the root. plainKey is only a tiebreaker.
...
// arenaAlloc's returned slice is valid only until the arena is reset.
...
// hasherReusesAddrPrefix: only for the hasher whose layout keyToHexNibbleHashCached assumes.
...
// NewEmpty must carry over the streaming sink, or a rotated buffer computes a stale root.
...
// defaultDirectMemLimit bounds in-memory ModeDirect; beyond it, batches spill to the etl collector.
...
// collectDirect: hashedKey and plainKey must be immutable, retained until HashSort consumes the batch.
// etl.Collect sorts by hashedKey, giving ModeDirect the same ordering as keyUpdateLessFn without a btree.
...
// PlainKeys is meaningful only in ModeDirect and ModeParallel; nil otherwise.
...
func (t *Updates) TouchPlainKey(key string, val []byte, fn func(c *KeyUpdate, val []byte)) {
	...
	case ModeParallel:
		// The dedup map only guards plain-key interning; every touch still reaches the trie/streamer.
...
// HashSort: fn must not retain hk or pk after returning; they are backed by reusable
// arena memory invalidated between batches. Caller must call warmuper.Wait() when done.
...
// keyUpdateLessFn orders by hashedKey; a different order diverges the trie root. plainKey is a tiebreaker.
