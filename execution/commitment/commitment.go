		...
		err := t.etl.Load(nil, "", func(k, v []byte, table etl.CurrentTableReader, next etl.LoadNextFunc) error {
			...
			if len(t.batchSlab) >= hashSortBatchSize {
				for i := range t.batchSlab {
					...
					if err := fn(t.batchSlab[i].hashedKey, toBytesZeroCopy(t.batchSlab[i].plainKey), nil); err != nil {
						return err
					}
				}
				if warmuper != nil {
		...
		t.tree.Ascend(func(item *KeyUpdate) bool {
			...
			if len(t.batchSlab) >= hashSortBatchSize {
				for i := range t.batchSlab {
					...
					if err := fn(t.batchSlab[i].hashedKey, toBytesZeroCopy(t.batchSlab[i].plainKey), t.batchSlab[i].update); err != nil {
						processErr = err
						return false
					}
				}
