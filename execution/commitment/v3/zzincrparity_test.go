...
func newIncrWorld(t *testing.T) *incrWorld {
	...
	w.v3 = &Trie{}
	w.v3.ResetContext(w.ctxV3)
	w.v3.SetTrieContextFactory(w.ctxV3.factory)
...
func (w *incrWorld) block(n int, entries []parityUpdate) bool {
	...
	rootV3, err := w.v3.Process(ctx, makeParityUpdates(w.t, commitment.ModeCollect, entries), "", nil, commitment.WarmupConfig{})
	require.NoErrorf(w.t, err, "block %d v3", n)
	require.Zero(w.t, w.ctxV3.accountCalls)
	require.Zero(w.t, w.ctxV3.storageCalls)
	w.checkRecords(n)
	...
	if bytes.Equal(rootHPH, rootV3) {
		return true
	}
	truth := w.rebuildFromScratch()
	if !bytes.Equal(truth, rootV3) {
		w.t.Fatalf("block %d: v3 root differs from a trie rebuilt from the same state\n v3    %x\n hph   %x\n fresh %x\nhistory:\n%s",
			n, rootV3, rootHPH, truth, strings.Join(w.history, ""))
	}
	w.hphDrift = n
	return false
}
...
func (w *incrWorld) checkRecords(n int) {
	w.t.Helper()
	var noAddr [32]byte
	leaves, issues := walkRecords(w.ctxV3, planeAccount, noAddr)
	...
	for addr, wantSlots := range slots {
		got, storageIssues := walkRecords(w.ctxV3, planeStorage, hashAddressPath([]byte(addr)))
...
func (w *incrWorld) rebuildFromScratch() []byte {
	...
	rootV3, err := fresh.v3.Process(context.Background(), makeParityUpdates(w.t, commitment.ModeCollect, entries), "", nil, commitment.WarmupConfig{})
	require.NoError(w.t, err)
	if !bytes.Equal(root, rootV3) {
		w.t.Fatalf("fresh rebuild disagrees: v3 %x hph %x", rootV3, root)
	}
	return root
}
