...
// account it injects storageRoot's result instead of streaming the slots. The
// account whose storage this is need not itself be in the round; storageSubtreeAccountKey
// recovers its key from any storage slot's plain key.
func dfsSubtreeDeep(w *HexPatriciaHashed, node *prefixNode, path []byte, threshold int, storageRoot func(node *prefixNode, path []byte, accountFresh, storageDestroyed bool) (cell, error)) error {
	if node == nil {
		return nil
	}
	...
	if deepStorage {
		storageDestroyed := accountUpdate != nil && accountUpdate.DeleteStorageSubtree
