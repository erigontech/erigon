...
// branchCacheEntry ~80B + maphash slot/hash ~40B + prefix/value slice headers ~48B = 168.
...
type ContractTrunkPreload struct {
	...
	usedBytes       int
	maxDepthReached int
	pinTxNum        uint64 // txN=0 escapes floor; evict via head txNum
}
...
func (p *ContractTrunkPreload) Run(
	...
	for len(p.queue) > 0 {
		...
		// HexToCompact may alias; copy for stable handle
