...
// hashedKeyPath is the nibble path to the surviving sibling; hashedExtLen is the extension length
// of that sibling (0 for branch nodes, >0 for leaf/extension nodes).
type CollapseTracer func(hashedKeyPath []byte, hashedExtLen int16)
...
func (hph *HexPatriciaHashed) detectCollapseBeforeDelete(hashedKey []byte) {
	...
	hph.collapseTracer(siblingPath, siblingCell.hashedExtLen)
}
...
func (hph *HexPatriciaHashed) detectCascadingCollapseAtRow(row int) {
	...
	hph.collapseTracer(siblingPath, survivingCell.hashedExtLen)
}
