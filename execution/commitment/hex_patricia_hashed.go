...
type cell struct {
	...
	hashDeriveDepth int16     // >=0: deriveHashedKeys pending at this depth; -1: already derived or not needed
...
// ensureDerivedHashedKeys performs lazy derivation of hashed keys if pending.
...
func (hph *HexPatriciaHashed) readBranchAndCheckForFlushing(prefix []byte) ([]byte, error) {
	...
	if be.DeferUpdatesEnabled() && be.HasPendingPrefix(prefix) {
		if err := be.ApplyDeferredUpdates(estimate.HalfCPUs(), hph.ctx.PutBranch); err != nil {
			return nil, err
		}
		be.ClearDeferred()
	}
...
func (hph *HexPatriciaHashed) unfold(hashedKey []byte, unfolding int16) error {
	...
	if hph.activeRows == 0 {
		...
		// Lazy derivation: ensure hashed keys are derived before reading hashedExtension
		if err := hph.ensureDerivedHashedKeys(upCell); err != nil {
			return err
		}
	...
	if upCell.hashedExtLen == 0 {
		depth = upDepth + 1
		target := -1
		if int(depth-1) < len(hashedKey) {
			target = int(hashedKey[depth-1])
		}
		return hph.unfoldBranchNode(row, depth, touched && !present, target)
	}
...
func (hph *HexPatriciaHashed) Process(ctx context.Context, updates *Updates, logPrefix string, progress chan *CommitProgress, warmup WarmupConfig) (rootHash []byte, err error) {
	...
	if hph.branchEncoder.DeferUpdatesEnabled() && !hph.leaveDeferredForCaller {
		if err = hph.branchEncoder.ApplyDeferredUpdates(estimate.HalfCPUs(), hph.ctx.PutBranch); err != nil {
