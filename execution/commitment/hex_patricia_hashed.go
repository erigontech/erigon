...
func (hph *HexPatriciaHashed) toWitnessTrie(hashedKey []byte, codeReads map[common.Hash]witnesstypes.CodeWithHash) (*trie.Trie, error) {
	...
	if hph.root.hashedExtLen > 0 {
		...
		if hph.traceW != nil {
			fmt.Fprintf(hph.traceW, "[witness] root node %s, pos %d\n", hph.root.FullString(), keyPos)
		}
	}
	...
	for row := 0; row < hph.activeRows && keyPos < int16(len(hashedKey)); row++ {
		...
		if cellToExpand.hashedExtLen > 0 { // extension cell
			...
			if hph.depths[row] < 64 && extKeyLength+hph.depths[row] > 64 { //&& cellToExpand.accountAddrLen > 0 {
				...
				depthAdjusted = true
				if hph.traceW != nil {
					fmt.Fprintf(hph.traceW, "[witness] adjusted hashExtLen=%d <- %d\n", extKeyLength, cellToExpand.hashedExtLen)
				}
			}
		...
		if fullNode, ok := currentNode.(*trie.FullNode); ok { // handle full node case
			for col := 0; col < 16; col++ {
				...
				if hph.traceW != nil {
					fmt.Fprintf(hph.traceW, "[witness, pos %d] FullNodeChild Hash (%d, %0x, depth=%d) %s proof %+v\n", keyPos, row, col, hph.depths[row], currentCell.FullString(), fullNode.Children[col])
				}
			}
			...
			if len(hashedKey) <= 64 { // no storage, stop here
				...
				if hph.traceW != nil {
					fmt.Fprintf(hph.traceW, "[witness] AccountNode (break) (%d, %0x, depth=%d) %s proof %+v\n", row, currentNibble, hph.depths[row], cellToExpand.FullString(), accNode)
				}
				break
			}
		...
		if nextShortNode, ok := nextNode.(*trie.ShortNode); ok && len(hashedKey) > 64 && keyPos+1 == 64 {
			...
			if nextAccNode, ok := nextShortNode.Val.(*trie.AccountNode); ok {
				...
				if hph.traceW != nil {
					fmt.Fprintf(hph.traceW, "[witness] AccountNode (+StorageTrie) (%d, %0x, depth=%d) %s [proof %+v\n", row, currentNibble, hph.depths[row], cellToExpand.FullString(), nextAccNode)
				}
			}
		}
	...
	err = updates.HashSort(ctx, nil, func(hashedKey, plainKey []byte, stateUpdate *Update) error {
		...
		if int16(len(plainKey)) == hph.accountKeyLen { // account
			...
			if hph.traceW != nil {
				...
				fmt.Fprintf(hph.traceW, "account with plainKey=%x, addrHash=%x FOUND = %v\n", plainKey, addrHash, update)
			}
		} else {
			...
			if hph.traceW != nil {
				fmt.Fprintf(hph.traceW, "storage found = %v\n", update.Storage[:update.StorageLen])
			}
		}
