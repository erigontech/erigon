...
func (s *state) Encode(buf []byte) ([]byte, error) {
	...
	buf = slices.Grow(buf, 3+len(s.Root)+2)
	buf = append(buf, byte(rootFlags))
	buf = binary.BigEndian.AppendUint16(buf, uint16(len(s.Root)))
	buf = append(buf, s.Root...)
	return binary.BigEndian.AppendUint16(buf, s.RootMask), nil
}

const (
	legacyStateDepthsLen  = 128
	legacyStateMapLen     = 2 * 128
	legacyStateBranchLen  = 16
	legacyStateAfterMapAt = legacyStateDepthsLen + legacyStateMapLen
	legacyStateTailLen    = legacyStateAfterMapAt + legacyStateMapLen + legacyStateBranchLen
)
...
func (s *state) Decode(buf []byte) error {
	...
	buf = buf[3:]
	if len(buf) < rootSize {
		return fmt.Errorf("state root: %w", io.ErrUnexpectedEOF)
	}
	...
	if version == stateRootVersionV2 {
		if len(buf) < 2 {
			return fmt.Errorf("state root mask: %w", io.ErrUnexpectedEOF)
		}
		s.RootMask = binary.BigEndian.Uint16(buf)
		return nil
	}

	if len(buf) < legacyStateTailLen {
		return fmt.Errorf("legacy state body: %w", io.ErrUnexpectedEOF)
	}
	s.RootMask = binary.BigEndian.Uint16(buf[legacyStateAfterMapAt:])
	return nil
}
