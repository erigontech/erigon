...
func makeV3CacheKey(tag byte, addrHash []byte, path []byte) []byte {
...
func v3CachePath(depth int) []byte {
...
func TestBranchCacheV3RoutingTiers(t *testing.T) {
		...
		t.Run(tc.name, func(t *testing.T) {
			for depth := range 7 {
				t.Run(string(rune('0'+depth)), func(t *testing.T) {
					...
					key := makeV3CacheKey(tc.tag, tc.addr, v3CachePath(depth))
...
func TestBranchCacheV3RoutingCollision(t *testing.T) {
...
func TestBranchCacheV3DispatchPrecondition(t *testing.T) {
	t.Run("root", func(t *testing.T) {
		c := NewBranchCache(100)
		defer c.Close()

		v1Key := nibbles.HexToCompact(nil)
		v3Key := makeV3CacheKey(0x40, nil, nil)
		c.Put(v1Key, []byte("v1-root"), 0, 0)
		c.Put(v3Key, []byte("v3-root"), 0, 0)

		got, _, ok := c.Get(v1Key)
		require.True(t, ok)
		require.Equal(t, []byte("v1-root"), got)
		got, _, ok = c.Get(v3Key)
		require.True(t, ok)
		require.Equal(t, []byte("v3-root"), got)
	})
	...
	t.Run("account-trunk", func(t *testing.T) {
		...
		v3Key := makeV3CacheKey(0x40, nil, path)
		c.Put(v1Key, []byte("v1-account"), 0, 0)
		c.Put(v3Key, []byte("v3-account"), 0, 0)
		...
		got, _, ok = c.Get(v3Key)
		require.True(t, ok)
		require.Equal(t, []byte("v3-account"), got)
	})
	...
	t.Run("storage-trunk", func(t *testing.T) {
		...
		v3Key := makeV3CacheKey(0x41, addrHash, path)
		c.PinEntry(v1Key, []byte("v1-storage"), 0, 0)
		c.PinEntry(v3Key, []byte("v3-storage"), 0, 0)
		...
		got, _, ok = c.Get(v3Key)
		require.True(t, ok)
		require.Equal(t, []byte("v3-storage"), got)
	})
