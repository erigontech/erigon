...
Hardware: Apple M2 Max, 12 cores, Go 1.25.7, darwin/arm64. Source: `go test -run=^$ -bench=Benchmark_Commitment_DirectVsParallel -benchtime=3x ./execution/commitment/`. Bench reuses one long-lived `*ParallelPatriciaHashed` across iterations to mirror production usage (worker pool stays warm). `MinSplitKeys = 64` (default). Numbers are ratios vs `ModeDirect` on the same corpus; raw ns/op ages badly across hardware.

- **500K-StorageHeavy** (1000 accounts × 499 storage slots each — the bloatnet shape). `ModeParallel-w1` is ~1.06× faster than `ModeDirect` (parallel overhead is negligible here because the storage subtries dwarf coordination cost). `ModeParallel-w8` is ~1.95× faster than `ModeDirect` — the realistic peak speedup. The curve is essentially flat from w4 to w12 (w4 ≈ 1.92×, w12 ≈ 1.86×) — storage-heavy tops out at ~4 effective workers on this corpus; beyond that, fold-time barrier contention and goroutine scheduling eat further gains.
- **100K-AccountsOnly** (uniform random account balances). `ModeParallel-w1` is ~1.35× *slower* than `ModeDirect` — the pure-overhead cost of the parallel codepath at zero concurrency on a workload where subtrees are shallow and uniform. `ModeParallel-w4` is ~1.66× faster, `w8` ~1.63×, `w12` ~1.47× — flat workloads do benefit, just less than storage-heavy. Past w8 the per-worker overhead starts dominating.
- **Open optimization**: CPU profile (`-cpuprofile`) on `500K/ModeParallel-w8` shows ~35% time in `runtime.usleep` and ~32% in `etl.Collector.Load` / `dispatchLeafKeys`. Scheduler thrashing from goroutine parking — not GC-driven (allocation reduction of 60% from pool reuse only moved time by ~4%). Likely candidates: per-key channel sends in the bucket dispatcher (500K channel ops on the storage-heavy corpus), and fold-barrier coordination via `foldSem`. Batching the dispatcher (N-key chunks instead of per-key sends) is the obvious next step.
