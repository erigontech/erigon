...
var errVerifyRecords = errors.New("commitment v4: record verification failed")
...
func (h *RecordHasher) Hash(key, value []byte, expect func(key, hash []byte) error) ([32]byte, error) {
	...
	record := NewRecord(value, depth)
	n := &h.n
	resetNode(n, h.path, tag)
	h.scratch = h.scratch[:0]
	if record.isLeafRoot() {
		hashedKey, leafValue := record.LeafRootBody()
		full := (*[32]byte)(hashedKey)
		appendLeaf(n, int(full[0]>>4), appendHashSuffix(&h.scratch, full, 1), leafValue)
	} else {
		l := record.layout()
		if l.emb != 0 {
			return [32]byte{}, fmt.Errorf("%w: %x: %w", errVerifyRecords, key, ErrUnfoldEmbedded)
		}
	...
	for nib := range 16 {
		...
		_, _, _, storageRoot, err := decodeAccountLeaf(n.leafValueAt(nib))
		if err != nil {
		...
		unpackPath(n.leafSuffixAt(nib), 63-len(n.path), full[len(n.path)+1:len(n.path)+1:64])
...
func (h *RecordHasher) parseKey(key []byte) (tag byte, addrHash []byte, err error) {
	...
	if count > 63 || len(key)-start-1 != packedLen(count) {
		return 0, nil, fmt.Errorf("%w: key %x: %w", errVerifyRecords, key, ErrKeyPathLength)
	}
