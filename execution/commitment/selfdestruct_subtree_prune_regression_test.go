...
// prevCapturingCtx wraps a PatriciaContext and records the prev (pre-image) passed to
// each PutBranch, so a test can assert the changeset pre-image a delete records.
type prevCapturingCtx struct {
	PatriciaContext
	prevs map[string][]byte
}

func (c *prevCapturingCtx) PutBranch(prefix, data, prev []byte) error {
	c.prevs[string(prefix)] = bytes.Clone(prev)
	return c.PatriciaContext.PutBranch(prefix, data, prev)
}

// TestDeleteStorageSubtreeBranches_TombstoneRecordsPrev pins item 4: the subtree-prune
// tombstone must pass the branch in hand as prev, so the merge runs and the changeset
// records the real pre-image. With a nil prev the changeset pre-image is empty and an
// unwind of the self-destruct cannot restore the pruned branch.
func TestDeleteStorageSubtreeBranches_TombstoneRecordsPrev(t *testing.T) {
	t.Parallel()
	ms := NewMockState(t)
	ctx := &prevCapturingCtx{PatriciaContext: ms, prevs: map[string][]byte{}}

	row, bm := generateCellRow(t, 4)
	cells := generateCellEncodeDataRow(t, row, bm)
	be := NewBranchEncoder(1024)
	encoded, err := be.EncodeBranch(bm, bm, bm, &cells)
	require.NoError(t, err)
	branch := bytes.Clone([]byte(encoded))

	prefix := []byte{0x1, 0x2}
	compact := nibbles.HexToCompact(prefix)
	require.NoError(t, ms.PutBranch(compact, branch, nil))

	hph := NewHexPatriciaHashed(length.Addr, ctx, DefaultTrieConfig())
	hph.branchEncoder.setDeferUpdates(false) // apply PutBranch synchronously so the prev is observable
	require.NoError(t, hph.deleteStorageSubtreeBranches(prefix))

	got, ok := ctx.prevs[string(compact)]
	require.True(t, ok, "tombstone must PutBranch at the deleted prefix")
	require.Equal(t, branch, got,
		"tombstone must record the real branch as prev (for unwind), not an empty pre-image")
}
