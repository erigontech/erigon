...
func (e *ExecModule) ValidateChain(ctx context.Context, blockHash common.Hash, blockNumber uint64) (ValidationResult, error) {
	...
	if flashUpdate.IsUpdate && flashUpdate.SD != nil {
		...
		// Stamp with this block: newBlockExec only adopts carried tracking whose
		// block number matches, so an unstamped transfer would be silently ignored.
		doms.SetParallelExecBlock(header.Number.Uint64())
	}
