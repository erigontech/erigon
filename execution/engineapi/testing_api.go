...
func (t *testingImpl) decodeTxnProvider(ctx context.Context, transactions *[]hexutil.Bytes, blockNumber, timestamp uint64) (txnprovider.TxnProvider, error) {
	...
	if t.db != nil {
		...
		defer dbTx.Rollback()
		sd, err := execctx.NewSharedDomainsWithTrieConfig(ctx, dbTx, t.logger, commitment.TrieConfig{Variant: commitment.VariantHexPatriciaTrie, DeferBranchUpdates: false})
		if err != nil {
