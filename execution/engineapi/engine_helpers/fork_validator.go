...
type validatePayloadFunc func(*sd.SharedDomains, kv.TemporalRwTx, uint64, []*types.Header, []*types.RawBody, *shards.Notifications) error

type ForkValidator struct {
	// current memory batch containing chain head that extend canonical fork.
	sharedDom *sd.SharedDomains
...
func (fv *ForkValidator) ValidatePayload(tx kv.TemporalRwTx, header *types.Header, body *types.RawBody, logger log.Logger) (status engine_types.EngineStatus, latestValidHash common.Hash, validationError error, criticalError error) {
	...
	fv.sharedDom, criticalError = sd.NewSharedDomains(tx, logger)
	if criticalError != nil {
...
func (fv *ForkValidator) validateAndStorePayload(sd *sd.SharedDomains, tx kv.TemporalRwTx, header *types.Header, body *types.RawBody, unwindPoint uint64, headersChain []*types.Header, bodiesChain []*types.RawBody,
