...
func (fv *ForkValidator) ValidatePayload(ctx context.Context, tx kv.TemporalRwTx, header *types.Header, body *types.RawBody, logger log.Logger) (status engine_types.EngineStatus, latestValidHash common.Hash, validationError error, criticalError error) {
	fv.lock.Lock()
	defer fv.lock.Unlock()
	...
	fv.sharedDom, criticalError = execctx.NewSharedDomains(ctx, tx, logger)
	if criticalError != nil {
