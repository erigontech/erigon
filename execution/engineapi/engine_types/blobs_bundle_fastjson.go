...
func (r *GetPayloadResponse) MarshalFastJSONTo(s *jsonstream.StackStream) error {
	...
	jsonstream.HexPtr(s, "blockValue", r.BlockValue)
	s.Field("blobsBundle")
...
func (p *ExecutionPayload) writeTo(s *jsonstream.StackStream) {
	...
	jsonstream.HexPtr(s, "baseFeePerGas", p.BaseFeePerGas)
	...
	jsonstream.HexPtr(s, "blobGasUsed", p.BlobGasUsed)
	jsonstream.HexPtr(s, "excessBlobGas", p.ExcessBlobGas)
	jsonstream.HexPtrOmitempty(s, "slotNumber", p.SlotNumber)
	jsonstream.HexPtrOmitempty(s, "blockAccessList", p.BlockAccessList)
	s.WriteObjectEnd()
}
