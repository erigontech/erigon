...
func TestV2HashBuilding(t *testing.T) {
	...
	for i, key := range keys {
		...
		if curr.Len() > 0 {
			var err error
			groups, err = GenStructStep(0, func(_ []byte) bool { return true }, false, false, curr.Bytes(), succ.Bytes(), hb, nil, 0, nil, nil, rlpValueTape, groups)
			if err != nil {
	...
	curr.Write(succ.Bytes())
	succ.Reset()
	if _, err := GenStructStep(0, func(_ []byte) bool { return true }, false, false, curr.Bytes(), succ.Bytes(), hb, nil, 0, nil, nil, rlpValueTape, groups); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
...
func TestV2Resolution(t *testing.T) {
	...
	for _, key := range keys {
		...
		if curr.Len() > 0 {
			var err error
			groups, err = GenStructStep(0, rs.HashOnly, false, false, curr.Bytes(), succ.Bytes(), hb, nil, 0, nil, nil, rlpValueTape, groups)
			if err != nil {
				t.Errorf("Could not execute step of structGen algorithm: %v", err)
			}
		}
	...
	curr.Write(succ.Bytes())
	succ.Reset()
	if _, err := GenStructStep(0, rs.HashOnly, false, false, curr.Bytes(), succ.Bytes(), hb, nil, 0, nil, nil, rlpValueTape, groups); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
	tr1 := New(common.Hash{})
	tr1.root = hb.root()
