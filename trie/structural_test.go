...
func TestEmbeddedStorage(t *testing.T) {
	...
	for _, key := range keys {
		...
		succ.WriteByte(16)
		if curr.Len() > 0 {
			groups, err = GenStructStep(func(_ []byte) bool { return false }, curr.Bytes(), succ.Bytes(), hb, &GenStructStepLeafData{rlphacks.RlpSerializableBytes(valueShort)}, groups, false)
			if err != nil {
				t.Errorf("Could not execute step of structGen algorithm: %v", err)
			}
		}
