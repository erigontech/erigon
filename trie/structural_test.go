...
func TestV2HashBuilding(t *testing.T) {
	...
	for i, key := range keys {
		...
		if curr.Len() > 0 {
			var err error
			groups, err = GenStructStep(func(_ []byte) bool { return true }, curr.Bytes(), succ.Bytes(), hb, GenStructStepLeafData{rlpValueTape}, groups)
			if err != nil {
	...
	curr.Write(succ.Bytes())
	succ.Reset()
	if _, err := GenStructStep(func(_ []byte) bool { return true }, curr.Bytes(), succ.Bytes(), hb, GenStructStepLeafData{rlpValueTape}, groups); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
...
func TestV2Resolution(t *testing.T) {
	...
	for _, key := range keys {
		...
		if curr.Len() > 0 {
			var err error
			groups, err = GenStructStep(rs.HashOnly, curr.Bytes(), succ.Bytes(), hb, GenStructStepLeafData{rlpValueTape}, groups)
			if err != nil {
				t.Errorf("Could not execute step of structGen algorithm: %v", err)
			}
		}
	...
	curr.Write(succ.Bytes())
	succ.Reset()
	if _, err := GenStructStep(rs.HashOnly, curr.Bytes(), succ.Bytes(), hb, GenStructStepLeafData{rlpValueTape}, groups); err != nil {
		t.Errorf("Could not execute step of structGen algorithm: %v", err)
	}
	tr1 := New(common.Hash{})
	tr1.root = hb.root()
