...
func HandleError(err error, stream jsonstream.Stream) {
	if err != nil {
		...
		if ok {
			...
			data, derr := fastjson.Marshal(de.ErrorData())
			if derr == nil {
...
func (h *handler) handleMsg(msg *jsonrpcMessage, stream jsonstream.Stream) {
	...
	h.startCallProc(func(cp *callProc) {
		...
		h.addSubscriptions(cp.notifiers)
		if answer != nil {
			buffer, _ := fastjson.Marshal(answer)
			stream.Write(buffer)
		}
		if needWriteStream {
...
func (h *handler) handleSubscriptionResult(msg *jsonrpcMessage) {
	...
	if err := fastjson.Unmarshal(msg.Params, &result); err != nil {
...
func (h *handler) handleResponse(msg *jsonrpcMessage) {
	...
	if op.err = fastjson.Unmarshal(msg.Result, &op.sub.subid); op.err == nil {
		go op.sub.start()
