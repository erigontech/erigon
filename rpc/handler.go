...
// Nothing here calls Write: it reaches the underlying writer on every call, committing the HTTP
// status before ServeHTTP can set it. The other writers hand the buffer over only past
// FlushThreshold, so a response that fits still leaves the status to ServeHTTP.
