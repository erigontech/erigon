...
type ErigonAPI interface {
	...
	BlockNumber(ctx context.Context, rpcBlockNumPtr *types.BlockNumber) (hexutil.Uint64, error)

	// Blocks related (see ./erigon_blocks.go)
	GetHeaderByNumber(ctx context.Context, number types.BlockNumber) (*types.Header, error)
	...
	GetBalanceChangesInBlock(ctx context.Context, blockNrOrHash types.BlockNumberOrHash) (map[common.Address]*hexutil.Big, error)

	// Receipt related (see ./erigon_receipts.go)
	GetLogsByHash(ctx context.Context, hash common.Hash) ([][]*types.Log, error)
	//GetLogsByNumber(ctx context.Context, number types.BlockNumber) ([][]*types.Log, error)
