...
func (api *APIImpl) Call(ctx context.Context, args ethapi2.CallArgs, requestedBlock *rpc.BlockNumberOrHash, overrides *ethapi2.StateOverrides) (hexutil.Bytes, error) {
	...
	result, _, err := transactions.DoCall(ctx, engine, args, tx, blockNrOrHash, header, overrides, nil, blockHashOverrides, api.GasCap, chainConfig, stateReader, api._blockReader, api.evmCallTimeout)
	if err != nil {
		return nil, err
	}
...
func (api *APIImpl) EstimateGas(ctx context.Context, argsOrNil *ethapi2.CallArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *ethapi2.StateOverrides) (hexutil.Uint64, error) {
	...
	caller, err := transactions.NewReusableCaller(ctx, engine, stateReader, overrides, header, args, api.GasCap, dbtx, api._blockReader, chainConfig, api.evmCallTimeout)
	if err != nil {
		return 0, err
	}

	// First try with the highest gas possible
...
func (api *APIImpl) CreateAccessList(ctx context.Context, args ethapi2.CallArgs, blockNrOrHash *rpc.BlockNumberOrHash, optimizeGas *bool) (*accessListResult, error) {
	...
	header, latest, err := headerByNumberOrHash(ctx, tx, bNrOrHash, api)
	if err != nil {
		return nil, err
	}
