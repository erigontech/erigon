...
func TestTxPoolContent(t *testing.T) {
	...
	txn, err := types.SignTx(types.NewTransaction(0, common.Address{1}, uint256.NewInt(expectValue), params.TxGas, uint256.NewInt(10*common.GWei), nil), *types.LatestSignerForChainID(uint256.MustFromBig(m.ChainConfig.ChainID)), m.Key)
	require.NoError(err)
