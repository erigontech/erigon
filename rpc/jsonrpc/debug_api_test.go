...
func TestAccountRange(t *testing.T) {
	...
	t.Run("valid account", func(t *testing.T) {
		addr := common.HexToAddress("0x537e697c7ab75a26f9ecf0ce810e3154dfcaaf55")
		n := rpc.BlockNumber(1)
		result, err := api.AccountRange(m.Ctx, rpc.BlockNumberOrHash{BlockNumber: &n}, addr.AsSlice(), 10, true, true, nil)
		require.NoError(t, err)
	...
	t.Run("with storage", func(t *testing.T) {
		...
		n := rpc.BlockNumber(1)
		result, err := api.AccountRange(m.Ctx, rpc.BlockNumberOrHash{BlockNumber: &n}, addr.AsSlice(), 1, false, false, nil)
		require.NoError(t, err)
...
func TestMapTxNum2BlockNum(t *testing.T) {
	...
	t.Run("descend", func(t *testing.T) {
		...
		txNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 1024, -1, order.Desc, kv.Unlim)
		require.NoError(t, err)
		txNumsIter := rawdbv3.TxNums2BlockNums(tx, rawdbv3.TxNums, txNums, order.Desc)
		expectTxNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 1024, -1, order.Desc, kv.Unlim)
		require.NoError(t, err)
	...
	t.Run("ascend", func(t *testing.T) {
		...
		txNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 0, 1024, order.Asc, kv.Unlim)
		require.NoError(t, err)
		txNumsIter := rawdbv3.TxNums2BlockNums(tx, rawdbv3.TxNums, txNums, order.Desc)
		expectTxNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 0, 1024, order.Asc, kv.Unlim)
		require.NoError(t, err)
		checkIter(t, expectTxNums, txNumsIter)
	})
	t.Run("ascend limit", func(t *testing.T) {
		...
		txNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 0, 1024, order.Asc, 2)
		require.NoError(t, err)
		txNumsIter := rawdbv3.TxNums2BlockNums(tx, rawdbv3.TxNums, txNums, order.Desc)
		expectTxNums, err := tx.IndexRange(kv.LogAddrIdx, addr.AsSlice(), 0, 1024, order.Asc, 2)
		require.NoError(t, err)
		checkIter(t, expectTxNums, txNumsIter)
	})
}
