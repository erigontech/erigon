...
// A pooled transaction is priced against the block that would include it, as geth prices it, and
// carries no location. Without a known head there is nothing to price against, so the fee cap
// stands.
func TestNewRPCPendingTransactionGasPrice(t *testing.T) {
	tip, feeCap := uint256.NewInt(2), uint256.NewInt(1_000_000_000)
	txn := types.NewEIP1559Transaction(*uint256.NewInt(1), 1, common.HexToAddress("deadbeef"), uint256.NewInt(1), 21000, nil, tip, feeCap, nil)

	priced := newRPCPendingTransaction(txn, uint256.NewInt(1000))
	require.NotNil(t, priced.GasPrice)
	require.Equal(t, big.NewInt(1002), priced.GasPrice.ToInt(), "min(tip + baseFee, feeCap)")
	require.Nil(t, priced.BlockHash)

	unpriced := newRPCPendingTransaction(txn, nil)
	require.Equal(t, feeCap.ToBig(), unpriced.GasPrice.ToInt())
}
