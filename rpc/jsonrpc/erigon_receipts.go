...
func (api *ErigonImpl) GetLogsByHash(ctx context.Context, hash common.Hash) ([]types.Logs, error) {
	...
	blockNumber, _, _, err := rpchelper.GetBlockNumber(ctx, rpc.BlockNumberOrHashWithHash(hash, true), tx, api._blockReader, nil)
	if err != nil {
...
func (api *ErigonImpl) GetLatestLogs(ctx context.Context, crit filters.FilterCriteria, logOptions filters.LogFilterOptions) (types.RPCLogs, error) {
	...
	for it.HasNext() {
		...
		for i := range blockLogs {
			blockLogs[i].Index = hexutil.Uint(logIndex)
			logIndex++
		}
