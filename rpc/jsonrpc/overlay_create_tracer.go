...
func (ct *OverlayCreateTracer) OnEnter(depth int, typ byte, from accounts.Address, to accounts.Address, precompile bool, input []byte, gas uint64, value uint256.Int, code []byte) {
	...
	if (vm.OpCode(typ) == vm.CREATE || vm.OpCode(typ) == vm.CREATE2) && to == ct.contractAddress {
		ct.isCapturing = true
		_, _, _, err := ct.evm.OverlayCreate(from.Value(), vm.NewCodeAndHash(ct.code), ct.gasCap, value, to.Value(), vm.OpCode(typ), true /* incrementNonce */)
		if err != nil {
			ct.err = err
		} else {
			if result, err := ct.evm.IntraBlockState().GetCode(ct.contractAddress.Value()); err != nil {
