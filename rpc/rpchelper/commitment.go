...
func (r *CommitmentReplay) ComputeCustomCommitmentFromStateHistory(
	...
	tsd, err := execctx.NewSharedDomains(ctx, ttx, r.logger, execctx.WithoutDeferredBranchUpdates(), execctx.WithoutSharedBranchCache(), execctx.WithSequentialCommitment())
	if err != nil {
		return nil, err
	}
	defer tsd.Close()
