...
func TestCacheBtreeOrderAccountStorage2(t *testing.T) {
	...
	var a1 = common.NewAddress(1)
	sc.SetAccountRead(a1.AsSlice(), &accounts.Account{})
	sc.SetAccountWrite(a1.AsSlice(), &accounts.Account{Incarnation: 2})
	x, ok := sc.GetAccount(a1.AsSlice())
	fmt.Printf("%+v,%t\n", x.Incarnation, ok)
...
func TestCacheBtreeOrderAccountStorage(t *testing.T) {
	t.Parallel()
	sc := NewStateCache(32, datasize.ByteSize(128*accountItemSize))
	var a1, a2 = common.NewAddress(1), common.NewAddress(2)
	sc.SetAccountWrite(a2.AsSlice(), &accounts.Account{})
	sc.SetAccountWrite(a1.AsSlice(), &accounts.Account{})
	...
	l2[0] = 2
	l3[0] = 3
	sc.SetStorageWrite(a1.AsArray(), 1, l1.Bytes(), nil)
	sc.SetStorageWrite(a1.AsArray(), 1, l2.Bytes(), nil)
	sc.SetStorageWrite(a2.AsArray(), 1, l3.Bytes(), nil)
	lastK = lastK[:0]
	if err := sc.WalkStorage(common.BytesToHash(sha3.NewLegacyKeccak256().Sum(a1.AsSlice())), 1, nil, func(locHash common.Hash, val []byte) error {
	...
	sc.SetCodeWrite(a1.AsSlice(), 1, []byte{1})
	sc.SetCodeWrite(a2.AsSlice(), 1, []byte{2})
	lastK = lastK[:0]
...
func TestAccountReads(t *testing.T) {
	...
	var addr1 = common.NewAddress(1)
	sc.SetAccountRead(addr1.AsSlice(), &account1)
	if _, ok := sc.GetAccount(addr1.AsSlice()); !ok {
		t.Fatalf("Expected to find account with addr1")
	}
	var addr2 = common.NewAddress(2)
	if _, ok := sc.GetAccount(addr2.AsSlice()); ok {
		t.Fatalf("Did not expect account with addr2")
	}
	var addr3 = common.NewAddress(3)
	sc.SetAccountAbsent(addr3.AsSlice())
	if a, ok := sc.GetAccount(addr3.AsSlice()); !ok || a != nil {
		t.Fatalf("Expected account with addr3 to be absent")
	}
	for i := 4; i <= 6; i++ {
		var account accounts.Account
		account.Balance.SetUint64(uint64(i))
		var addr = common.NewAddress(byte(i))
		sc.SetAccountRead(addr.AsSlice(), &account)
	}
	...
	for i := 4; i <= 6; i++ {
		...
		account.Balance.SetUint64(uint64(i))
		var addr = common.NewAddress(byte(i))
		if _, ok := sc.GetAccount(addr.AsSlice()); !ok {
...
func TestAccountReadWrites(t *testing.T) {
	...
	account1.Balance.SetUint64(1)
	var addr1 = common.NewAddress(1)
	sc.SetAccountWrite(addr1.AsSlice(), &account1)
	if _, ok := sc.GetAccount(addr1.AsSlice()); !ok {
		t.Fatalf("Expected to find account with addr1")
	}
	if sc.WriteCount() != 1 {
	...
	sc.SetAccountWrite(addr1.AsSlice(), &account11)
	if a, ok := sc.GetAccount(addr1.AsSlice()); !ok {
		t.Fatalf("Expected to find account with addr1")
	} else {
	...
	var addr2 = common.NewAddress(2)
	sc.SetAccountRead(addr2.AsSlice(), &account2)
	...
	sc.SetAccountWrite(addr2.AsSlice(), &account22)
	if a, ok := sc.GetAccount(addr2.AsSlice()); !ok {
	...
	var addr3 = common.NewAddress(3)
	sc.SetAccountWrite(addr3.AsSlice(), &account3)
	sc.SetAccountDelete(addr3.AsSlice())
	if a, ok := sc.GetAccount(addr3.AsSlice()); !ok || a != nil {
	...
	var addr4 = common.NewAddress(4)
	sc.SetAccountRead(addr4.AsSlice(), &account4)
	sc.SetAccountDelete(addr4.AsSlice())
	if a, ok := sc.GetAccount(addr4.AsSlice()); !ok || a != nil {
	...
	var addr5 = common.NewAddress(5)
	sc.SetAccountDelete(addr5.AsSlice())
	if a, ok := sc.GetAccount(addr5.AsSlice()); !ok || a != nil {
...
func TestReplaceAccountReadsWithWrites(t *testing.T) {
	...
	for i := 1; i <= 4; i++ {
		...
		var account accounts.Account
		account.Balance.SetUint64(uint64(i))
		sc.SetAccountWrite(addr.AsSlice(), &account)
	}
...
func TestGetDeletedAccount(t *testing.T) {
	...
	sc.SetAccountWrite(addr1.AsSlice(), &account11)
	acc := sc.GetDeletedAccount(addr1.AsSlice())
	if acc != nil {
		t.Fatalf("Did not expect to find deleted account before deletion")
	}
	sc.SetAccountDelete(addr1.AsSlice())
	acc = sc.GetDeletedAccount(addr1.AsSlice())
	if acc == nil {
...
func TestReadWriteAbsentDeleteStorage(t *testing.T) {
	...
	for i := 1; i <= 4; i++ {
		...
		var loc common.Hash
		loc[1] = byte(i)
		sc.SetStorageAbsent(addr.AsSlice(), 1, loc.Bytes())
	}
	if sc.readQueuesLen() != 4 {
	...
	for i := 1; i <= 4; i++ {
		...
		var loc common.Hash
		loc[1] = byte(i)
		if s, ok := sc.GetStorage(addr.AsSlice(), 1, loc.Bytes()); !ok || s != nil {
	...
	for i := 1; i <= 6; i++ {
		...
		var val common.Hash
		val[2] = byte(i)
		sc.SetStorageRead(addr.AsSlice(), 2, loc.Bytes(), val.Bytes())
	}
	if sc.readQueuesLen() != 4 {
		t.Fatalf("expected 4 reads got: %d", sc.readQueuesLen())
	}
	...
	for i := 1; i <= 2; i++ {
		...
		var loc common.Hash
		loc[1] = byte(i)
		if _, ok := sc.GetStorage(addr.AsSlice(), 2, loc.Bytes()); ok {
	...
	for i := 1; i <= 4; i++ {
		...
		var loc common.Hash
		loc[1] = byte(i)
		sc.SetStorageDelete(addr.AsSlice(), 1, loc.Bytes())
	}
	if sc.readQueuesLen() != 0 {
	...
	for i := 1; i <= 4; i++ {
		...
		var val common.Hash
		val[2] = byte(i)
		sc.SetStorageWrite(addr.AsArray(), 1, loc.Bytes(), val.Bytes())
	}
	if sc.WriteCount() != 4 {
	...
	for i := 1; i <= 4; i++ {
		...
		var loc common.Hash
		loc[1] = byte(i)
		if _, ok := sc.GetStorage(addr.AsSlice(), 1, loc.Bytes()); !ok {
...
func TestReadStorageExisting(t *testing.T) {
	...
	val1[2] = 1
	sc.SetStorageRead(addr1.AsSlice(), 1, loc1.Bytes(), val1.Bytes())
	defer func() {
		//nolint:staticcheck
		if r := recover(); r != nil {
		}
...
func TestCodeReadWriteAbsentDelete(t *testing.T) {
	...
	for i := 1; i <= 4; i++ {
		var addr = common.NewAddress(byte(i))
		sc.SetCodeAbsent(addr.AsSlice(), 1)
	}
	if sc.readQueuesLen() != 4 {
		t.Fatalf("expected 4 reads got: %d", sc.readQueuesLen())
	}
	for i := 1; i <= 4; i++ {
		var addr = common.NewAddress(byte(i))
		if c, ok := sc.GetCode(addr.AsSlice(), 1); !ok || c != nil {
	...
	for i := 1; i <= 6; i++ {
		...
		sc.SetCodeRead(addr.AsSlice(), 2, code)
	}
	if sc.readQueuesLen() != 4 {
		t.Fatalf("expected 4 reads got: %d", sc.readQueuesLen())
	}
	...
	for i := 1; i <= 2; i++ {
		var addr = common.NewAddress(byte(i))
		if _, ok := sc.GetCode(addr.AsSlice(), 2); ok {
	...
	for i := 1; i <= 4; i++ {
		var addr = common.NewAddress(byte(i))
		sc.SetCodeDelete(addr.AsSlice(), 1)
	}
	if sc.readQueuesLen() != 0 {
		t.Fatalf("expected 0 reads got: %d", sc.readQueuesLen())
	}
	...
	for i := 1; i <= 4; i++ {
		var addr = common.NewAddress(byte(i))
		var code = []byte{byte(i), 2, 3}
		sc.SetCodeWrite(addr.AsSlice(), 1, code)
	}
	if sc.WriteCount() != 4 {
		t.Fatalf("expected 4 writes, got %d", sc.WriteCount())
	}
	for i := 1; i <= 4; i++ {
		var addr = common.NewAddress(byte(i))
		if _, ok := sc.GetCode(addr.AsSlice(), 1); !ok {
...
func TestReadCodeExisting(t *testing.T) {
	...
	var addr1 = common.NewAddress(1)
	code1 := []byte{1, 2, 3}
	sc.SetCodeRead(addr1.AsSlice(), 1, code1)
	defer func() {
		//nolint:staticcheck
		if r := recover(); r != nil {
		}
