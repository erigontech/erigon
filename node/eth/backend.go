...
func New(ctx context.Context, stack *node.Node, config *ethconfig.Config, logger log.Logger, tracer *tracers.Tracer) (*Ethereum, error) {
	...
	// The validation stages bake in whatever context they are built with, so build them on a swappable one:
	// that is what lets a pre-exec round impose a deadline on execution (see execmodule.RoundContext).
	roundCtx := execmodule.NewRoundContext(backend.sentryCtx)
	validationSync := stageloop.NewInMemoryExecution(roundCtx, backend.chainDB, config, backend.sentryProvider.Client,
	...
	backend.rpcFilters.SetPreconfirmedProvider(backend.execModule.PinPreconfirmed, backend.execModule.PreconfirmedBody)
	...
	if config.InternalCL && (clparams.EmbeddedSupported(config.NetworkID) || config.CaplinConfig.IsDevnet()) {
		...
		if config.CaplinConfig.DevValidatorSeed != "" {
			go func() {
				// No gate here: the genesis time is applied inside RunCaplinService before the
				// beacon API opens, so /eth/v1/beacon/genesis cannot serve a placeholder and this
				// validator cannot build its clock from one.
