...
// NOTE: on the snapshot-flow branch this file also asserts
//   var _ rpchelper.ChainConfigRestartable = (*CaplinService)(nil)
// The ChainConfigRestartable interface is part of the CL-rewind
// orchestration that lives only on that branch. On the componentization
// integration branch the L2 wiring drives CaplinService directly
// (Start/Stop/Restart), so the interface conformance check is dropped
// here rather than dragging the rewind machinery over.
