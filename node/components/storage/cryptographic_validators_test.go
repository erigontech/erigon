...
func TestReceiptRootValidator_ReceiptDomainRequiresDeps(t *testing.T) {
	t.Parallel()
	// Zero-value PruneMode (uninitialised) → validator does not skip,
	// reaches the dep checks.
	v := ReceiptRootValidator{}
	files := []*snapshot.FileEntry{
...
// TestReceiptRootValidator_SkipsWhenHistoryPruned pins the contract:
// under any pruning mode (minimal/full/blocks) receipt history is
// pruned, so the receipt-root recompute has nothing to read — the
// check is skipped (return nil), before the dep checks. Receipt files
// are optional and an unvalidatable optional file is not a failure.
func TestReceiptRootValidator_SkipsWhenHistoryPruned(t *testing.T) {
	t.Parallel()
	v := ReceiptRootValidator{PruneMode: prune.MinimalMode}
	files := []*snapshot.FileEntry{
		{Domain: snapshot.DomainReceipt, FromStep: 0, ToStep: 256},
	}
	require.NoError(t, v.ValidateStep(context.Background(), files),
		"receipt validator must skip (not fail) when receipt history is pruned")
}
