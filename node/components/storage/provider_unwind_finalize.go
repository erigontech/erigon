...
func (p *Provider) FinalizeUnwind() error {
	...
	if p.logger != nil {
		p.logger.Info("[storage] Provider.FinalizeUnwind: deferred snapshot-trim ops executed", "files", len(staged.names))
	}
	return nil
}
...
func (p *Provider) AbortUnwind() {
	...
	p.pendingTrim = nil
	p.pendingTrimLock.Unlock()
	if staged != nil && len(staged.names) > 0 && p.logger != nil {
		p.logger.Info("[storage] Provider.AbortUnwind: staged snapshot-trim ops dropped (no FS / network mutations)", "files", len(staged.names))
	}
}
