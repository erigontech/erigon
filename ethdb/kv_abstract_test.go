...
func TestBucketCRUD(t *testing.T) {
	...
	for _, db := range writeDBs {
		msg := fmt.Sprintf("%T", db)
		if err := db.Update(ctx, func(tx ethdb.Tx) error {
			normalBucketMigrator, ok := tx.Bucket(dbutils.Buckets[15]).(ethdb.BucketMigrator)
			if !ok {
				return nil
			}
			deprecatedBucket := tx.Bucket(dbutils.DeprecatedBuckets[0])
			deprecatedBucketMigrator, ok := deprecatedBucket.(ethdb.BucketMigrator)
			if !ok {
				return nil
			}

			t.Run("BucketCRUD "+msg, func(t *testing.T) {
				require := require.New(t)

				require.True(normalBucketMigrator.Exists())
				require.True(errors.Is(normalBucketMigrator.Drop(), ethdb.ErrAttemptToDeleteNonDeprecatedBucket))
