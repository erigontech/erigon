...
func TestSequence(t *testing.T) {
	...
	for _, db := range writeDBs {
		...
		i, err = tx.MakeSequence(dbutils.Buckets[0], 1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.MakeSequence(dbutils.Buckets[0], 6)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.MakeSequence(dbutils.Buckets[1], 1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), i)
		i, err = tx.MakeSequence(dbutils.Buckets[1], 6)
		require.NoError(t, err)
		require.Equal(t, uint64(1), i)
		i, err = tx.MakeSequence(dbutils.Buckets[1], 1)
		require.NoError(t, err)
