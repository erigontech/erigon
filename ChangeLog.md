...
- rpc: the `callTracer` `withLog` option of `debug_traceTransaction`, `debug_traceBlock*` and `debug_traceCall*` now numbers a log's `index` over the whole block instead of restarting at `0x0` on every transaction, matching geth's `callTracer` and the index the same log carries in `eth_getLogs` and in its receipt. Each bundle of `debug_traceCallMany` still numbers from zero, since a bundle is a simulated block of its own. This can be a breaking change for clients that join trace logs to `eth_getLogs` indices (#23797) — by @lupin012
