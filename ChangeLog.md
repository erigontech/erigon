...
- db: the default `full` retention window and the `blocks` mode state-history window increase from 262,144 to 1,100,000 blocks (~5 months), slightly above the consensus layer's `MIN_EPOCHS_FOR_BLOCK_REQUESTS` window of 33,024 epochs (1,056,768 slots). Existing `full` and `blocks` datadirs using their mode defaults migrate automatically, including nodes already migrated by #21342. Archive and minimal modes are unchanged. Widening the stored window does not restore data already pruned under the old default; that history becomes available as the node advances, or immediately after a re-sync.
