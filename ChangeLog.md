...
- db: the global `full` retention window and the `blocks` mode state-history window increase from 262,144 to 1,100,000 blocks. The consensus-layer request window counts slots, with at most one execution block per slot; on Ethereum mainnet, the new distance conservatively covers the 1,056,768-slot `MIN_EPOCHS_FOR_BLOCK_REQUESTS` window with a 43,232-block margin, and missed slots extend the covered time span. Existing `full` and `blocks` datadirs using their mode defaults migrate automatically, including nodes already migrated by #21342; archive, minimal, and custom distances are unchanged. Disk use grows as the wider window fills, while fresh syncs download and process more snapshot data and can use more bandwidth, I/O, and time. Data already pruned under the old default returns only as the node advances or after a re-sync. Downgrading to Erigon 3.6 restores its 262,144-block default and can delete the newly retained backlog; preserve 1,100,000 as explicit history and block distances before a rollback if that data is needed. (#23708) — by @yperbasis
...
- rpc: prune gates and `eth_capabilities` now use the on-disk block and state-history floors as well as the configured windows, so a node refilling a widened window does not advertise or silently query data that is still missing. Head and already-pruned requests avoid physical-floor reads. Successful reads are cached for the exact chain head for up to one second; local block-floor entries also follow the pinned snapshot generation. Concurrent requests for the same head and generation share one floor read, while different heads refresh independently. (#23708) — by @yperbasis
