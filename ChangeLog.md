...
- build, docker: amd64 release artifacts (Docker image, `.tar.gz`, `.deb`) are now built for the `x86-64-v2` baseline (SSE4.2, POPCNT) instead of `x86-64`; the separate `amd64v2` Docker platform and tarball are removed since the plain `amd64` artifact already carries those instructions. CPUs older than Intel Nehalem (2008) / AMD Bulldozer (2011), and hypervisor profiles that mask v2 CPU features, fail with an illegal instruction (SIGILL) at startup. This can be a breaking change for operators pinned to the legacy `amd64v2`-tagged artifact or running on older/masked-CPU hosts (#23877) — by @AskAlexSharov
