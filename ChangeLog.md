...
- rpc: prune gates and `eth_capabilities` now use the on-disk block and state-history floors as well as the configured windows, so a node refilling a widened window does not advertise or silently query data that is still missing. Head and already-pruned requests avoid physical-floor reads, while successful floor reads are cached for the exact chain head for up to one second and concurrent refreshes are coalesced. (#23708) — by @yperbasis
