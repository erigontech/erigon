...
- rpc: prune gates and `eth_capabilities` now use the on-disk block and state-history floors as well as the configured windows, so a node refilling a widened window does not advertise or silently query data that is still missing (#23708) — by @yperbasis
