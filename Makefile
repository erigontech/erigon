...
GOTEST = CGO_CFLAGS="$(CGO_CFLAGS)" CGO_LDFLAGS="$(CGO_LDFLAGS)" GOPRIVATE="$(GOPRIVATE)" GODEBUG=cgocheck=0 GOTRACEBACK=1 $(GO) test $(GO_FLAGS) ./tests/... -p 2
...
test-integration: 
	$(GOTEST) --timeout 240m -tags $(BUILD_TAGS),integration -failfast
