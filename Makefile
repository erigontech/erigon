...
test: test-erigon-lib test-erigon-db test-p2
	$(GOTEST) -short --timeout 10m -coverprofile=coverage-test.out | grep -v '=== CONT ' | grep -v '=== RUN' | grep -v '=== PAUSE' | grep -v 'PASS: ' | grep -vE '^ok ' | grep -v 'coverage: '

## test-all:                  run all tests with a 1h timeout
test-all: test-erigon-lib-all test-erigon-db-all test-p2p-all
	$(GOTEST) --timeout 60m -coverprofile=coverage-test-all.out -race | grep -v '=== CONT ' | grep -v '=== RUN' | grep -v '=== PAUSE' | grep -v 'PASS: ' | grep -vE '^ok ' | grep -v 'coverage: '
