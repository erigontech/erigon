...
func CallerFileHandler(h Handler) Handler {
	return FuncHandler(func(r *Record) error {
		r.Ctx = append(r.Ctx, "caller", fmt.Sprint(r.Caller(6)))
		return h.Log(r)
	})
}
...
func CallerFuncHandler(h Handler) Handler {
	return FuncHandler(func(r *Record) error {
		r.Ctx = append(r.Ctx, "fn", fmt.Sprintf("%+n", r.Caller(6)))
		return h.Log(r)
	})
}
...
func CallerStackHandler(format string, h Handler) Handler {
	return FuncHandler(func(r *Record) error {
		s := stack.Trace().TrimBelow(r.Caller(6)).TrimRuntime()
		if len(s) > 0 {
			...
			if ok {
				...
				if err != nil {
					...
					if cs, ok := v.(stack.CallStack); ok {
						v = cs.TrimBelow(r.Caller(6)).TrimRuntime()
					}
					r.Ctx[i] = v
				}
			}
