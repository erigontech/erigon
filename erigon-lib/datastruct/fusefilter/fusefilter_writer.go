...
func NewWriter(filePath string) (*Writer, error) {
	...
	return &Writer{tmpFile: f, filePath: filePath}, nil
}
...
func (w *Writer) Build() error {
	...
	//xx, _ := xorfilter.Populate(keysHashes)
	//bf, _ := bloomfilter.New(bloomfilter.OptimalM(uint64(len(keysHashes)), 0.01))
	//for _, k := range keysHashes {
	//	bf.AddHash(k)
	//}
	//bfb, _ := bf.MarshalBinary()
	//_, fName := filepath.Split(w.filePath)
	//fmt.Printf("[dbg] sz: xor=%s, fuse=%s, bloom=%s, %s\n", datasize.ByteSize(len(xx.Fingerprints)).HumanReadable(), datasize.ByteSize(len(filter.Fingerprints)).HumanReadable(), datasize.ByteSize(len(bfb)).HumanReadable(), fName)
	...
	_header[0] = 1 //version
	header := _header[1:]
	binary.BigEndian.PutUint64(header[:], filter.Seed)
	...
	_, err = fw.Write(_header[:])
	if err != nil {
		return err
	}

	_, err = fw.Write([]byte(filter.Fingerprints))
	if err != nil {
		return err
	}
	err = fw.Flush()
...
func castToBytes(in []uint64) []byte {
	...
	return unsafe.Slice((*byte)(ptr), len(in)*8) // each uint64 is 8 bytes
}

// castToArrU64 converts []byte to []uint64 without copying data
