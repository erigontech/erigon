...
func Test_HexPatriciaHashed_UniqueRepresentation2(t *testing.T) {
	...
	var rSeq, rBatch []byte
	{
		...
		for i := 0; i < len(updates); i++ {
			...
			updsOne := WrapKeyUpdates(t, ModeDirect, HashAndNibblizeKey, plainKeys[i:i+1], updates[i:i+1])
	...
	}
	{
		...
		updsTwo := WrapKeyUpdates(t, ModeDirect, HashAndNibblizeKey, plainKeys, updates)
...
func sortUpdatesByHashIncrease(t *testing.T, plainKeys [][]byte, updates []Update) ([][]byte, []Update) {
	...
	for i, pk := range plainKeys {
		ku[i] = &KeyUpdate{plainKey: string(pk), hashedKey: HashAndNibblizeKey(pk), update: &updates[i]}
	}
...
func Test_HexPatriciaHashed_StateRestoreAndContinue(t *testing.T) {
	...
	updOne := WrapKeyUpdates(t, ModeDirect, HashAndNibblizeKey, plainKeys, updates)
	defer updOne.Close()
	...
	updTwo := WrapKeyUpdates(t, ModeDirect, HashAndNibblizeKey, plainKeys, updates)
	defer updTwo.Close()
...
func Test_HexPatriciaHashed_ProcessUpdates_UniqueRepresentationInTheMiddle(t *testing.T) {
	...
	}
	{
		...
		updsTwo := WrapKeyUpdates(t, ModeDirect, HashAndNibblizeKey, plainKeys[:somewhere+1], updates[:somewhere+1])
