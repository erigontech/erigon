...
func (cell *Cell) FullString() string {
	...
	if cell.alhlen > 0 {
		b.WriteString(fmt.Sprintf("memHash=%x ", cell.accLeafHash[:cell.alhlen]))
	}
	if cell.extLen > 0 {
...
func (cell *Cell) fillFromLowerCell(lowCell *Cell, lowDepth int, preExtension []byte, nibble int) {
	...
	// if lowCell.alhlen > 0 {
	// 	copy(cell.accLeafHash[:], lowCell.accLeafHash[:cell.alhlen])
	// 	cell.alhlen = lowCell.alhlen
	// } else {
	// 	cell.alhlen = 0
	// }
...
func (cell *Cell) fillFromFields(data []byte, pos int, fieldBits PartFlags) (int, error) {
	...
	if fieldBits&AccLeafHashPart != 0 {
		...
		if l > 0 {
			...
			copy(cell.accLeafHash[:], data[pos:pos+int(l)])
			cell.alhlen = int(l)
			pos += int(l)
		}
	} else {
...
func (cell *Cell) setStorage(value []byte) {
	...
	cell.loaded.addFlag(cellLoadStorage)
}
...
func (hph *HexPatriciaHashed) computeCellHash(cell *Cell, depth int, buf []byte) ([]byte, error) {
	...
	if cell.spl > 0 {
		...
		if singleton {
			if !cell.loaded.storage() && cell.alhlen > 0 {
	...
	if cell.apl > 0 {
		...
		if !cell.loaded.account() {
			...
			fmt.Printf("account was not loaded %s\n", cell.FullString())
		}
		...
		accLeafHash, err := hph.accountLeafHashWithKey(buf, cell.downHashedKey[:65-depth], rlp.RlpEncodedBytes(valBuf[:valLen]))
		if err != nil {
			return nil, err
		}
		fmt.Printf("NEW %x\n", accLeafHash)
...
func (hph *HexPatriciaHashed) needUnfolding(hashedKey []byte) int {
	...
	if hph.activeRows == 0 {
		...
		if hph.trace {
			fmt.Printf("currentKey: [%x] needUnfolding cell (%d, %x, depth=%d), cell.h=[%x]\n", hph.currentKey[:hph.currentKeyLen], hph.activeRows-1, col, depth, cell.h[:cell.hl])
		}
	}
...
func (hph *HexPatriciaHashed) fold() (err error) {
	...
	default: // Branch node
		...
		for bitset, j := hph.afterMap[row], 0; bitset != 0; j++ {
			...
			nibble := bits.TrailingZeros16(bit)
			cell := &hph.grid[row][nibble]
			fmt.Printf("foldingCell MODIFIED+ALHASH=%t DEPTHCOND=%t %s\n", hph.touchMap[row]&hph.afterMap[row]&uint16(1<<nibble) > 0 && cell.alhlen > 0, depth <= 64 && cell.spl > 0, cell.FullString())
			if hph.touchMap[row]&hph.afterMap[row]&uint16(1<<nibble) > 0 && cell.alhlen > 0 ||
				depth <= 64 && cell.spl > 0 {
				fmt.Printf("row %d nibble %x, depth=%d hash DROP %s\n", row, 1<<nibble, depth, cell.FullString())
				cell.alhlen = 0
				hadToReset.Add(1)
			}
			if cell.alhlen == 0 {
				if !cell.loaded.account() && cell.apl > 0 {
					hadToLoad.Add(1)
					if err := hph.ctx.GetAccount(cell.apk[:cell.apl], cell); err != nil {
				...
				if !cell.loaded.storage() && cell.spl > 0 {
					hadToLoad.Add(1)
					if err := hph.ctx.GetStorage(cell.spk[:cell.spl], cell); err != nil {
