...
type Cell struct {
	...
	Storage         [length.Hash]byte               // storage value/storage root hash
	...
	downHashedLen      int  // length of downHashedKey, if any
	hashLen            int  // Length of the hash (or embedded)
	StorageLen         int  // length of storage value/storageRootHash
	...
	extLen             int  // length of extension, >0 when downHashedKeyLen >0
	Delete             bool // true if cell should be deleted

	loaded cellLoaded // folded Cell have only hash, unfolded have all fields
}

type cellLoaded uint8

func (f cellLoaded) String() string {
...
func (f cellLoaded) account() bool {
	return f&cellLoadAccount != 0
}

func (f cellLoaded) storage() bool {
	return f&cellLoadStorage != 0
}

func (f cellLoaded) addFlag(loadFlags cellLoaded) cellLoaded {
...
const (
	cellLoadNone    = cellLoaded(0)
	cellLoadAccount = cellLoaded(1)
	cellLoadStorage = cellLoaded(2)
)
...
func (cell *Cell) String() string {
	...
	if cell.accountPlainKeyLen > 0 {
		b.WriteString(fmt.Sprintf("apk=%x ", cell.accountPlainKey[:cell.accountPlainKeyLen]))
	}
	if cell.storagePlainKeyLen > 0 {
		b.WriteString(fmt.Sprintf("spk=%x", cell.storagePlainKey[:cell.storagePlainKeyLen]))
...
func (cell *Cell) FullString() string {
	...
	if cell.downHashedLen > 0 {
		b.WriteString(fmt.Sprintf("downHashedKey=%x ", cell.downHashedKey[:cell.downHashedLen]))
	}
...
func (cell *Cell) fillFromUpperCell(upCell *Cell, depth, depthIncrement int) {
	...
	// if upCell.lhLen > 0 {
	// 	copy(cell.leafHash[:], upCell.leafHash[:upCell.lhLen])
	// 	cell.lhLen = upCell.lhLen
	// } else {
	// 	cell.lhLen = 0
	// }
...
func (cell *Cell) fillFromLowerCell(lowCell *Cell, lowDepth int, preExtension []byte, nibble int) {
	...
	// if lowCell.lhLen > 0 {
	// 	copy(cell.leafHash[:], lowCell.leafHash[:cell.lhLen])
	// 	cell.lhLen = lowCell.lhLen
	// } else {
	// 	cell.lhLen = 0
	// }
...
func (hph *HexPatriciaHashed) computeCellHashLen(cell *Cell, depth int) int {
	if cell.storagePlainKeyLen > 0 && depth >= 64 {
		...
		var lenPrefix [4]byte
		pt := rlp.GenerateStructLen(lenPrefix[:], totalLen)
		if totalLen+pt < length.Hash-1 { // still print if len < 31
			fmt.Printf("\tTOTAL LEN %d + PREFIXLen %d = %d\n", totalLen, pt, totalLen+pt)
			return totalLen + pt
		}
	}
		...
		if singleton {
			if !cell.loaded.storage() {
				if cell.lhLen > 0 {
					hph.keccak.Reset()
					if hph.trace {
						fmt.Printf("LEAF HASH storage USED %x spk %x\n", cell.leafHash[:cell.lhLen], cell.storagePlainKey[:cell.storagePlainKeyLen])
					}
					skippedLoad.Add(1)
					storageRootHash = *(*[length.Hash]byte)(cell.leafHash[:cell.lhLen])
					storageRootHashIsSet = true
				} else {
					if err = hph.ctx.GetStorage(cell.storagePlainKey[:cell.storagePlainKeyLen], cell); err != nil {
						return nil, err
					}
					hadToLoad.Add(1)
					cell.loaded = cell.loaded.addFlag(cellLoadStorage)
			...
			if !cell.loaded.storage() {
				if cell.lhLen > 0 {
					...
					hph.keccak.Reset()
					if hph.trace {
						fmt.Printf("LEAF HASH storage USED %x spk %x\n", res, cell.storagePlainKey[:cell.storagePlainKeyLen])
					}
					skippedLoad.Add(1)
					return res, nil
				}
			...
			if hph.trace {
				fmt.Printf("LEAF HASH storage memoized %x spk %x\n", accLeafHash, cell.storagePlainKey[:cell.storagePlainKeyLen])
			}
	...
	if cell.accountPlainKeyLen > 0 {
		...
		if !cell.loaded.account() {
			if cell.lhLen > 0 {
				...
				skippedLoad.Add(1)
				if hph.trace {
					fmt.Printf("LEAF HASH account USED %x apk %x\n", res, cell.accountPlainKey[:cell.accountPlainKeyLen])
				}
				return res, nil
			}
			if err = hph.ctx.GetAccount(cell.accountPlainKey[:cell.accountPlainKeyLen], cell); err != nil {
				return nil, err
			}
			hadToLoad.Add(1)
			cell.loaded = cell.loaded.addFlag(cellLoadAccount)
		...
		leafHash, err := hph.accountLeafHashWithKey(buf, cell.downHashedKey[:65-depth], rlp.RlpEncodedBytes(valBuf[:valLen]))
		if err != nil {
			return nil, err
		}
		if hph.trace {
...
func (hph *HexPatriciaHashed) unfold(hashedKey []byte, unfolding int) error {
	...
	if hph.activeRows == 0 {
		...
		if hph.trace {
			fmt.Printf("unfold root: touched: %t present: %t downHashedKey: %x\n", touched, present, upCell.downHashedKey[:upCell.downHashedLen])
		}
	} else {
	...
	if upCell.downHashedLen >= unfolding {
		depth = upDepth + unfolding
		nibble = int(upCell.downHashedKey[unfolding-1])
		copyLen = unfolding - 1
	} else {
		depth = upDepth + upCell.downHashedLen
		nibble = int(upCell.downHashedKey[upCell.downHashedLen-1])
		copyLen = upCell.downHashedLen - 1
	}
	...
	cell := &hph.grid[row][nibble]
	cell.fillFromUpperCell(upCell, depth, min(unfolding, upCell.downHashedLen))
	if hph.trace {
	...
	if copyLen > 0 {
		copy(hph.currentKey[hph.currentKeyLen:], upCell.downHashedKey[:copyLen])
	}
...
func (hph *HexPatriciaHashed) fold() (err error) {
	...
	default: // Branch node
		...
		for bitset, j := hph.afterMap[row], 0; bitset != 0; j++ {
			...
			nibble := bits.TrailingZeros16(bit)
			cell := &hph.grid[row][nibble]
			if hph.touchMap[row]&hph.afterMap[row]&uint16(1<<nibble) > 0 && cell.lhLen > 0 || depth <= 64 && cell.storagePlainKeyLen > 0 {
				if hph.trace {
			...
			if cell.lhLen == 0 {
				if !cell.loaded.account() && cell.accountPlainKeyLen > 0 {
					hadToLoad.Add(1)
					if err := hph.ctx.GetAccount(cell.accountPlainKey[:cell.accountPlainKeyLen], cell); err != nil {
				...
				if !cell.loaded.storage() && cell.storagePlainKeyLen > 0 {
					hadToLoad.Add(1)
					if err := hph.ctx.GetStorage(cell.storagePlainKey[:cell.storagePlainKeyLen], cell); err != nil {
			...
			if !cell.loaded.storage() && cell.storagePlainKeyLen > 0 && cell.lhLen > 0 {
				b := 0
				if cell.lhLen == 32 {
					b = 1
				}
...
func (hph *HexPatriciaHashed) ProcessTree(ctx context.Context, tree *Updates, logPrefix string) (rootHash []byte, err error) {
	...
	err = tree.HashSort(ctx, func(hashedKey, plainKey []byte) error {
		...
		// Update the cell
		stagedCell.reset() // instead we can use needed cell in a row directly
		if len(plainKey) == hph.accountKeyLen {
			...
			if !stagedCell.Delete {
				...
				if hph.trace {
					fmt.Printf("Proc:GetAccount %s", cell.FullString())
				}
			}
		} else {
			...
			if !stagedCell.Delete {
				...
				if hph.trace {
					fmt.Printf("Proc:GetStorage %s", cell.FullString())
				}
			}
		}
		...
		if stagedCell.Delete {
			if hph.trace {
				fmt.Printf("DELETE %x:  %s\n", plainKey, stagedCell.FullString())
			}
...
func (hph *HexPatriciaHashed) ProcessKeys(ctx context.Context, plainKeys [][]byte, logPrefix string) (rootHash []byte, err error) {
	...
	for i, hashedKey := range hashedKeys {
		...
		if len(plainKey) == hph.accountKeyLen {
			...
			if !stagedCell.Delete {
				cell := hph.updateCell(plainKey, hashedKey) // TODO could fetch data directly to needed cell instead
				cell.setAccountFields(stagedCell.CodeHash[:], &stagedCell.Balance, stagedCell.Nonce)
...
func (hph *HexPatriciaHashed) SetState(buf []byte) error {
	...
	if hph.root.accountPlainKeyLen > 0 {
		...
		if err := hph.ctx.GetAccount(hph.root.accountPlainKey[:hph.root.accountPlainKeyLen], &hph.root); err != nil {
			return err
		}
	}
	if hph.root.storagePlainKeyLen > 0 {
		...
		if err := hph.ctx.GetStorage(hph.root.storagePlainKey[:hph.root.storagePlainKeyLen], &hph.root); err != nil {
			return err
		}
