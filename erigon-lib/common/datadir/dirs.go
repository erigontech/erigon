...
type Dirs struct {
	...
	Nodes                     string
	CaplinBlobs               string
	CaplinBlobRecoveryRequest string
...
func New(datadir string) Dirs {
	...
	dirs := Dirs{
		...
		CaplinBlobRecoveryRequest: filepath.Join(datadir, "caplin", "blob-recovery-request"),
	...
	dir.MustExist(dirs.Chaindata, dirs.Tmp,
		...
		dirs.Downloader, dirs.TxPool, dirs.Nodes, dirs.CaplinBlobs, dirs.CaplinIndexing, dirs.CaplinLatest, dirs.CaplinGenesis, dirs.CaplinColumnData, dirs.CaplinBlobRecoveryRequest)
...
func (d Dirs) RenameOldVersions() error {
	directories := []string{
		...
		d.Nodes, d.CaplinBlobs, d.CaplinIndexing, d.CaplinLatest, d.CaplinGenesis, d.CaplinColumnData, d.CaplinBlobRecoveryRequest,
	}
	renamed := 0
	torrentsRemoved := 0
	removed := 0
