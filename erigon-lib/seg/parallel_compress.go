...
func extractPatternsInSuperstrings(ctx context.Context, superstringCh chan []byte, dictCollector *etl.Collector, cfg Params, completion *sync.WaitGroup, logger log.Logger) {
...
func DictionaryBuilderFromCollectors(ctx context.Context, cfg Params, logPrefix, tmpDir string, collectors []*etl.Collector, lvl log.Lvl, logger log.Logger) (*DictionaryBuilder, error) {
	t := time.Now()
