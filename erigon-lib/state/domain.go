...
func (d *Domain) openDirtyFiles() (err error) {
	...
	d.dirtyFiles.Walk(func(items []*filesItem) bool {
		for _, item := range items {
			...
			if item.existence == nil && d.Accessors&AccessorExistence != 0 {
...
func (w *domainBufferedWriter) PutWithPrev(key1, key2, val, preval []byte, prevStep uint64) error {
	step := w.h.ii.txNum / w.h.ii.aggregationStep
...
func (d *Domain) buildFileRange(ctx context.Context, stepFrom, stepTo uint64, collation Collation, ps *background.ProgressSet) (StaticFiles, error) {
	...
	if d.Accessors.Has(AccessorBTree) {
		...
		bt, err = CreateBtreeIndexWithDecompressor(btPath, btM, valuesDecomp, d.Compression, *d.salt, ps, d.dirs.Tmp, d.logger, d.noFsync, d.Accessors)
		if err != nil {
...
func (d *Domain) buildFiles(ctx context.Context, step uint64, collation Collation, ps *background.ProgressSet) (StaticFiles, error) {
	...
	if d.Accessors&AccessorHashMap != 0 {
	...
	if d.Accessors&AccessorBTree != 0 {
	...
	if d.Accessors&AccessorExistence != 0 {
...
func buildHashMapAccessor(ctx context.Context, d *seg.Decompressor, compressed seg.FileCompression, idxPath string, values bool, cfg recsplit.RecSplitArgs, ps *background.ProgressSet, logger log.Logger) (err error) {
	defer func() {
		if rec := recover(); rec != nil {
			err = fmt.Errorf("buildHashMapAccessor: %s, %s", rec, dbg.Stack())
		}
	}()
	count := d.Count()
	if !values {
