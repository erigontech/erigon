...
func (d *Domain) buildFileRange(ctx context.Context, stepFrom, stepTo uint64, collation Collation, ps *background.ProgressSet) (StaticFiles, error) {
	...
	valuesComp.Close()
	valuesComp = nil
	_valuesDecomp, err := seg.NewDecompressor(collation.valuesPath)
	if err != nil {
		return StaticFiles{}, fmt.Errorf("open %s values decompressor: %w", d.filenameBase, err)
	}

	if d.Accessors.Has(AccessorHashMap) {
		if err = d.buildHashMapAccessor(ctx, stepFrom, stepTo, d.dataReader(_valuesDecomp), ps); err != nil {
	...
	if d.Accessors.Has(AccessorBTree) {
		...
		bt, err = CreateBtreeIndexWithDecompressor(btPath, DefaultBtreeM, d.dataReader(_valuesDecomp), *d.salt.Load(), ps, d.dirs.Tmp, d.logger, d.noFsync, d.Accessors)
		if err != nil {
...
func NewSegStreamReader(s *seg.PagedReader, limit int) *SegStreamReader {
	s.Reset(0)
...
func (sr *SegStreamReader) Next() (k, v []byte, err error) {
	k, v, _, _, _ = sr.s.Next2Copy(nil, nil)
	if sr.limit > 0 {
		sr.limit--
	}
	return k, v, nil
