...
func (h *History) vFileName(fromStep, toStep uint64) string {
	return fmt.Sprintf("%s-%s.%d-%d.v", h.versionType.DataV.String(), h.filenameBase, fromStep, toStep)
}
...
func (h *History) vAccessorFilePath(fromStep, toStep uint64) string {
	return filepath.Join(h.dirs.SnapAccessors, fmt.Sprintf("%s-%s.%d-%d.vi", h.versionType.AccessorVI.String(), h.filenameBase, fromStep, toStep))
}
