...
func (ht *HistoryRoTx) historySeekInFiles(key []byte, txNum uint64) ([]byte, bool, error) {
	...
	if traceGetAsOf == ht.h.filenameBase {
		fmt.Printf("DomainGetAsOf(%s, %x, %d) -> %s, histTxNum=%d, isNil(v)=%t offset=(%d) compression:%s\n", ht.h.filenameBase, key, txNum, g.FileName(), histTxNum, v == nil, offset, ht.h.histCfg.Compression.String())
	}
