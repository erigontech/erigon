...
func testDbAndAggregatorv3(tb testing.TB, aggStep uint64) (kv.RwDB, *Aggregator) {
	...
	db := mdbx.NewMDBX(logger).InMem(dirs.Chaindata).Label(kv.ChainDB).GrowthStep(32 * datasize.MB).MapSize(2 * datasize.GB).WithTableCfg(func(defaultBuckets kv.TableCfg) kv.TableCfg {
		return kv.ChaindataTablesCfg
	}).MustOpen()
	tb.Cleanup(db.Close)
