...
func TestAggregatorV3_Merge(t *testing.T) {
	...
	for txNum := uint64(1); txNum <= txs; txNum++ {
		...
		err = domains.DomainPut(kv.StorageDomain, addr, []byte{addr[0], loc[0]}, nil, 0)
		require.NoError(t, err)
	...
	for j := uint64(0); j < keysCount; j++ {
		...
		case r > 66 && r <= 80:
			...
			for i := 0; i < maxStorageKeys; i++ {
				...
				err = domains.DomainPut(kv.StorageDomain, sk[:length.Addr], uint256.NewInt(txNum).Bytes(), prev, step)
				require.NoError(t, err)
			}
...
func TestAggregatorV3_ReplaceCommittedKeys(t *testing.T) {
	...
	for txNum = uint64(1); txNum <= txs/2; txNum++ {
		...
		err = domains.DomainPut(kv.StorageDomain, addr, []byte{addr[0], loc[0]}, prev2, 0)
		require.NoError(t, err)
	...
	for txNum = txNum + 1; txNum <= txs; txNum++ {
		...
		require.NoError(t, err)
		err = domains.DomainPut(kv.StorageDomain, addr, []byte{addr[0], loc[0]}, prev, step)
		require.NoError(t, err)
	}
