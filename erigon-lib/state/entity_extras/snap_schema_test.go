...
func TestE2SnapSchema(t *testing.T) {
	...
	info, ok := p.Parse("v1-000100-000500-bodies.seg")
	require.True(t, ok)

	require.Equal(t, "v1-000100-000500-bodies.seg", info.Name)
	...
	info, ok = p.Parse("v1-000100-000500-bodies.idx")
	require.True(t, ok)
	require.Equal(t, "v1-000100-000500-bodies.idx", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(100*1000), info.From)
	require.Equal(t, uint64(500*1000), info.To)
	require.Equal(t, "bodies", info.FileType)
	...
	_, ok = p.Parse("v1-000100-000500-headers.seg")
	require.False(t, ok)

	_, ok = p.Parse("v1-000100-000500-bodies.dat")
	require.False(t, ok)

	// for transactions
	p = NewE2SnapSchemaWithIndexTag(dirs, "transactions", []string{"transactions", "transactions-to-block"})
	_, ok = p.Parse("v1-000100-000500-transactions.seg")
	require.True(t, ok)

	info, ok = p.Parse("v1-000100-000500-transactions-to-block.idx")
	require.True(t, ok)
	require.Equal(t, "v1-000100-000500-transactions-to-block.idx", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(100*1000), info.From)
	require.Equal(t, uint64(500*1000), info.To)
	...
	info, ok = p.Parse("v1-000100-000500-transactions.idx")
	require.True(t, ok)
	require.Equal(t, "v1-000100-000500-transactions.idx", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(100*1000), info.From)
	require.Equal(t, uint64(500*1000), info.To)
	...
	require.Equal(t, "v1-000005-000020-transactions.seg", fName)

	acc1Full := p.AccessorIdxFile(Version(1), stepFrom, stepTo, 0)
	_, fName = filepath.Split(acc1Full)
	require.Equal(t, "v1-000005-000020-transactions.idx", fName)

	acc2Full := p.AccessorIdxFile(Version(1), stepFrom, stepTo, 1)
	_, fName = filepath.Split(acc2Full)
	require.Equal(t, "v1-000005-000020-transactions-to-block.idx", fName)
...
func TestE3SnapSchemaForDomain1(t *testing.T) {
	...
	info, ok = p.Parse("v1-accounts.288-296.kv")
	require.True(t, ok)
	...
	info, ok = p.Parse("v1-accounts.288-296.kvei")
	require.True(t, ok)
...
func TestE3SnapSchemaForCommitmentDomain(t *testing.T) {
	...
	info, ok = p.Parse("v1-commitments.288-296.kvi")
	require.True(t, ok)
...
func TestE3SnapSchemaForHistory(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "v1-accounts.192-256.v", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(stepFrom), info.From)
	require.Equal(t, uint64(stepTo), info.To)
	require.Equal(t, "accounts", info.FileType)
	...
	info, ok = p.Parse("v1-accounts.192-256.vi")
	require.True(t, ok)
	require.Equal(t, "v1-accounts.192-256.vi", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(stepFrom), info.From)
	require.Equal(t, uint64(stepTo), info.To)
	require.Equal(t, "accounts", info.FileType)
...
func TestE3SnapSchemaForII(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "v1-logaddrs.128-192.ef", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(stepFrom), info.From)
	require.Equal(t, uint64(stepTo), info.To)
	...
	info, ok = p.Parse("v1-logaddrs.128-192.efi")
	require.True(t, ok)
	require.Equal(t, "v1-logaddrs.128-192.efi", info.Name)
	require.Equal(t, Version(1), info.Version)
	require.Equal(t, uint64(stepFrom), info.From)
	require.Equal(t, uint64(stepTo), info.To)
	require.Equal(t, "logaddrs", info.FileType)
	...
	_, ok = p.Parse("v1-logaddrs.128-192.crazy")
	require.False(t, ok)
