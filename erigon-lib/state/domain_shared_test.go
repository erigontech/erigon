...
func TestSharedDomain_IteratePrefix(t *testing.T) {
	...
	for i := uint64(0); i < stepSize*2; i++ {
		domains.SetTxNum(i)
		if err = domains.DomainPut(kv.AccountsDomain, acc(i), nil, acc(i), nil); err != nil {
			panic(err)
		}
		if err = domains.DomainPut(kv.StorageDomain, acc(i), nil, acc(i), nil); err != nil {
			panic(err)
		}
	}
	...
	{ // delete marker is in RAM
		...
		if err := domains.DomainDel(kv.StorageDomain, acc(1), nil, nil); err != nil {
			panic(err)
		}
		if err := domains.DomainDel(kv.StorageDomain, acc(stepSize+2), nil, nil); err != nil {
			panic(err)
		}
	...
	{ // delete/update more keys in RAM
		...
		if err := domains.DomainDel(kv.StorageDomain, acc(4), nil, nil); err != nil {
			panic(err)
		}
		if err := domains.DomainPut(kv.StorageDomain, acc(5), nil, acc(5), nil); err != nil {
			panic(err)
		}
