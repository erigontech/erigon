...
func TestDomain_IterationBasic(t *testing.T) {
	...
	writer.SetTxNum(2)
	err = writer.Put([]byte("addr1"), []byte("loc1"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr1"), []byte("loc2"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr1"), []byte("loc3"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr2"), []byte("loc1"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr2"), []byte("loc2"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr3"), []byte("loc1"), []byte("value1"), tx)
	require.NoError(t, err)
	err = writer.Put([]byte("addr3"), []byte("loc2"), []byte("value1"), tx)
	require.NoError(t, err)
	dc.Close()
