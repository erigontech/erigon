...
func TestInvIndexAfterPrune(t *testing.T) {
	...
	err = db.Update(ctx, func(tx kv.RwTx) error {
		...
		err = ic.Prune(ctx, tx, 0, 16, math.MaxUint64, logEvery, nil, false)
		require.NoError(t, err)
		return nil
	})
	require.NoError(t, err)
...
func mergeInverted(tb testing.TB, db kv.RwDB, ii *InvertedIndex, txs uint64) {
	...
	for step := uint64(0); step < txs/ii.aggregationStep-1; step++ {
		func() {
			...
			ic := ii.MakeContext()
			defer ic.Close()
			err = ic.Prune(ctx, tx, step*ii.aggregationStep, (step+1)*ii.aggregationStep, math.MaxUint64, logEvery, nil, false)
			require.NoError(tb, err)
			var found bool
