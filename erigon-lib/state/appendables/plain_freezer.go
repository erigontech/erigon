...
type ValueKeyFetcher interface {
	...
	GetKeys(from, to BaseNum, tx kv.Tx) stream.Uno[VKType]
}
...
// what does this do?
func (sf *PlainFreezer) Freeze(ctx context.Context, from, to BaseNum, tx kv.Tx) error {
	can_stream := sf.fetcher.GetKeys(from, to, tx)
