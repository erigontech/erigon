...
func testDbAggregatorWithFiles(t testing.TB, cfg *testAggConfig) (kv.RwDB, *Aggregator) {
	t.Helper()

	db, agg := testDbAndAggregatorv3(t.(*testing.T), cfg.stepSize)
	...
	keys, vals := generateInputData(t, length.Addr, 16, txCount)
