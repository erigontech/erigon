...
func (ii *InvertedIndex) reCalcVisibleFiles(toTxNum uint64) {
	...
	ii._visible = newIIVisible(ii.filenameBase, calcVisibleFiles(ii.dirtyFiles, ii.Accessors, checker, true, toTxNum))
}
	...
	ii.dirtyFiles.Walk(func(items []*FilesItem) bool {
		for _, item := range items {
			...
			if item.existence == nil {
				...
				if exists {
					if item.existence, err = existence.OpenFilter(fPath, true); err != nil {
						_, fName := filepath.Split(fPath)
						ii.logger.Warn("[agg] InvertedIndex.openDirtyFiles", "err", err, "f", fName)
						// don't interrupt on error. other files may be good
					}
...
func (iit *InvertedIndexRoTx) seekInFiles(key []byte, txNum uint64) (found bool, equalOrHigherTxNum uint64, err error) {
	...
	for i := 0; i < len(iit.files); i++ {
		...
		var offset uint64
		var ok bool
		existenceFilter := iit.files[i].src.existence
		if existenceFilter != nil {
			ok = existenceFilter.ContainsHash(hi)
			if !ok {
				continue
			}
...
func (ii *InvertedIndex) buildFiles(ctx context.Context, step uint64, coll InvertedIndexCollation, ps *background.ProgressSet) (InvertedFiles, error) {
	...
	if ii.Accessors.Has(AccessorExistence) {
		if existenceFilter, err = existence.OpenFilter(ii.efAccessorExistenceFilterFilePath(step, step+1), true); err != nil {
			return InvertedFiles{}, err
		}
	}
...
func (ii *InvertedIndex) buildMapAccessor(ctx context.Context, fromStep, toStep uint64, data *seg.Reader, ps *background.ProgressSet) error {
	...
	if ii.iiCfg.Accessors.Has(AccessorExistence) {
		useFuse := true
		if err := buildExistanceFilter(ctx, data, useFuse, ii.efAccessorExistenceFilterFilePath(fromStep, toStep), *cfg.Salt, ps); err != nil {
			return err
		}
	}
	return nil
