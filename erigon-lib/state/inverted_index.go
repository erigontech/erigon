...
func (iit *InvertedIndexRoTx) prune(ctx context.Context, rwTx kv.RwTx, txFrom, txTo, limit uint64, logEvery *time.Ticker, fn func(key []byte, maxTxNum uint64) error) (stat *InvertedIndexPruneStat, err error) {
	...
	var txKey [8]byte
	binary.BigEndian.PutUint64(txKey[:], txTo)

	// Invariant: if some `txNum=N` pruned - it's pruned Fully
	// Means: can use DeleteCurrentDuplicates all values of given `txNum`
	k, _, err := keysCursor.Seek(txKey[:])
	if err != nil {
		return nil, fmt.Errorf("iterate over %s index keys: %w", ii.filenameBase, err)
	}
	if k == nil {
		k, _, err = keysCursor.Last()
		if err != nil {
			return nil, fmt.Errorf("iterate over %s index keys: %w", ii.filenameBase, err)
		}
	}
	for ; k != nil; k, _, err = keysCursor.PrevNoDup() {
		if err != nil {
			return nil, fmt.Errorf("iterate over %s index keys: %w", ii.filenameBase, err)
		}

		txNum := binary.BigEndian.Uint64(k)
		fmt.Printf("prune see: txNum=%d, %d-%d\n", txNum, txFrom, txTo)
		if txNum >= txTo {
			continue
		}
		if txNum < txFrom || limit == 0 {
			break
		}
		limit--
		...
		fmt.Printf("prune keys: kCopy=%x\n", kCopy)
		if err = rwTx.Delete(ii.keysTable, kCopy); err != nil {
			return nil, err
		}
