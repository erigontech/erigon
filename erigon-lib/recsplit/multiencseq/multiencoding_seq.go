...
func (s *MultiEncodingSequence) AddOffset(offset uint64) {
	if s.currentEnc == SimpleEncoding {
		s.sseq.AddOffset(offset)
		return
	} else if s.currentEnc == PlainEliasFano {
		s.ef.AddOffset(offset)
		return
	}
	panic("unknown encoding")
}

func (s *MultiEncodingSequence) Build() {
	if s.currentEnc == SimpleEncoding {
		s.sseq.Build()
		return
	} else if s.currentEnc == PlainEliasFano {
