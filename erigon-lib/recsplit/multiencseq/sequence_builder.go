...
// A MultiEncodingSeqBuilder is used to build serialized number sequences.
//
...
type MultiEncodingSeqBuilder struct {
	baseNum  uint64
...
func NewBuilder(baseNum, count, max uint64, optimize bool) *MultiEncodingSeqBuilder {
	return &MultiEncodingSeqBuilder{
		baseNum:  baseNum,
...
func (b *MultiEncodingSeqBuilder) AddOffset(offset uint64) {
	b.ef.AddOffset(offset)
}

func (b *MultiEncodingSeqBuilder) Build() {
	b.ef.Build()
}

func (b *MultiEncodingSeqBuilder) AppendBytes(buf []byte) []byte {
	if !b.optimize {
...
func (b *MultiEncodingSeqBuilder) simpleEncoding(buf []byte) []byte {
...
func (b *MultiEncodingSeqBuilder) rebasedEliasFano(buf []byte) []byte {
