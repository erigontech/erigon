...
func (rs *RecSplit) Build(ctx context.Context) error {
	...
	log.Warn(fmt.Sprintf("[dbg] build done: %s, leaf=%d, failsCnt=%d, bktSz=%d, took=%s, fileSize=%skb\n", rs.indexFileName, rs.leafSize, rs.failsCnt, rs.bucketSize, time.Since(rs.t), datasize.ByteSize(st.Size()).KBytes()))
	return nil
}
