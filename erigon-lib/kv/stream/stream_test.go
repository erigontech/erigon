...
func TestUnionPairs(t *testing.T) {
	...
	t.Run("simple", func(t *testing.T) {
		...
		_ = tx.Put(kv.AccountChangeSetDeprecated, []byte{2}, []byte{9})
		_ = tx.Put(kv.AccountChangeSetDeprecated, []byte{3}, []byte{9})
		it, _ := tx.Range(kv.E2AccountsHistory, nil, nil, order.Asc, kv.Unlim)
		it2, _ := tx.Range(kv.AccountChangeSetDeprecated, nil, nil, order.Asc, kv.Unlim)
