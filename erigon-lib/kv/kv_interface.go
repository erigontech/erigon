...
//         - auto-open/close agg.BeginFilesRoTx() on Begin/Commit/Rollback
//         - to keep DB small - only for Hot/Recent data (can be update/delete by re-org).
//         - And TemporalRoTx/TemporalRwTx actually open Read-Only files view (BeginFilesRoTx) - no concept of "Read-Write view of snapshot files".
//         - using next entities:
