...
// IntersectUno
type IntersectUno[T constraints.Ordered] struct {
	x, y               Uno[T]
...
func Intersect[T constraints.Ordered](x, y Uno[T], limit int) Uno[T] {
	...
	m := &IntersectUno[T]{x: x, y: y, limit: limit}
	m.advance()
	return m
}
func (m *IntersectUno[T]) HasNext() bool {
	return m.err != nil || (m.limit != 0 && m.xHasNext && m.yHasNext)
}
func (m *IntersectUno[T]) advance() {
	m.advanceX()
	m.advanceY()
...
func (m *IntersectUno[T]) advanceX() {
	if m.err != nil {
		return
	}
...
func (m *IntersectUno[T]) advanceY() {
	if m.err != nil {
		return
	}
...
func (m *IntersectUno[T]) Next() (T, error) {
	if m.err != nil {
...
// FilteredUno - analog `map` (in terms of map-filter-reduce pattern)
// please avoid reading from Disk/DB more elements and then filter them. Better
// push-down filter conditions to lower-level iterator to reduce disk reads amount.
type FilteredUno[T any] struct {
	it      Uno[T]
	filter  func(T) bool
	hasNext bool
	err     error
...
func Filter[T any](it Uno[T], filter func(T) bool) *FilteredUno[T] {
	i := &FilteredUno[T]{it: it, filter: filter}
	i.advance()
	return i
}
func (m *FilteredUno[T]) advance() {
	if m.err != nil {
		return
	}
	m.hasNext = false
...
func (m *FilteredUno[T]) HasNext() bool { return m.err != nil || m.hasNext }
func (m *FilteredUno[T]) Next() (k T, err error) {
	k, err = m.nextK, m.err
	m.advance()
	return k, err
}
func (m *FilteredUno[T]) Close() {
	if x, ok := m.it.(Closer); ok {
		x.Close()
	}
}
