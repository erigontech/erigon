...
func TestRemoteKvRange(t *testing.T) {
	...
	require.NoError(writeDB.Update(ctx, func(tx kv.RwTx) error {
		wc, err := tx.RwCursorDupSort(kv.AccountChangeSetDeprecated)
		require.NoError(err)
	...
	require.NoError(db.View(ctx, func(tx kv.Tx) error {
		c, err := tx.Cursor(kv.AccountChangeSetDeprecated)
		require.NoError(err)
		...
		cnt := 0
		require.NoError(tx.ForEach(kv.AccountChangeSetDeprecated, nil, func(_, _ []byte) error {
			if cnt == 0 {
				k, v, err = c.Next()
				require.NoError(err)
				require.Equal([]byte{1}, k)
		...
		require.NoError(writeDB.Update(ctx, func(tx kv.RwTx) error {
			require.NoError(tx.Put(kv.AccountChangeSetDeprecated, []byte{4}, []byte{1}))
			return nil
		}))
	...
	err = db.View(ctx, func(tx kv.Tx) error {
		cntRange := func(from, to []byte) (i int) {
			it, err := tx.Range(kv.AccountChangeSetDeprecated, from, to, order.Asc, kv.Unlim)
			require.NoError(err)
			for it.HasNext() {
				_, _, err = it.Next()
				require.NoError(err)
	...
	err = db.View(ctx, func(tx kv.Tx) error {
		cntRange := func(from, to []byte) (i int) {
			it, err := tx.Range(kv.AccountChangeSetDeprecated, from, to, order.Asc, 2)
			require.NoError(err)
			for it.HasNext() {
				_, _, err := it.Next()
				require.NoError(err)
	...
	err = db.View(ctx, func(tx kv.Tx) error {
		cntRange := func(from, to []byte) (i int) {
			it, err := tx.Range(kv.AccountChangeSetDeprecated, from, to, order.Desc, 2)
			require.NoError(err)
			for it.HasNext() {
				_, _, err := it.Next()
				require.NoError(err)
