...
5. **Reconfiguration and Restart**:
   * Restart Erigon.
   * Crucially, reconfigure your **validator keys manager** to point directly to the Erigon Beacon API, as Erigon/Caplin now handles both layers internally). See also [here](fundamentals/staking/caplin.md).
...
4. **Synchronization**: Start syncing Erigon. Monitor the sync status using the `eth_syncing` JSON-[RPC method](fundamentals/interacting-with-erigon/) or a health check.
