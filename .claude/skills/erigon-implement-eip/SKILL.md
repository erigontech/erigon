...
These protocol tests are defined by JSON test fixture files inside release tarballs (e.g. `fixtures_develop.tar.gz`, `fixtures_bal.tar.gz`) pinned by `test-fixtures.json` and lazy-downloaded by `make test-fixtures` (which `make test-all` depends on) into `test-fixtures-cache/`. The migrated tests stream fixtures directly from those tarballs via `testutil.WalkTar`.
