...
func TestInsertIncorrectStateRoot(t *testing.T) {
	// Configure and generate a sample block chain
	var (
		db       = ethdb.NewMemDatabase()
		key, _   = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
	...
	incorrectBlock := types.NewBlockX(incorrectHeader, blocks[0].Transactions(), blocks[0].Uncles())
	...
	if _, err := blockchain.InsertChain(types.Blocks{blocksNew[0]}); err == nil {
		t.Fatal("should fail", blocksNew[0].Root() == blocks[0].Root())
	} else {
		fmt.Println("expected error", err)
	}
	...
	if balance := st.GetBalance(theAddr); balance.Cmp(big.NewInt(2000)) != 0 {
