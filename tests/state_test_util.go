...
func (t *StateTest) RunNoVerify(tx kv.RwTx, subtest StateSubtest, vmconfig vm.Config) (*state.IntraBlockState, libcommon.Hash, error) {
	...
	_, err = MakePreState(&chain.Rules{}, tx, t.json.Pre, readBlockNr, etconfig2.EnableHistoryV4InTest)
	if err != nil {
		return nil, libcommon.Hash{}, UnsupportedForkError{subtest.Fork}
	}
	...
	r = rpchelper.NewLatestStateReader(tx, etconfig2.EnableHistoryV4InTest)
	w = rpchelper.NewLatestStateWriter(txc, writeBlockNr, etconfig2.EnableHistoryV4InTest)
	statedb := state.New(r)
