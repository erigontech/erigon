...
func (tt *TransactionTest) Run(chainID *big.Int) error {
	validateTx := func(rlpData hexutil.Bytes, signer types.Signer, rules *params.Rules) (*common.Address, *common.Hash, uint64, error) {
		...
		// Intrinsic gas
		requiredGas, err := core.IntrinsicGas(msg.Data(), msg.AccessList(), 0, msg.To() == nil, rules.IsHomestead, rules.IsIstanbul, rules.IsShanghai)
		if err != nil {
			return nil, nil, 0, err
		}
	...
	for _, testcase := range []struct {
		...
		sender, txhash, intrinsicGas, err := validateTx(tt.RLP, *testcase.signer, testcase.config.Rules(0, new(big.Int)))
