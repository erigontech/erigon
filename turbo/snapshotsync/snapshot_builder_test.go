...
func TestState(t *testing.T) {
	db:=kv.MustOpen(filepath.Join(t.TempDir(), "chaindata"))
	db = kv.NewSnapshotKV().DB(db).Open()
	defer db.Close()

	for i:=uint8(1); i<20; i++ {
		if i%5==0 {
			snapshotDB:=kv.NewMDBX().InMem().WithBucketsConfig(func(defaultBuckets dbutils.BucketsCfg) dbutils.BucketsCfg {
				return BucketConfigs[SnapshotType_state]
			}).MustOpen()
			sntx,err:=snapshotDB.BeginRw(context.Background())
			if err!=nil {
			    t.Fatal(err)
			}
			err = db.View(context.Background(), func(tx ethdb.Tx) error {
				return tx.ForEach(dbutils.PlainStateBucket, []byte{}, func(k, v []byte) error {
					return sntx.Append(dbutils.PlainStateBucket, common.CopyBytes(k), common.CopyBytes(v))
				})
			})
			if err!=nil {
			    t.Fatal(err)
			...
			err = snapshotDB.View(context.Background(), func(tx ethdb.Tx) error {
				return db.(kv.WriteDB).WriteDB().Update(context.Background(), func(writeTX ethdb.RwTx) error {
					return tx.ForEach(dbutils.PlainStateBucket, []byte{}, func(k, v []byte) error {
						return writeTX.Delete(dbutils.PlainStateBucket, k, nil)
					})
				})
			})
			if err!=nil {
			...
			c:=make(chan struct{})
			db.(kv.SnapshotUpdater).UpdateSnapshots("state",snapshotDB, c)
			<-c
		...
		err := db.Update(context.Background(), func(tx ethdb.RwTx) error {
			return tx.Put(dbutils.PlainStateBucket, dbutils.PlainGenerateCompositeStorageKey(common.Address{i}.Bytes(),1, common.Hash{}.Bytes()), []byte{i,i})
		})
		if err!=nil {
			t.Fatal(err)
		}
	...
	fmt.Println("writeDB")
	err = db.(kv.WriteDB).WriteDB().View(context.Background(), func(tx ethdb.Tx) error {
		return tx.ForEach(dbutils.PlainStateBucket, []byte{}, func(k, v []byte) error {
			fmt.Println(k, v)
			return nil
		})
	...
	err = db.(kv.SnapshotUpdater).StateSnapshot().View(context.Background(), func(tx ethdb.Tx) error {
		return tx.ForEach(dbutils.PlainStateBucket, []byte{}, func(k, v []byte) error {
			fmt.Println(k, v)
			return nil
		})
