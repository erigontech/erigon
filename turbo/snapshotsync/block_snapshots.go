...
func DumpTxs(ctx context.Context, db kv.RoDB, segmentFile, tmpDir string, blockFrom, blockTo uint64, workers int, lvl log.Lvl) (firstTxID uint64, err error) {
	...
	f, err := compress.NewCompressorGSA(ctx, "Transactions", segmentFile, tmpDir, compress.MinPatternScore, workers)
	if err != nil {
...
func DumpHeaders(ctx context.Context, db kv.RoDB, segmentFilePath, tmpDir string, blockFrom, blockTo uint64, workers int, lvl log.Lvl) error {
	...
	f, err := compress.NewCompressorGSA(ctx, "Headers", segmentFilePath, tmpDir, compress.MinPatternScore, workers)
	if err != nil {
		return err
	}
	defer f.Close()
...
func DumpBodies(ctx context.Context, db kv.RoDB, segmentFilePath, tmpDir string, blockFrom, blockTo uint64, workers int, lvl log.Lvl) error {
	...
	f, err := compress.NewCompressorGSA(ctx, "Bodies", segmentFilePath, tmpDir, compress.MinPatternScore, workers)
	if err != nil {
		return err
	}
	defer f.Close()
...
func (m *Merger) merge(ctx context.Context, toMerge []string, targetFile string, logEvery *time.Ticker) error {
	f, err := compress.NewCompressorGSA(ctx, "merge", targetFile, m.tmpDir, compress.MinPatternScore, m.workers)
	if err != nil {
		return err
	}
	defer f.Close()
...
func cpSegmentByWords(ctx context.Context, srcF, dstF, tmpDir string) error {
	...
	defer d.Close()
	out, err := compress.NewCompressorGSA(ctx, "", dstF, tmpDir, compress.MinPatternScore, workers)
	if err != nil {
		return err
	}
	defer out.Close()
