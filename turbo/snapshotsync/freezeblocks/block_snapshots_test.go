...
func TestMergeSnapshots(t *testing.T) {
	...
	expectedFileName := snaptype.SegmentFileName(1, 500_000, 1_000_000, snaptype.Transactions)
	...
	expectedFileName = snaptype.SegmentFileName(1, 1_800_000, 1_900_000, snaptype.Transactions)
	d, err = compress.NewDecompressor(filepath.Join(dir, expectedFileName))
	require.NoError(err)
	defer d.Close()
	a = d.Count()
...
func TestOpenAllSnapshot(t *testing.T) {
	logger := log.New()
	dir, require := t.TempDir(), require.New(t)
	chainSnapshotCfg := snapcfg.KnownCfg(networkname.MainnetChainName, nil, nil, 0)
