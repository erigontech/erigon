...
func (br *BlockRetire) RetireBlocks(ctx context.Context, requestedMinBlockNum uint64, requestedMaxBlockNum uint64, lvl log.Lvl, seedNewSnapshots func(downloadRequest []snapshotsync.DownloadRequest) error, onDeleteSnapshots func(l []string) error, onFinish func() error) error {
	...
	if includeBor {
		...
		for {
			...
			log.Warn("[dbg] ret1", "br.blockReader.FrozenBlocks()", br.blockReader.FrozenBlocks(), "requestedMinBlockNum", requestedMinBlockNum)
			log.Warn("[dbg] ret2", "requestedMinBlockNum", requestedMinBlockNum, "minBlockNum", minBlockNum)
	...
	for {
		...
		log.Warn("[dbg] ret3", "minBlockNum", minBlockNum, "maxBlockNum", maxBlockNum)
