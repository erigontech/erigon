...
func (sn *HeaderSegment) reopenSeg(dir string) (err error) {
	sn.closeSeg()
	fileName := snaptype.SegmentFileName(sn.from, sn.to, snaptype.Headers)
...
func (sn *HeaderSegment) reopenIdx(dir string) (err error) {
	...
	fileName := snaptype.IdxFileName(sn.from, sn.to, snaptype.Headers.String())
...
func (sn *BodySegment) reopenSeg(dir string) (err error) {
	sn.closeSeg()
	fileName := snaptype.SegmentFileName(sn.from, sn.to, snaptype.Bodies)
	sn.seg, err = compress.NewDecompressor(path.Join(dir, fileName))
	if err != nil {
		return fmt.Errorf("%w, fileName: %s", err, fileName)
	}
...
func (sn *BodySegment) reopenIdx(dir string) (err error) {
	...
	fileName := snaptype.IdxFileName(sn.from, sn.to, snaptype.Bodies.String())
...
func (sn *TxnSegment) reopenSeg(dir string) (err error) {
	sn.closeSeg()
	fileName := snaptype.SegmentFileName(sn.from, sn.to, snaptype.Transactions)
	sn.Seg, err = compress.NewDecompressor(path.Join(dir, fileName))
	if err != nil {
		return fmt.Errorf("%w, fileName: %s", err, fileName)
	}
...
func (sn *TxnSegment) reopenIdx(dir string) (err error) {
	...
	fileName := snaptype.IdxFileName(sn.from, sn.to, snaptype.Transactions.String())
	...
	*/
	...
	fileName = snaptype.IdxFileName(sn.from, sn.to, snaptype.Transactions2Block.String())
...
Loop:
	for _, fName := range fileNames {
		...
		case snaptype.Headers:
			...
			if !exists {
				sn = &HeaderSegment{Range: Range{f.From, f.To}}
			}
		...
		case snaptype.Bodies:
			...
			if !exists {
				sn = &BodySegment{Range: Range{f.From, f.To}}
			}
		...
		case snaptype.Transactions:
			...
			if !exists {
				sn = &TxnSegment{Range: Range{f.From, f.To}}
			}
...
func (s *RoSnapshots) ReopenSegments(types []snaptype.Type) error {
	files, _, err := segments(s.dir, func(dir string, in []snaptype.FileInfo) (res []snaptype.FileInfo) {
...
func Segments(dir string) (res []snaptype.FileInfo, missingSnapshots []Range, err error) {
	return segments(dir, allTypeOfSegmentsMustExist)
}

func segments(dir string, segmentsTypeCheck func(dir string, in []snaptype.FileInfo) []snaptype.FileInfo) (res []snaptype.FileInfo, missingSnapshots []Range, err error) {
	list, err := snaptype.Segments(dir)
	if err != nil {
		return nil, missingSnapshots, err
	}
	...
	}
	{
		...
		l, _ = noGaps(noOverlaps(segmentsTypeCheck(dir, l)))
		res = append(res, l...)
	}
	{
		var l []snaptype.FileInfo
