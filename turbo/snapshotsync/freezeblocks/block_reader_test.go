...
func TestBlockReaderLastFrozenSpanIdWhenSegmentFilesArePresent(t *testing.T) {
	...
	createTestSegmentFile(t, 0, 500_000, heimdall.Enums.Spans, dir, snaptype.V1_0, logger)
...
func TestBlockReaderLastFrozenSpanIdReturnsLastSegWithIdx(t *testing.T) {
	...
	createTestSegmentFile(t, 0, 500_000, heimdall.Enums.Spans, dir, snaptype.V1_0, logger)
	createTestSegmentFile(t, 500_000, 1_000_000, heimdall.Enums.Spans, dir, snaptype.V1_0, logger)
	createTestSegmentFile(t, 1_000_000, 1_500_000, heimdall.Enums.Spans, dir, snaptype.V1_0, logger)
	// delete idx file for last bor span segment to simulate segment with missing idx file
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 1_000_000, 1_500_000, heimdall.Spans.Name()))
...
func TestBlockReaderLastFrozenSpanIdReturnsZeroWhenAllSegmentsDoNotHaveIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 1, 500_000, heimdall.Spans.Name()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 500_000, 1_000_000, heimdall.Spans.Name()))
	err = os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 1_000_000, 1_500_000, heimdall.Spans.Name()))
	err = os.Remove(idxFileToDelete)
...
func TestBlockReaderLastFrozenEventIdReturnsLastSegWithIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 1_000_000, 1_500_000, heimdall.Events.Name()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	borRoSnapshots := heimdall.NewRoSnapshots(ethconfig.BlocksFreezing{ChainName: networkname.BorMainnet}, dir, 0, logger)
	defer borRoSnapshots.Close()
...
func TestBlockReaderLastFrozenEventIdReturnsZeroWhenAllSegmentsDoNotHaveIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 0, 500_000, heimdall.Events.Name()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 500_000, 1_000_000, heimdall.Events.Name()))
	err = os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, 1_000_000, 1_500_000, heimdall.Events.Name()))
	err = os.Remove(idxFileToDelete)
...
func createTestBorEventSegmentFile(t *testing.T, from, to, eventId uint64, dir string, logger log.Logger) {
	...
	compressor, err := seg.NewCompressor(
		context.Background(),
		"test",
		filepath.Join(dir, snaptype.SegmentFileName(snaptype.V1_0, from, to, heimdall.Enums.Events)),
		dir,
		compressCfg,
		log.LvlDebug,
		logger,
	...
	idx, err := recsplit.NewRecSplit(
		recsplit.RecSplitArgs{
			...
			BucketSize: 10,
			TmpDir:     dir,
			IndexFile:  filepath.Join(dir, snaptype.IdxFileName(snaptype.V1_0, from, to, heimdall.Events.Name())),
			LeafSize:   8,
		},
		logger,
	)
