...
func TestBlockReaderLastFrozenSpanIdReturnsLastSegWithIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(1, 1_000_000, 1_500_000, bortype.BorSpans.String()))
...
func TestBlockReaderLastFrozenSpanIdReturnsZeroWhenAllSegmentsDoNotHaveIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(1, 1, 500_000, bortype.BorSpans.String()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(1, 500_000, 1_000_000, bortype.BorSpans.String()))
	err = os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(1, 1_000_000, 1_500_000, bortype.BorSpans.String()))
	err = os.Remove(idxFileToDelete)
...
func TestBlockReaderLastFrozenEventIdReturnsLastSegWithIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(1, 1_000_000, 1_500_000, bortype.BorEvents.String()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	borRoSnapshots := NewBorRoSnapshots(ethconfig.BlocksFreezing{Enabled: true}, dir, 0, logger)
	defer borRoSnapshots.Close()
...
func TestBlockReaderLastFrozenEventIdReturnsZeroWhenAllSegmentsDoNotHaveIdx(t *testing.T) {
	...
	idxFileToDelete := filepath.Join(dir, snaptype.IdxFileName(1, 0, 500_000, bortype.BorEvents.String()))
	err := os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(1, 500_000, 1_000_000, bortype.BorEvents.String()))
	err = os.Remove(idxFileToDelete)
	require.NoError(t, err)
	idxFileToDelete = filepath.Join(dir, snaptype.IdxFileName(1, 1_000_000, 1_500_000, bortype.BorEvents.String()))
	err = os.Remove(idxFileToDelete)
...
func createTestBorEventSegmentFile(t *testing.T, from, to, eventId uint64, dir string, logger log.Logger) {
	...
	idx, err := recsplit.NewRecSplit(
		recsplit.RecSplitArgs{
			...
			BucketSize: 10,
			TmpDir:     dir,
			IndexFile:  filepath.Join(dir, snaptype.IdxFileName(1, from, to, bortype.BorEvents.String())),
			LeafSize:   8,
		},
		logger,
	)
