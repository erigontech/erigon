...
func TestParseCompressedFileName(t *testing.T) {
	...
	require.Error(err)
	_, _, _, err = ParseCompressedFileName("1-2-bodies-v2.seg")
	require.Error(err)
	_, _, _, err = ParseCompressedFileName("1-2-bodies-v0.seg")
	require.Error(err)

	from, to, tt, err := ParseCompressedFileName("1-2-bodies-v1.seg")
	require.NoError(err)
