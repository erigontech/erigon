...
func squeezeStorage(ctx context.Context, dirs datadir.Dirs, logger log.Logger) error {
	...
	aggOld, err := state.NewAggregator(ctx, dirsOld, config3.DefaultStepSize, db, logger, false)
	if err != nil {
		panic(err)
	}
	defer aggOld.Close()
...
func squeezeCode(ctx context.Context, dirs datadir.Dirs, logger log.Logger) error {
	db := dbCfg(kv.ChainDB, dirs.Chaindata).MustOpen()
	defer db.Close()
	agg, err := state.NewAggregator(ctx, dirs, config3.DefaultStepSize, db, logger, false)
	if err != nil {
		return err
	}
	defer agg.Close()
