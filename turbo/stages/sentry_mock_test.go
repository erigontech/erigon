...
func TestHeaderStep(t *testing.T) {
	...
	initialCycle := true
	if _, err := stages.StageLoopStep(m.Ctx, m.DB, m.Sync, initialCycle, m.Log, nil); err != nil {
		t.Fatal(err)
	}
}
...
func TestMineBlockWith1Tx(t *testing.T) {
	...
	{ // Do 1 step to start txPool
		...
		initialCycle := true
		if _, err := stages.StageLoopStep(m.Ctx, m.DB, m.Sync, initialCycle, log.New(), nil); err != nil {
			t.Fatal(err)
		}
	}
...
func TestAnchorReplace2(t *testing.T) {
	...
	if _, err := stages.StageLoopStep(m.Ctx, m.DB, m.Sync, initialCycle, m.Log, hook); err != nil {
		t.Fatal(err)
	}
}
