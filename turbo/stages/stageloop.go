...
func StageLoop(ctx context.Context, db ethdb.Database, hd *headerdownload.HeaderDownload, bd *bodydownload.BodyDownload, bodyReqSend func(context.Context, *bodydownload.BodyRequest) bool, wakeUpChan chan struct{}, timeout int) error {
	...
	for {
		if len(files) > 0 || len(buffer) > 0 {
			...
			if err := bodydownload.Forward("2/14 Bodies", ctx, db, bd, bodyReqSend, wakeUpChan, timeout); err != nil {
