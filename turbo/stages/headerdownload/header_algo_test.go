...
func TestInserter1(t *testing.T) {
	...
	if _, err = hi.FeedHeaderPoW(tx, snapshotsync.NewBlockReader(), &h1, data1, h1Hash, 1, nil); err != nil {
		t.Errorf("feed empty header 1: %v", err)
	}
	data2, _ := rlp.EncodeToBytes(&h2)
	if _, err = hi.FeedHeaderPoW(tx, snapshotsync.NewBlockReader(), &h2, data2, h2Hash, 2, nil); err != nil {
