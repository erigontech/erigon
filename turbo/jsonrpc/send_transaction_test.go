...
func TestCheckDynamicTxFee(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Gas cap less than block gas limit",
			gasCap:        uint256.NewInt(limit / 2),
			blockGasLimit: 1000000,
			expectError:   false,
		},
		{
			name:          "Gas cap equal to block gas limit",
			gasCap:        uint256.NewInt(limit),
			blockGasLimit: 1000000,
			expectError:   false,
		},
		{
			name:          "Gas cap greater than block gas limit",
			gasCap:        uint256.NewInt(limit * 2),
			blockGasLimit: 1000000,
			expectError:   true,
		},
		...
		t.Run(tt.name, func(t *testing.T) {
			txn, err := types.SignTx(
				types.NewEIP1559Transaction(
					...
					params.TxGas,
					tt.gasCap,
					uint256.NewInt(5*params.Wei), // Gas tip
					tt.gasCap,
					nil,
				),
			...
			if tt.expectError {
				require.Error(err)
				require.Equal("fee cap is bigger than the block gas limit", err.Error())
			} else {
				require.NoError(err)
			}
		})
