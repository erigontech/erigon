...
func (api *APIImpl) GetTransactionReceipt(ctx context.Context, txnHash common.Hash) (map[string]interface{}, error) {
	...
	if txNumMin+1 > txNum && !isStateSync { //TODO: what a magic is this "2" and how to avoid it
	...
	if isStateSync {
		...
		fmt.Printf("[dbg] here2 isStateSync=%t txNum=%d, txNumMin=%d, txsAmount=%d\n", isStateSync, txNum, txNumMin, len(block.Transactions()))
	...
	fmt.Printf("[dbg] here1 isStateSync=%t txNum=%d, txNumMin=%d, txnIndex=%d, blockNum=%d\n", isStateSync, txNum, txNumMin, txnIndex, blockNum)
