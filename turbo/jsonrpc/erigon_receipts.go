...
const batchGetTransactionsReceiptsLimit = 5000
...
func (api *ErigonImpl) BatchGetTransactionsReceipts(ctx context.Context, txHashes []common.Hash) ([]map[string]interface{}, error) {
	...
	for _, txHash := range txHashes {
		...
		if blockNumTxHashesMap[blockNum] == nil {
			blockNumTxHashesMap[blockNum] = make(map[common.Hash]struct{})
		}

		blockNumTxHashesMap[blockNum][txHash] = struct{}{}
	}

	if len(blockNumTxHashesMap) == 0 {
		return nil, nil
	}

	var result []map[string]interface{}

	for blockNum, txHashesMap := range blockNumTxHashesMap {
		...
		if block == nil {
			continue // not error, see https://github.com/ledgerwatch/erigon/issues/1645
		}
		...
		for idx, transaction := range block.Transactions() {
			...
			if _, ok := txHashesMap[txnHash]; ok {
				var borTx types.Transaction
				if transaction == nil && cc.Bor != nil {
					borTx = rawdb.ReadBorTransactionForBlock(tx, blockNum)
					if borTx == nil {
					...
					result = append(result, marshalReceipt(borReceipt, borTx, cc, block.HeaderNoCopy(), txnHash, false))

					continue
				}

				result = append(result, marshalReceipt(receipts[idx], block.Transactions()[idx], cc, block.HeaderNoCopy(), txnHash, true))
			}
		}
	}
