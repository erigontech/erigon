...
* __FULL HEADER__: contains the complete block header with all its information parameters
    - _key_ : 8-byte RLP-encoded block number + 32-byte block hash
    - _value_ : the RLP-encoded block header structured as defined in the [Ethereum Yellow Paper](https://github.com/ethereum/yellowpaper):
        ...
        * logs bloom: 64-byte Bloom filter composed from indexable information in each log entry from the transaction receipts
        ...
        * extra data: up to 32-byte array of additional relevant data for this block
...
* __TOTAL DIFFICULTY HEADER__: contains the total mining difficulty (TD) of the chain ending in such specific block
    - _key_ : 8-byte RLP-encoded block number + 32-byte block hash + `0x74` suffix (ASCII code for `t` character)
    - _value_ : RLP-encoded total difficulty value: the cumulative difficulty value from first block to this one
* __CANONICAL HEADER__: contains the block hash
    - key: 8-byte RLP-encoded block number + `0x6E` suffix (ASCII code for `n` character)
...
* block number 0 is encoded as 0x0000000000000000
