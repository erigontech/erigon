...
16. **How do I monitor my node's sync progress?** There are three complementary ways:
    ...
    * **Query the JSON-RPC** — call `eth_syncing` against the RPC daemon (default port `8545`):
      ...
      While syncing the result is an object with `currentBlock`, `highestBlock`, and a `stages` array (per-stage progress, matching the log prefix); once fully synced it is simply `false`. (`startingBlock` reports the block the current sync session started from.)
