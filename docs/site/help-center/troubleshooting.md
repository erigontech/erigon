...
If the node is wedged and you are ready to lose it, `kill -SIGQUIT <pid>` dumps every goroutine stack and terminates
that process in one step. Erigon installs a stack-dump handler for `SIGUSR1` and shutdown handlers for `SIGINT` and
`SIGTERM`, but none for `SIGQUIT` — so `SIGQUIT` falls through to the Go runtime's own behaviour, the same one this
repository's CI watchdog relies on. Pass an explicit PID rather than `$(pidof erigon)`: `SIGQUIT` is destructive, and on
a host running more than one instance `pidof` would kill all of them.
...
* **Cloud** — the firewall is stateful, so return traffic is handled for you. Outbound is allow-all, but only for as
  long as no outbound rule exists: per Hetzner's FAQ, *"If you define one or more outbound rules, the outbound direction
  also changes to implicit 'deny'"*. Adding outbound rules for the ports below would therefore cut off DNS, NTP and
  HTTPS — including the snapshot webseeds — and break the initial download. Define **inbound rules only** and leave the
  outbound list empty.
* **Dedicated servers (Robot)** — the firewall is stateless, so nothing is inferred from an outgoing connection. The
  listening ports in the table below are not enough on their own: replies to Erigon's own outbound connections — DNS,
  HTTPS to the snapshot webseeds, and outgoing peer dials — arrive on ephemeral local ports and need inbound rules of
  their own (TCP with `ack` set, plus the UDP ranges you use), and any outbound filtering needs matching reply rules.
  Follow [Hetzner's notes on stateless-firewall configuration](https://docs.hetzner.com/robot/dedicated-server/firewall/#notes-about-stateless-firewall-configuration)
  rather than porting a Cloud ruleset across.
...
This table is not exhaustive — it lists what a default node needs. See [Default ports](/fundamentals/default-ports) for
every port Erigon can open, including the opt-in Shutter port.

The Caplin ports are its defaults (`--caplin.discovery.port` and `--caplin.discovery.tcpport`); change the rules to
match if you override them. Note the second flag's help text calls it a DISCV5 port, which is a misnomer: DISCV5 is
UDP-only and runs on `4000`, while `4001` is the libp2p TCP listen address Caplin advertises for peering. If you run an
**external** consensus client instead of Caplin, open the ports that client uses rather than the Caplin ones.
Lighthouse, Teku and Nimbus default to `9000` TCP+UDP, and Lighthouse additionally uses `9001` UDP for QUIC; Prysm
defaults to `13000` TCP and `12000` UDP. Check your client's own documentation rather than assuming `9000`.

Without these, the node may appear to have peers (via the cloud dashboard) but will suffer poor block propagation.
Configure Cloud firewalls in the Hetzner Cloud Console under **Firewalls** or via `hcloud firewall`; for dedicated
servers the equivalent lives in the Robot panel under **Server → Firewall**.

A public-facing Erigon node should also not attempt to peer with IPv4 ranges that are reserved for special use. Blocking
them is worth doing explicitly on Hetzner, whose abuse and netscan detection flags outbound dials to reserved ranges —
that is what prompted this note, rather than their firewall filtering the traffic. Do not apply this to a node using
`--caplin.local-discovery`, which deliberately peers over private IPs. The authoritative list is
...
([RFC 6890](https://datatracker.ietf.org/doc/html/rfc6890)); the ranges below are that registry's entries most often
blocked for an Ethereum node — it is a reference list, not a ruleset to paste verbatim: `127.0.0.0/8` and
`255.255.255.255/32` are included for completeness but must **not** go into an outbound drop rule, for the reason given
after the list (`100.64.0.0/10` comes from [RFC 6598](https://datatracker.ietf.org/doc/html/rfc6598), and
`192.88.99.0/24` has since been deprecated by [RFC 7526](https://datatracker.ietf.org/doc/html/rfc7526) — blocking it
remains correct):
...
A near-identical list expressed
...
is available on Stack Exchange, but do not paste it in as a blanket `OUTPUT` policy:

* It omits `127.0.0.0/8`, which is correct — an `OUTPUT` drop on loopback would break Erigon's own inter-component
  traffic, such as the RPC Daemon reaching the core node on `127.0.0.1:9090`.
* It has no separate `255.255.255.255/32` rule, but that address is **not** left reachable: `240.0.0.0/4` spans
  `240.0.0.0`–`255.255.255.255` and already covers it.
* On **Hetzner Cloud** it drops the metadata endpoint at `169.254.169.254`, which sits inside `169.254.0.0/16`, along
  with legitimate private-network and multicast traffic. Exempt what your instance needs, or scope the rules to Erigon's
  peer dialing instead of the whole `OUTPUT` chain.
