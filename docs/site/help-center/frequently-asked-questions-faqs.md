...
5. **How much disk space is required?** It depends on the network and the pruning mode, and it grows as the chain grows — an archive node needs several times what a full node does. Rather than repeat figures that go stale, see [Hardware Requirements](/get-started/hardware-requirements), which carries a measured figure and a measurement date for every network and pruning mode.
