...
5. **How much disk space is required?** It depends on the network and the pruning mode, and it grows as the chain grows. See [Hardware Requirements](/get-started/hardware-requirements) for a measured figure and measurement date, for the networks and pruning modes it measures.
