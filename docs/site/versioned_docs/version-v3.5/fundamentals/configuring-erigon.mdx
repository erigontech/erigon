...
* `--chain value`: Sets the name of the [network](/v3.5/fundamentals/supported-networks) to join.
  * Default: `mainnet`
...
These flags control database performance and memory usage. See [Database](/v3.5/fundamentals/database) for the on-disk layout.
...
Flags for managing how old chain data is handled and stored. See [Snapshots Management](/v3.5/fundamentals/snapshots-management) for snapshot tooling and disk usage.

* `--prune.mode value`: Selects a pruning preset (`full`, `archive`, `minimal`, `blocks`). See also [Pruning Modes](/v3.5/fundamentals/pruning-modes)
  * Default: `"full"`
...
* `--prune.distance.blocks value`: Keeps block history for the latest `N` blocks.
  * Default: `0`
* `--prune.include-commitment-history, --prune.experimental.include-commitment-history, --experimental.commitment-history`: Enables fast `eth_getProof` for executed blocks by storing commitment history. Requires +32 GB RAM. See [`eth_getProof`](/v3.5/interacting-with-erigon/eth#eth_getproof).
  * Default: `false`
...
Options for configuring the [transaction pool](/v3.5/fundamentals/modules/txpool).
...
These flags manage network connectivity, peer discovery, and traffic control. See [Default Ports](/v3.5/fundamentals/default-ports) for the full port map.

* `--port value`: The main network listening port.
  * Default: `30303`
* `--p2p.protocol value`: The version of the `eth` P2P protocol.
  * Default: `69`, `70`, `71`
* `--nat value`: The NAT port mapping mechanism (See [here](/v3.5/fundamentals/nat) for more details).
...
Flags for configuring various RPC servers and their behavior. See [Interacting with Erigon](/v3.5/interacting-with-erigon) for the JSON-RPC API reference.
...
* `--authrpc.port value`: The HTTP-RPC server listening port for the Engine API.
  * Default: `8551`
* `--authrpc.jwtsecret value`: The path to the [JWT secret](/v3.5/fundamentals/jwt) file for the consensus layer.
...
Flags for configuring the Model Context Protocol (MCP) server. The embedded [MCP server](/v3.5/fundamentals/mcp) is **enabled by default** on
...
Flags for controlling logging and performance profiling. See [Logs](/v3.5/fundamentals/logs) for log files, levels, and rotation.
...
* `--metrics`: Enables metrics collection and reporting. See [Creating a Dashboard](/v3.5/fundamentals/creating-a-dashboard).
  * Default: `false`
...
Flags for configuring the [Sentry](/v3.5/fundamentals/modules/sentry) component.
...
These flags control the block synchronization and data downloading process, including the BitTorrent protocol settings. See [Downloader](/v3.5/fundamentals/modules/downloader) for how snapshot downloading works.
...
Flags for configuring the [Caplin](/v3.5/fundamentals/caplin) consensus layer.
...
Flags for configuring the [Shutter Network](/v3.5/staking/shutter-network) encrypted transactions mempool.
...
When running Erigon in [Docker](/v3.5/fundamentals/docker-compose), you can configure user permissions and data directories:
