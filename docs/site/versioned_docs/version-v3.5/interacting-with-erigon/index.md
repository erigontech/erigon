...
The Erigon RPC Service, managed by Erigon's modular [RPC Daemon](/v3.5/fundamentals/modules/rpc-daemon), supports various API namespaces, which can be enabled or disabled using the `--http.api` flag. The available namespaces include:

* [`eth`](/v3.5/interacting-with-erigon/eth): Standard Ethereum API.
* [`erigon`](/v3.5/interacting-with-erigon/erigon): Erigon-specific extensions.
* [`engine`](/v3.5/interacting-with-erigon/engine): The JSON-RPC Interface for Execution and Consensus Layer Communication.
* [`web3`](/v3.5/interacting-with-erigon/web3): Web3 compatibility API.
* [`net`](/v3.5/interacting-with-erigon/net): Network information API.
* [`debug`](/v3.5/interacting-with-erigon/debug): Debugging and tracing API.
* [`trace`](/v3.5/interacting-with-erigon/trace): Transaction tracing API.
* [`txpool`](/v3.5/interacting-with-erigon/txpool): Transaction pool API.
* [`admin`](/v3.5/interacting-with-erigon/admin): Node administration API
* [`bor`](/v3.5/interacting-with-erigon/bor): Polygon Bor-specific API (when running on Polygon)
* [`ots`](/v3.5/interacting-with-erigon/ots): These methods are specifically tailored for use with Otterscan, an open-source, fast block explorer.
* [`internal`](/v3.5/interacting-with-erigon/internal): Erigon specific API for development and debugging purposes.
* [`gRPC`](/v3.5/interacting-with-erigon/grpc): API for lower-level data access.
* [`overlay`](/v3.5/interacting-with-erigon/overlay): Erigon-specific overlay API for replaying blocks with state overrides (archive nodes only).
* [`parity`](/v3.5/interacting-with-erigon/parity): Partial OpenEthereum compatibility (`parity_listStorageKeys`).
* [`graphql`](/v3.5/interacting-with-erigon/graphql): GraphQL endpoint following EIP-1767.
