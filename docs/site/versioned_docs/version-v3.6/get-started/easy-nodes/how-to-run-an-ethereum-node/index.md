...
1. Confirm your machine meets the necessary [Hardware Requirements](/v3.6/get-started/hardware-requirements) based on your desired pruning mode.
2. **Install Docker**:
...
* Add `--prune.mode=minimal` to run minimal [Pruning Mode](/v3.6/fundamentals/pruning-modes) or `--prune.mode=archive` to run an archive node
...
* `--http.addr=0.0.0.0 --http.api=eth,web3,net,debug,trace,txpool` to use RPC and e.g. be able to connect your [web3 wallet](/v3.6/fundamentals/web3-wallet). `0.0.0.0` is the address *inside* the container, which is what lets Docker forward to it at all; the `127.0.0.1:8545:8545` port mapping is what keeps it off your LAN. Change that mapping to `8545:8545` only if you intend to expose RPC to other machines
...
When you get familiar with running Erigon from CLI you may also consider [staking](/v3.6/staking/caplin) and/or running an Ethereum node with an [external Consensus Layer](/v3.6/get-started/easy-nodes/how-to-run-an-ethereum-node/ethereum-with-an-external-cl).
...
Additional flags can be added to [configure](/v3.6/fundamentals/configuring-erigon/) Erigon with several options.
