...
* `--chain=hoodi` specifies which [network](/v3.6/fundamentals/supported-networks) to sync
* `--prune.mode=minimal` tells Erigon to use minimal [Pruning Mode](/v3.6/fundamentals/pruning-modes)
...
Running Erigon as a [dedicated system user](/v3.6/fundamentals/security) — for example under `systemd` — is easiest when the binaries live outside your source checkout, where that user can reach them. Pass a destination to `make install`:
...
Once you have Erigon installed, you can see [Basic Usage](/v3.6/fundamentals/basic-usage) to configure your node and confirm it is syncing.
