...
* `--prune.mode value`: Selects a pruning preset (`full`, `archive`, `minimal`, `blocks`). See also [Prune Modes](/v3.4/fundamentals/prune-modes)
  * Default: `"full"`
...
* `--nat value`: The NAT port mapping mechanism (See [here](/v3.4/fundamentals/configuring-erigon/nat) for more details).
