...
Two things to know about their scope. They cover the **current** documentation and the help center only — archived versions, published under `/vX.Y/`, are not included. And the text is cleaned rather than copied verbatim: MDX components are stripped, and section index pages built as card grids are rendered as the list of links they present followed by the rest of their prose. Read the corpus as the documentation's prose, not as a byte-for-byte mirror of the rendered site.
