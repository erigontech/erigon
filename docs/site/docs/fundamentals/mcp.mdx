...
Two things to know about their scope. They cover the **current** documentation and the help center only — archived versions, published under `/vX.Y/`, are not included. And the text is cleaned rather than copied verbatim: MDX components are stripped, and on section index pages built as card grids each grid becomes the list of links it presents, with the page's prose kept around it in document order. Read the corpus as the documentation's prose, not as a byte-for-byte mirror of the rendered site.
