...
This mode serves two HTTP transports on the same port — **streamable HTTP** (the current MCP standard) at `/mcp` and **SSE (Server-Sent Events)** at `/sse` for older clients — and is the simplest option when you are already running a full Erigon node.

### Standalone (`mcp` binary)

A separate `mcp` binary that connects to an existing Erigon node either via its JSON-RPC endpoint or directly via the MDBX database. Supports both **stdio** (for Claude Desktop) and **HTTP** (streamable HTTP + SSE) transports.
...
| `--transport` | `stdio` | Transport type: `stdio` or `http` (`sse` is a deprecated alias of `http`) |
| `--sse.addr` | `127.0.0.1:8553` | HTTP listen address (when `--transport http`) |
...
<TabItem value="embedded-(http)" label="Embedded (HTTP)">
```json
{
  "mcpServers": {
    "erigon": {
      "type": "http",
      "url": "http://127.0.0.1:8553/mcp"
    }
  }
}
```
...
**Embedded HTTP mode (recommended — zero extra binaries):**

```bash
claude mcp add --transport http erigon http://127.0.0.1:8553/mcp
```

Older Claude Code versions without streamable HTTP support can use the SSE endpoint instead:
...
:::tip
The embedded HTTP mode (`http://127.0.0.1:8553/mcp`, or `/sse` for SSE-only clients) works out of the box as long as Erigon is running. No separate binary is needed.
:::
...
`txpool_status`, `txpool_content`, `txpool_contentFrom` — inspect pending/queued transactions, e.g. to find out why a transaction is stuck.

### Tracing (`debug_*`, `trace_*`)

`debug_traceTransaction`, `debug_traceBlockByNumber`, `debug_traceCall` (default tracer: `callTracer`; selectable per call), `debug_getModifiedAccountsByNumber`, plus Parity-style `trace_transaction`, `trace_block` and `trace_filter`.

### P2P (`net_*`, `admin_*`)

`net_version`, `net_listening`, `net_peerCount`, `admin_nodeInfo`, `admin_peers` — read-only connectivity diagnostics; peer-management methods are not exposed.
...
| [llms-full.txt](https://docs.erigon.tech/llms-full.txt) | The text of every documentation page, as one plain-text file | You want the whole corpus in context, offline, with nothing scraped |
...
The corpus is cleaned rather than copied verbatim: MDX components are stripped, and section index pages built as card grids are reduced to the list of links they present. Read it as the documentation's prose, not as a byte-for-byte mirror of the rendered site.
