...
* `--exec.serial`: Force serial execution by clamping the parallel executor to a single worker. Wins over `--exec.workers` and `EXEC3_WORKERS` — use to disable parallelism for diagnostics or like-for-like baseline comparisons.
  * Default: `false`
* `--exec.no-prune`: Disable all DB pruning: state-aggregator (Domain/InvertedIndex) plus stage-level pruning (Execution: ChangeSets3/BlockAccessList; TxLookup; WitnessProcessing; Snapshots: PruneAncientBlocks/canonical markers/retirement). Equivalent to `NO_PRUNE=true`. Diagnostic / perf-comparison use only.
  * Default: `false`
...
* `STATE_CACHE_ACCOUNTS` - Byte budget for the account cache (default: `150MB`). **Default changed in v3.6** — was `1GB` in v3.5.
* `STATE_CACHE_STORAGE` - Byte budget for the storage cache (default: `1GB`)
* `STATE_CACHE_CODE` - Byte budget for the code cache, applied separately to each of its two content-addressed layers (default: `512MB`)
* `STATE_CACHE_CODE_INDEX` - Byte budget for the address-to-code index, shared by its two address-keyed LRUs (default: `32MB`, floor 1024 entries each). **Default changed in v3.6** — was `16MB` in v3.5.
...
The four `STATE_CACHE_*` budgets are new in v3.6. Values parse as human-readable sizes (`150MB`, `1GB`), and an unparseable value aborts startup.
...
The budgets are also not reservations. The account, storage and code caches start small and grow in steps, funding each step from one process-wide envelope sized at 1/32 of the memory available to Erigon (system RAM, cgroup limit or `GOMEMLIMIT`, whichever is lowest) and shared with other in-process caches such as the commitment branch cache. A step that would overflow the envelope is refused, and that cache evicts within its current size until envelope space frees up.

What that means for sizing: each cache starts at 1024 entries, or at its own ceiling if that is smaller — a `STATE_CACHE_CODE` below about 12 MiB starts and stays under 1024 entries — and reserves envelope space only as it fills. Twice `STATE_CACHE_CODE` is therefore the largest reservation the two byte-bearing layers can take, not one they hold from the start. Budgets summing past the envelope therefore mean it *can* saturate, not that it will — that depends on the working set. On a 32 GB host the whole envelope is 1 GB, below the sum of the defaults above, so a large enough working set saturates it; past that point raising those three budgets together adds no aggregate capacity and only changes which cache wins the space left. A much larger host may never saturate it, and there raising them does help. `STATE_CACHE_CODE_INDEX` sits outside the envelope altogether — its tables are sized up front from the budget, so above the 1024-entry floor noted above, raising it adds index capacity independently of how saturated the envelope is.
