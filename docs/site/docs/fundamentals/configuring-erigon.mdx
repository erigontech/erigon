...
* `COMMITMENT_AFTER_EXEC` - Makes execution wait for commitment calculation of block N before starting block N+1. Diagnostic setting that reduces execution/commitment overlap; default: `false`
