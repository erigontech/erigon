...
* `STATE_CACHE_CODE` - Byte budget for the code cache, applied to each of its two content-addressed layers, so worst-case code residency is twice this figure (default: `512MB`)
...
The four `STATE_CACHE_*` budgets are new in v3.6. Values parse as human-readable sizes (`150MB`, `1GB`), and an unparseable value aborts startup. Each is converted into an entry-count cap using a typical per-domain entry size, so resident memory tracks the figure approximately rather than exactly.

The budgets are ceilings, not reservations: the account, storage and code caches start small and grow in steps, funding each step from one process-wide envelope sized at 1/32 of the memory available to Erigon (system RAM, cgroup limit or `GOMEMLIMIT`, whichever is lowest). A step that would overflow the envelope is refused, and that cache evicts within its current size instead until envelope space frees up. That envelope is shared with other in-process caches, including the commitment branch cache, so raising every budget at once buys nothing: on a 32 GB host the whole envelope is 1 GB, less than the sum of the defaults above. `STATE_CACHE_CODE_INDEX` is the exception — its tables are sized up front from the budget and never draw on the envelope.
