...
* `--wit-protocol`: Enables the WIT protocol (`wit/0`) for stateless witness data exchange. Must be set explicitly. Accepts `--polygon.wit-protocol` as a deprecated alias.
  * Default: `false`
