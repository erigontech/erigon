...
* `--prune.include-receipts, --persist.receipts, --experiment.persist.receipts.v2`: Downloads and caches historical receipts. Without it, receipts and logs are re-derived on demand from state history, so the related RPC calls still work, just with higher latency.
  * Default: `true` (enabled) in every prune mode, on fresh datadirs
...
* `--db.writemap`: Enables `WRITE_MAP` for fast database writes.
  * Default: `true`
* `--db.read.concurrency value`: Maximum number of concurrent open database read transactions (the MDBX read-transaction semaphore); when it is full, extra readers wait for a slot by default, though some RPC paths (HTTP/WebSocket) fail fast with an overload response instead. Raise it for nodes serving heavy parallel RPC. Lowering it does not reduce read concurrency: each parallel-execution worker holds a long-lived read transaction, so a value below the worker count would deadlock. Erigon raises a lower value, with a warning, to a floor that sums the exec workers, the parallel-commitment readers (GOMAXPROCS + 1, with `--experimental.parallel-commitment`), the BAL warmup readers, the block read-ahead workers and a fixed reserve; rpcdaemon uses the value as given. To genuinely reduce read concurrency, lower `--exec.workers` instead.
...
**Commitment-trie construction.** The following flag changes how the commitment (state-root) trie is computed during execution.
...
* `--caplin.discovery.port value`: The port for the Caplin DISCV5 protocol.
  * Default: `4000`
* `--caplin.discovery.tcpport value`: The TCP port for Caplin libp2p.
	* Default: `4001`
* `--caplin.discovery.quicport value`: The QUIC UDP port for Caplin libp2p.
	* Default: `4001`
...
* `USE_STATE_CACHE` - Enables the executor's domain-shared read cache (default: `true`; flag equivalent: `--exec.state-cache`). When `false` no cache is built, and neither the budgets nor the pin controller below have any effect.
* `STATE_CACHE_ACCOUNTS` - Byte budget for the account cache (default: `150MB`). **New in v3.6**; the built-in budget it overrides was `1GB` in v3.5.
* `STATE_CACHE_STORAGE` - Byte budget for the storage cache (default: `1GB`)
* `STATE_CACHE_CODE` - Byte budget for the code cache, applied separately to each of its two byte-bearing layers; a third, size-only layer is sized independently (default: `512MB`)
* `STATE_CACHE_CODE_INDEX` - Byte budget for the address-to-code index, shared by its two address-keyed LRUs (default: `32MB`, floor 1024 entries each). **New in v3.6**; the built-in budget was `16MB` in v3.5.
* `DISABLE_ADAPTIVE_PIN` - Disables the adaptive trunk-pin controller, which watches per-contract miss pressure on the commitment branch cache and pins the hottest contracts' trunk branches in RAM to cut per-block I/O (default: `true`, so pinning is off; new in v3.6). Set to `false` to enable it — it targets workloads where a single contract dominates storage reads, and costs RAM on the commit path.

The four `STATE_CACHE_*` budgets are new in v3.6. Values parse as human-readable sizes (`150MB`, `1GB`); an unparseable value aborts startup, though only when the state cache is enabled — with `USE_STATE_CACHE=false` nothing reads them.

None of the four is an exact byte bound. Each is divided by a typical per-domain entry size to produce an entry-count cap, and it is that cap, not a running byte total, that evicts — so residency follows the entries actually stored. The code layers assume roughly 12 KiB per contract, and a working set of larger contracts will exceed the configured figure. `STATE_CACHE_CODE` applies to each of the two byte-bearing code layers, so their combined nominal ceiling is twice the value; a third, size-only layer answers `EXTCODESIZE`/`EXTCODEHASH` — `STATE_CACHE_CODE` does not size it, but it grows on demand from the same envelope, up to a ceiling of about 61 MiB. A contract inserted into both at once is stored once and referenced twice, but the layers are independent LRUs: once their eviction orders diverge, a refill into one while the other still holds a live entry leaves two backing arrays for the same contract, so heap use can approach the two-layer total. The account and storage caps are additionally clamped to between 1024 and 2^24 entries, which for their entry sizes means roughly 96 KiB–1.5 GiB for accounts and 88 KiB–1.375 GiB for storage; a value outside its own range makes no difference.

The budgets are not reserved up front. The account, storage and code caches start small and grow in steps, funding each step from one process-wide envelope sized at 1/32 of the memory available to Erigon (system RAM, cgroup limit or `GOMEMLIMIT`, whichever is lowest) and shared with other in-process caches such as the commitment branch cache. A growth step that would overflow the envelope is refused, and that cache evicts within its current size until envelope space frees up. The small construction-time reservation is taken unconditionally, so a full envelope can be modestly overdrawn.

What that means for sizing: each of these state caches starts at `max(1024, shards × 16)` entries, capped by its own ceiling. The shard count is the smaller of the ceiling over 64 and `GOMAXPROCS × 16`, each rounded up to a power of two, so on a multi-core host the initial reservation is larger than the 1024-entry floor — with 8 cores it settles at 2048 entries once the budget is big enough to support that many shards. A `STATE_CACHE_CODE` below about 12 MiB is bounded by its ceiling instead and stays under 1024 entries. That initial capacity is reserved from the envelope at construction, not on first use: the two byte-bearing code layers take 24 MiB between them before a single block executes. Only growth beyond it is demand-driven, so twice `STATE_CACHE_CODE` is the largest reservation those layers can reach, not one they hold from the start. `STATE_CACHE_CODE_INDEX` differs again — it fixes its entry count at construction but never reserves envelope bytes at all.
