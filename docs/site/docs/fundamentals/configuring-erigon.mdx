...
None of the four is an exact byte bound. Each is divided by a typical per-domain entry size to produce an entry-count cap, and it is that cap, not a running byte total, that evicts — so residency follows the entries actually stored. The code layers assume roughly 12 KiB per contract, and a working set of larger contracts will exceed the configured figure; with average entries the two layers together hold about twice `STATE_CACHE_CODE`. The account and storage caps are additionally clamped to between 1024 and roughly 16.7M entries, so for those entry sizes a budget below about 100 KB, or above about 1.4 GB, stops making a difference.
