...
**HTTPS and IPC transports.** *(New in v3.7)* These flags are accepted by both the `erigon` binary and the standalone [RPC Daemon](/fundamentals/modules/rpc-daemon) — the embedded RPC server reads them from the same configuration. They serve the same JSON-RPC API as `--http`, over TLS or a local socket instead.
...
* `--https.enabled`: Enables a JSON-RPC HTTPS server alongside the plain HTTP one.
  * Default: `false`
