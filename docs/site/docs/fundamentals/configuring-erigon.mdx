...
None of the four is an exact byte bound. Each is divided by a typical per-domain entry size to produce an entry-count cap, and it is that cap, not a running byte total, that evicts — so residency follows the entries actually stored. The code layers assume roughly 12 KiB per contract, and a working set of larger contracts will exceed the configured figure. `STATE_CACHE_CODE` is applied to each of the two layers, so the combined *nominal* capacity and envelope reservation are twice the value; actual heap use is normally lower, because a contract cached through both layers is stored once and referenced twice. The account and storage caps are additionally clamped to between 1024 and roughly 16.7M entries, so for those entry sizes a budget below about 100 KB, or above about 1.4 GB, stops making a difference.
...
What that means for sizing: once the envelope is saturated, raising those three budgets together adds no aggregate capacity — it only changes which cache wins the space that is left. On a 32 GB host the whole envelope is 1 GB, below the sum of the defaults above, so it saturates; a much larger host may never saturate it, and there raising them does help. `STATE_CACHE_CODE_INDEX` sits outside the envelope altogether — its tables are sized up front from the budget, so above the 1024-entry floor noted above, raising it adds index capacity independently of how saturated the envelope is.
