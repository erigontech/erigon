...
* While syncing, the result is an object with `currentBlock`, `highestBlock`, and a `stages` array (one entry per stage, with `{stage_name, block_number}`). Subtract `currentBlock` from `highestBlock` to see how many blocks remain; the `stages` array gives you the same per-stage progress as the log prefix above. `startingBlock` reports the block the current sync session started from, so `(currentBlock - startingBlock) / (highestBlock - startingBlock)` is the progress of this run.
