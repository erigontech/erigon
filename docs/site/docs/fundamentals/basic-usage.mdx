...
```
INFO[02-20|15:00:00.123] [3/12 Bodies] Downloading block bodies   block=18234567 ...
```

The prefix `[3/12 Bodies]` means: *"stage 3 of 12, currently in the Bodies stage"*. As stages complete, the prefix advances (`[4/12 Senders]`, `[5/12 Execution]`, …) until you reach `[12/12 Finish]`, at which point the node is at the chain tip.
...
* While syncing, the result is an object containing `startingBlock`, `currentBlock`, and `highestBlock` (all hex). Subtract `currentBlock` from `highestBlock` to see how many blocks remain.
