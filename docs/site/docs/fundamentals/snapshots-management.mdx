...
description: "Understand and manage Erigon snapshot files — check disk usage by category, compare node type estimates, and plan retention upgrades."
sidebar_position: 5
---
...
── Estimated Size by Node Type ────────────────────────────────
  ...
  full             459.0GB     -1.4TB    last 1.100.000  last 1.100.000
  blocks             1.3TB   -512.0GB    all blocks      last 1.100.000
  minimal          342.3GB     -1.5TB    last 100.000    last 100.000
```

The byte totals above are illustrative rather than capacity guarantees. Sizes use Erigon's `ByteCount` format (e.g. `850.4GB`, `1.2TB` — binary scale, no space). Block/step counts and prune-window sizes use `.` as the thousands separator, matching the binary's own output.
...
## Retention-window upgrades

### v3.7 default window increase

Erigon v3.7 increases the **Full** block and state-history windows and the **Blocks** state-history window from 262,144 to 1,100,000 blocks. On the first v3.7 start, existing datadirs automatically migrate fields left at these named-mode defaults. Fields configured explicitly on that start retain their requested values; Archive and Minimal modes are unchanged. See the [Upgrading guide](../get-started/installation/upgrading#v37-pruning-window-upgrade) to keep the v3.6 defaults. The wider setting does not restore previously deleted data: the available window grows as the node advances, or immediately after a re-sync.

The extra snapshots increase disk use as the window fills. Fresh syncs also download and process more snapshot data, which can increase network traffic, disk I/O, and sync time. A normal in-place upgrade that keeps the datadir's existing `preverified.toml` does not download the missing backlog. Running `erigon snapshots reset` or deleting that file can make Erigon request those files on the next start. A downgrade to Erigon 3.6 restores its 262,144-block default and can prune the newly accumulated data. See the [Upgrading guide](../get-started/installation/upgrading#v37-pruning-window-upgrade) before rolling back.

### Historical v3.5 EIP-8252 change

Erigon v3.5 retargeted the **Full** and **Blocks** pruning modes to the EIP-8252 reorg-retention window (262,144 blocks, ~36 days), up from the previous 100,000-block window. Erigon v3.7 supersedes that default with the 1,100,000-block window described above. The v3.5 change altered **which snapshot files were present on disk** after upgrading — most notably, full nodes pruned older block segments down to the window, freeing space. Existing datadirs migrated automatically on first start.

Operators who needed more than that window had to set the distance before the first v3.5 start because pruning is not reversible. To retain every post-merge block under Full mode, v3.5 accepted `--prune.distance.blocks=18446744073709551615`; the readable `keep-post-merge` alias arrived in v3.6. `--prune.mode=blocks` keeps every block back to genesis. See [Pruning Modes](pruning-modes) for current behavior.

After a prune pass, run `seg du` to inspect the resulting on-disk breakdown.
