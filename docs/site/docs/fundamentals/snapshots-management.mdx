...
- **Plain writes must actually be selected for this datadir.** The compiled default is plain from v3.6, but `references_in_commitment_branches` in `snapshots/erigondb.toml` overrides it, and an explicit `true` survives the upgrade. While it is set, merges keep writing *referenced* output and nothing migrates, however long the node runs.
...
Reconciles the local snapshot set against the published preverified set, then lets the next start fetch what is missing in the published v3.6 form, which is already plain. Files whose names appear in the preverified set are kept; at the default `--local=true` the ones removed are those the set does not describe, plus any file whose `.torrent` carries the wrong infohash or cannot be read at all.
