...
The snapshot set on disk reflects the node's **pruning mode**. `seg du` automatically detects which mode your current files correspond to. For full details on choosing a pruning mode, see [Pruning Modes](prune-modes).
