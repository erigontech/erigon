...
The snapshot set on disk reflects the node's **pruning mode**, and `seg du` detects which mode your current files correspond to when the layout is distinguishable. Young chains that have not crossed a pruning boundary can report `unknown`. For what each mode retains and how to choose one, see [Pruning Modes](pruning-modes).
...
Erigon v3.7 increases the **Full** block and state-history windows and the **Blocks** state-history window from 262,144 to 1,100,000 blocks. Existing datadirs that use these mode defaults migrate automatically. The wider setting does not restore previously deleted data: the available window grows as the node advances, or immediately after a re-sync. Archive, minimal, and custom distances are unchanged.

The extra snapshots increase disk use as the window fills. Fresh syncs also download and process more snapshot data, which can increase network traffic, disk I/O, and sync time. An in-place upgrade does not download the missing backlog. A downgrade to Erigon 3.6 restores its 262,144-block default and can prune the newly accumulated data. See the [Upgrading guide](../get-started/installation/upgrading#v37-pruning-window-upgrade) before rolling back.
