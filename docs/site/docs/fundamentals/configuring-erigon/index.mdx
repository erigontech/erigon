...
* `--sync.loop.block.limit value`: Sets the maximum number of blocks to process per loop iteration.
  * Default: `5000`
* `--sync.initial-cycle-block-ttl value`: Block window for treating a recently synced node as past the initial sync cycle. A larger value keeps a brief restart in the fast (no-downtime) pipeline mode; a node that has been off long enough to fall more than this many blocks behind re-enters the initial cycle. Set to `1` to always furious-prune (useful for bloatnet).
  * Default: `1024` (~3h on mainnet)
