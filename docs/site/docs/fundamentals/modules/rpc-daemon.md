...
The RPC daemon is a core component of Erigon that implements the [RPC Service](../../fundamentals/modules/rpc-daemon/) by processing JSON remote procedure calls (RPCs). It can be deployed in-process (running inside Erigon) or out-of-process (as a standalone service).
