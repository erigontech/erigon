...
For optimal performance, store the datadir on a single fast NVMe device formatted with ext4 or XFS —
see [Filesystem](../get-started/hardware-requirements#filesystem). If that is not feasible, you can keep the history on
a cheaper disk and still achieve good performance.
