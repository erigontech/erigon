...
| CPU | 4–8 cores for Full nodes, 8–16 cores for Archive nodes | More cores are generally better for intense sync and query operations. On amd64, the CPU must support the `x86-64-v2` baseline (SSE4.2, POPCNT): Intel Nehalem (2008), AMD Bulldozer (2011) or newer. |
...
:::note
Erigon checks this for you at startup. It detects the filesystem type behind each of its directories — which may sit on different mounts — and logs one warning per unrecognised type:

```text
WARN [diskutils] Filesystem is neither ext4 nor XFS, performance is unverified  fstype=zfs paths=/data/erigon see=https://docs.erigon.tech/get-started/hardware-requirements
```

It is a warning, not a refusal: the node starts and runs either way. A type Erigon cannot detect is logged at debug level instead, so no warning does not by itself confirm ext4 or XFS.
:::
...
The amount of disk space recommended and RAM you need depends on the [pruning mode](../fundamentals/pruning-modes) you want to run. **Current Disk Usage** values listed below are obtained using the standard Erigon + [Caplin](../fundamentals/caplin) configuration, varying only the `--prune.mode` flag unless a tab notes otherwise. Each figure is the size of the whole datadir with the node stopped, excluding the transient `temp/` directory.

:::note
These nodes were measured before the receipts cache was on by default. From 3.7 it is on, which adds about 441 GB on a mainnet archive node and a few GB on full and minimal nodes, whose receipts follow their history window. See [Database](../fundamentals/database). The recommended disk sizes below already leave room for it.
:::
...
| --- | --- | --- | --- | --- |
| Archive | {/* ds:mainnet:archive */}2.25 TB{/* ds:end */} | 4 TB | 32 GB | 64 GB |
| Full (Default) | {/* ds:mainnet:full */}534.94 GB{/* ds:end */} | 2 TB | 16 GB | 32 GB |
| Minimal | {/* ds:mainnet:minimal */}510.82 GB{/* ds:end */} | 1 TB | 16 GB | 64 GB |

_Current Disk Usage measured as of {/* ds-date:mainnet */}2026-09-17{/* ds:end */}, the oldest of the measurements in this table; usage grows over time as the chain grows._

</TabItem>
<TabItem value="gnosis-chain" label="Gnosis Chain">
| **Pruning Mode**  | **Current Disk Usage** | **Disk Size (Recommended)** | **RAM (Required)** | **RAM (Recommended)** |
| -------------- | ---------------------- | --------------------------- | ------------------ | --------------------- |
| Archive        | {/* ds:gnosis:archive */}803.35 GB{/* ds:end */} | 1 TB                        | 16 GB              | 32 GB                 |
| Full (Default) | {/* ds:gnosis:full */}314.35 GB{/* ds:end */}    | 1 TB                        | 8 GB               | 16 GB                 |
| Minimal        | {/* ds:gnosis:minimal */}311.75 GB{/* ds:end */} | 500 GB                      | 8 GB               | 16 GB                 |

_Current Disk Usage measured as of {/* ds-date:gnosis */}2026-09-19{/* ds:end */}, the oldest of the measurements in this table; usage grows over time as the chain grows._
</TabItem>
<TabItem value="sepolia" label="Sepolia">
| **Pruning Mode** | **Current Disk Usage** |
| -------------- | ---------------------- |
| Archive        | {/* ds:sepolia:archive */}1.12 TB{/* ds:end */} |
| Full (Default) | {/* ds:sepolia:full */}301.82 GB{/* ds:end */} |
| Minimal        | {/* ds:sepolia:minimal */}272.59 GB{/* ds:end */} |

_Current Disk Usage measured as of {/* ds-date:sepolia */}2026-09-18{/* ds:end */}, the oldest of the measurements in this table; usage grows over time as the chain grows._
</TabItem>
<TabItem value="hoodi" label="Hoodi">
| **Pruning Mode** | **Current Disk Usage** |
| -------------- | ---------------------- |
| Archive        | {/* ds:hoodi:archive */}132.92 GB{/* ds:end */} |
| Full (Default) | {/* ds:hoodi:full */}62.86 GB{/* ds:end */} |
| Minimal        | {/* ds:hoodi:minimal */}58.89 GB{/* ds:end */} |

_Current Disk Usage measured as of {/* ds-date:hoodi */}2026-09-20{/* ds:end */}, the oldest of the measurements in this table; usage grows over time as the chain grows._
</TabItem>
<TabItem value="chiado" label="Chiado">
| **Pruning Mode** | **Current Disk Usage** |
| -------------- | ---------------------- |
| Archive        | {/* ds:chiado:archive */}29.98 GB{/* ds:end */} |
| Full (Default) | {/* ds:chiado:full */}16.07 GB{/* ds:end */} |
| Minimal        | {/* ds:chiado:minimal */}16.00 GB{/* ds:end */} |

_Current Disk Usage measured as of {/* ds-date:chiado */}2026-09-20{/* ds:end */}, the oldest of the measurements in this table; usage grows over time as the chain grows._
</TabItem>
</Tabs>
...
| -------- | -------- | -------------- | -------- |
| Ethereum | ~14 h 20 m | ~5 h 40 m    | ~5 h 20 m |
| Gnosis   | ~5 h 15 m  | ~2 h 50 m    | ~2 h 50 m |

:::info
Measured on Erigon 3.6.1 in September 2026, syncing each node from an empty datadir on one machine (16-core Intel Core Ultra 9 185H, 93 GB RAM, NVMe SSD on ext4), one node at a time. Time is measured from process start to the first report of a head at the chain tip.

**The download rate was capped at 400 Mbps, and that cap — not the hardware — is what set these times.** Most of the elapsed time is snapshot download rather than block execution, so your available [bandwidth](#bandwidth-requirements) influences the result far more than CPU does. Note that 400 Mbps is eight times the 50 Mbps recommended above for a staking node: on a link at that recommended speed, expect these times to be several times longer, with block execution rather than download eventually setting the floor. Treat them as an order of magnitude, not a target — they are a single sample per configuration, with no variance estimate, and sync time also depends on peer quality and chain activity on the day.

Note that full and minimal land within a few percent of each other on both chains. The gap between them is small; the gap to archive is about 1.9× on Gnosis and 2.5× on Ethereum.
:::
