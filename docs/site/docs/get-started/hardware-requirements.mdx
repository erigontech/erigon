...
Erigon's bottleneck is disk **latency**, not throughput or IOPS — see [Improving Performance](/help-center/known-issues#improving-performance). Two properties of the workload make its reads impossible to predict, and therefore impossible to pre-warm:
...
* **ZFS** behaves more like a database than a filesystem. It keeps its own write-ahead log and its own cache (**ARC**), and that cache competes with the OS page cache for the same RAM. Its features — compression, encryption, dedup — each cost latency, and tuning it for this workload is not a solved problem.
