...
| Disk Configuration | A single NVMe device is enough for any pruning mode. | An archive node fits on one 4 TB drive; RAID 0 is not needed and doubles the chance of losing the datadir. See [Filesystem](#filesystem). |
| Filesystem | ext4 | See [Filesystem](#filesystem) below. |
...
Use **ext4** on a single NVMe device. This is what Erigon is developed and measured on, and the disk figures below are obtained on it.

Mount with `noatime` so reads do not generate metadata writes:

```
/dev/nvme0n1p2  /data  ext4  rw,noatime  0 1
```

**XFS** is a reasonable alternative and is used by several operators. Filesystems other than these two are not part of what we measure, so treat their performance as unverified rather than assumed.

A few points that follow from Erigon's storage model — see [Database](../fundamentals/database) for the layout:

* **One device is enough.** An archive node fits on a single 4 TB drive, so striping across disks buys nothing and doubles the probability of losing the datadir. Use RAID 0 only if you already have several smaller drives and no single one is large enough.
* **Most of the datadir is re-downloadable.** Snapshot files are content-addressed and fetched from peers, so redundancy matters far less here than for a database-backed service. Only `chaindata/` is worth backing up.
...
_Current Disk Usage measured as of {/* ds-date:sepolia */}2026-07-29{/* ds:end */}; usage grows over time as the chain grows. Execution layer only. Full and minimal are not measured on this network: only the QA full-node and minimal-node sync runs upload disk-usage figures, and both run on Ethereum mainnet and Gnosis Chain._
</TabItem>
...
_Current Disk Usage measured as of {/* ds-date:hoodi */}2026-07-29{/* ds:end */}; usage grows over time as the chain grows. Execution layer only. Full and minimal are not measured on this network: only the QA full-node and minimal-node sync runs upload disk-usage figures, and both run on Ethereum mainnet and Gnosis Chain._
</TabItem>
...
_Current Disk Usage measured as of {/* ds-date:chiado */}2026-07-29{/* ds:end */}; usage grows over time as the chain grows. Execution layer only. Full and minimal are not measured on this network: only the QA full-node and minimal-node sync runs upload disk-usage figures, and both run on Ethereum mainnet and Gnosis Chain._
</TabItem>
</Tabs>
