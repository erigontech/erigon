...
On the first start with Erigon v3.7, fields left at the `full` or `blocks` named-mode defaults are written to the datadir's MDBX settings with a wider window. The `full` block and state-history windows, and the `blocks` mode state-history window, increase from 262,144 to 1,100,000 blocks. Explicitly configured fields retain their requested values. To keep the v3.6 window, set the relevant flags before the first v3.7 start: use `--prune.distance=262144 --prune.distance.blocks=262144` for `full`, or `--prune.distance=262144` for `blocks`.

Data already pruned by an older release is not restored. The physical window grows as the node advances; re-sync the datadir if the full window is needed immediately. Plan for higher disk use while the window fills. A fresh sync downloads and processes more snapshot data than the previous default, so it can require more bandwidth, disk I/O, and time. A normal in-place upgrade that keeps the datadir's existing `preverified.toml` does not download the missing backlog. Running `erigon snapshots reset` or deleting that file can make Erigon request those files on the next start.

:::warning
Downgrading a migrated datadir to Erigon 3.6 restores that release's 262,144-block default and can permanently prune the backlog accumulated under v3.7. Before a rollback, back up the datadir and configure 1,100,000 explicitly for every finite window you need to preserve. For `full`, use `--prune.distance=1100000 --prune.distance.blocks=1100000`; for `blocks`, use `--prune.distance=1100000`. If the datadir has a finite commitment-history distance above 262,144, Erigon 3.6 also requires `--prune.distance` to be at least that distance or it will refuse to start.
:::
