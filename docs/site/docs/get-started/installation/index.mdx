...
* The multi-platform Docker image is available for `linux/amd64` and `linux/arm64` platforms and is now based on Debian Bookworm. There's no need to pull a different image for another supported platform. The `linux/amd64` image is compiled for the `x86-64-v2` baseline (SSE4.2, POPCNT); Intel Nehalem (2008) and AMD Bulldozer (2011) or newer qualify, older hardware and CPU-masked hypervisor profiles are not supported.
