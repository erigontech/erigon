...
* The multi-platform Docker image is available for `linux/amd64` and `linux/arm64` platforms and is now based on Debian Bookworm. There's no need to pull a different image for another supported platform. The `linux/amd64` image is compiled for the `x86-64-v2` baseline (SSE4.2, POPCNT); Intel Nehalem (2008) and AMD Bulldozer (2011) or newer qualify, older hardware and hypervisor profiles that mask v2 CPU features are not supported.
...
The amd64 artifacts are compiled for the `x86-64-v2` baseline (SSE4.2, POPCNT). Intel Nehalem (2008) and AMD
...
* For **GCC** (version 11 or newer): [https://gcc.gnu.org/install/index.html](https://gcc.gnu.org/install/index.html).
...
For the full image, supported boards, and setup instructions, see the [Ethereum on ARM documentation](https://ethereum-on-arm-documentation.readthedocs.io/) and the [project on GitHub](https://github.com/EOA-Blockchain-Labs/ethereumonarm).
