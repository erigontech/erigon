...
export const go = (id) => {
  ...
  if (!el.open) {
    el.addEventListener('toggle', scroll, { once: true });
    el.open = true;
  } else {
    scroll();
  }
...
Go to the Erigon [releases page](https://github.com/erigontech/erigon/releases) on GitHub and select the latest stable version (e.g., `{ERIGON_VERSION}`) or whichever version you prefer.
...
| 64-bit Intel/AMDv2 | Compressed Archive (.tar.gz) | erigon_v`{ERIGON_VERSION}`_amd64v2.tar.gz |
