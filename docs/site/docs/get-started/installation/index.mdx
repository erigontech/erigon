...
:::warning
**Windows**: note that Docker on Windows is affected by [WSL2 Performance and Data Storage](/docs/get-started/installation/#install-wsl).
:::
...
* `--chain=hoodi` specifies which [network](/docs/fundamentals/supported-networks) to sync
* `--prune.mode=minimal` tells Erigon to use minimal [Sync Mode](/docs/fundamentals/sync-modes)
...
Go to the Erigon [releases page](https://github.com/erigontech/erigon/releases) on GitHub and select the latest stable version (e.g., `3.4.x`) or whichever version you prefer.
...
| --- | --- | --- |
| 64-bit Intel/AMD | Debian Package (.deb) | erigon_`3.4.x`_amd64.deb |
| 64-bit ARM | Debian Package (.deb) | erigon_`3.4.x`_arm64.deb |
| 64-bit Intel/AMD | Compressed Archive (.tar.gz) | erigon_v`3.4.x`_linux_amd64.tar.gz |
| 64-bit Intel/AMDv2 | Compressed Archive (.tar.gz) | erigon_v`3.4.x`_amd64v2.tar.gz |
| 64-bit ARM | Compressed Archive (.tar.gz) | erigon_v`3.4.x`_linux_arm64.tar.gz |
...
Once WSL2 is set up, you can build and run Erigon exactly as you would on a regular [Linux/macOS](/docs/get-started/installation/#linuxmacos) distribution.
...
- [Basic Usage](/docs/fundamentals/basic-usage)
