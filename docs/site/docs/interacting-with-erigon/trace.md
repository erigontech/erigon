...
1. `Object` - The filter object
   ...
   * `mode`: `String` - (optional) Default is `"intersection"`: OR within each address list, AND between the two lists. An omitted, `null`, or empty list imposes no restriction. Set `"union"` to match either populated list. Explicit `"intersection"` remains supported; other mode values return `-32602`.
   ...
   **Migration from the union default:** a query with both `fromAddress: [X]` and `toAddress: [X]` now selects calls from X to X. To keep selecting all activity involving X, add `mode: "union"`. One-sided queries remain restricted only by the populated list. `after` and `count` apply to the combined matching result, not separately to each list.
