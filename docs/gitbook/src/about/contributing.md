...
This documentation is powered by Gitbook but can be edited locally by
...
2. Open a Branch and Submit a PR:
    ...
    3. Before proceeding, make a backup of the `docs/gitbook/src/SUMMARY.md` file. Remove all empty lines and any lines
       starting with `##` to ensure compatibility with MdBook.
    4. To render the documentation locally, navigate in your terminal to the `/docs/gitbook` folder and run
       `mdbook serve`.
    5. Edit and preview your changes locally. Only modify content in Markdown format. Ensure all updates are verified in
       the local render.
