...
| Rate Limiting        | Protect against DoS attacks by limiting the processing capacity for batch requests and block range queries.                                                                                               | Configure `--rpc.batch.concurrency`, `--rpc.batch.limit`, and `--rpc.blockrange.limit` flags.                                                                                        |
