...
metaLinks:
  alternates:
    - https://app.gitbook.com/s/3DGBf2RdbfoitX1XMgq0/fundamentals/security
---
...
| Block Range Limiting | Prevent resource exhaustion (CPU/Memory) and hangs from "heavy" queries that scan excessive block ranges. Setting a limit protects the rpcdaemon from denial-of-service scenarios caused by large range queries. | Configure `--rpc.blockrange.limit` flag (default: `0` = unlimited).                                                                                                                  |
