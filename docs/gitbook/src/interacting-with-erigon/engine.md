...
You can optionally force Caplin to use the Engine API interface by setting the `--caplin.use-engine-api` flag. When this
flag is active, Caplin connects via the same [JWT](../../../get-started/fundamentals/jwt.md)-authenticated HTTP endpoint
used by external CL clients.
