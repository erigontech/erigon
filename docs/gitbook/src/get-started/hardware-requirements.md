...
The amount of disk space recommended and RAM you need depends on the [sync mode](../fundamentals/sync-modes.md) you want
to run. **Current Disk Usage** values listed below are obtained using the standard
Erigon + [Caplin](../fundamentals/caplin.md) configuration, with the sole exception of the `--prune.mode` flag.
