...
* Add `--prune.mode=minimal` to run minimal [Sync Mode](https://erigon.gitbook.io/docs/fundamentals/sync-modes) or `--prune.mode=archive` to run an archive node
...
When you get familiar with running Erigon from CLI you may also consider [staking](../../../archive/staking.md) and/or run a Gnosis node with an [external Consensus Layer](gnosis-with-an-external-cl.md).
