...
- [ ] Create `db/state/commitment_convert.go` with package header, copyright (2026), and types: `ConvertOpts`, `fileState`, `xformFns`, sentinel errors (`errSkip`, `errMixedKeyEncoding`, `errMixedSqueezeState`, `errRangeMatch`).
- [ ] Implement `detectKeyEncoding(samples [][]byte) (v2 bool, err error)` — per-key V1/V2 canonicality vote, tie-break to V1 unless every sample is V2-canonical, return error on mixed.
- [ ] Implement `detectKeyEncoding(samplePairs []sampledPair) (v2 bool, err error)` — for each non-state key call `nibbles.DecodeKeyV2(k)`; any error → V1 (return false); all canonical → V2 (return true). Error on zero non-state samples.
- [ ] Implement `detectSqueezeState(samplePairs []sampledPair) (squeezed bool, err error)` — for each non-state value, parse `commitment.BranchData`, scan embedded plain-key fields; return `true` immediately on the first key with length `< binary.MaxVarintLen64`; return `false` if all sample plain-keys are `≥ 10` bytes (20 or 52 expected).
- [ ] Implement `detectFileState(at *AggregatorRoTx, file VisibleFile, samples int) (fileState, error)` — uses BT ordinal lookup on the file's accessor to read `samples` (k, v) pairs at evenly-distributed positions; if BT ordinal API is unavailable, stride-skip on `at.d[kv.CommitmentDomain].dataReader(file.decompressor)` (log fallback). Delegates to the two detectors.
- [ ] Write tests in `commitment_convert_test.go` covering: all-V1 keys (every sample passes), all-V2 keys (every sample DecodeKeyV2-canonical), a single sample with bad parity byte (must vote V1), all-squeezed values (short embedded keys), all-unsqueezed (only 20/52-byte keys), partially-squeezed (one short key in sample → squeezed), and zero non-state samples → error with file name.
- [ ] **Known-ambiguous V1/V2 detection test**: hand-craft a V1 file whose every key also decodes V2-canonically (force trailing 0x00 or 0x01 in each `HexNibblesToCompactBytes` output). Verify the test exposes the mis-classification path; document the floor probability in the code comment.
- [ ] Run `go test ./db/state/... -run TestDetect -count=1 -failfast` — must pass before Task 3.
...
- [ ] Implement `keyXform(detected, target bool) func([]byte) ([]byte, error)` returning the appropriate function from the 4-case table. V1→V2: `nibbles.EncodeKeyV2(commitment.UncompactNibbles(k))`. V2→V1: `commitment.HexNibblesToCompactBytes(decoded)` where `decoded, err := nibbles.DecodeKeyV2(k)` and error propagates. Pass-throughs for matching axes.
- [ ] Implement `buildValueTransformer(detected, target bool, accounts, storage *DomainRoTx, af, sf *FilesItem, rng MergeRange) (valueTransformer, error)` returning a `valueTransformer` (the same type `dumpStepRangeOnDisk` and `commitmentValTransformDomain` use):
  - Both axes match → return `nil` (pass-through; `dumpStepRangeOnDisk` already treats `nil` as no-op).
  - unsqueezed → squeezed → call `at.d[kv.CommitmentDomain].commitmentValTransformDomain(rng, accounts, storage, af, sf)` and return its result.
  - squeezed → unsqueezed → return a closure: `func(v []byte, startTxNum, endTxNum uint64) ([]byte, error) { return ExpandShortenedKeysInBranch(v, af, sf, startTxNum, endTxNum) }`.
- [ ] **No state-value transformer**: KeyCommitmentState rows are not passed through `vt` because the converter routes them around the wal (push directly to `dumpStepRangeToPath` with their original value) — verify this is achievable with `TemporalMemBatch` semantics, or accept that the pass-through valueTransformer flows them unchanged when `vt(stateValue, ...) == stateValue` (which it does by construction in both squeeze directions, since state value has no embedded plain keys).
- [ ] Write tests for each of the 4 keyXform cases with hand-crafted V1/V2 key bytes (pull golden vectors from `execution/commitment/nibbles/nibbles_v2_test.go` — that file exists; note `nibbles_test.go` was referenced in error earlier).
- [ ] Write tests for `buildValueTransformer`: build a tiny synthetic squeezed BranchData and a tiny unsqueezed one, roundtrip through both directions, assert byte-equal.
- [ ] Run `go test ./db/state/... -run TestXform -count=1 -failfast` — must pass before Task 4.
