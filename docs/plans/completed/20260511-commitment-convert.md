...
- [x] Implement `convertCommitmentFile(ctx, at *AggregatorRoTx, file VisibleFile, dstDir string, opts ConvertOpts, progressPrefix string, logger log.Logger) (sizeDelta datasize.ByteSize, deltaPct float32, err error)`. The orchestrator builds `progressPrefix` once per file (e.g. `"(3/12 files, 25.0% overall)"`) and passes it in; `convertCommitmentFile` just appends it to its own log lines, no arithmetic inside.
- [x] Detect state via `detectFileState(at, file, 48)`; return `errSkip` if state == target on both axes.
- [x] Resolve `(stepFrom, stepTo)` from `file` (e.g., `file.StartRootNum()/stepSize`, `file.EndRootNum()/stepSize`). Resolve `(startTxNum, endTxNum)` similarly. Look up `af := at.d[kv.AccountsDomain].rawLookupFileByRange(startTxNum, endTxNum)` and `sf := at.d[kv.StorageDomain].rawLookupFileByRange(startTxNum, endTxNum)`. Missing → return error wrapping `errRangeMatch` with the step range.
- [x] Build keyXform via Task 3 helper. Build `vt valueTransformer` via Task 3's `buildValueTransformer`.
- [x] Allocate a fresh `*TemporalMemBatch{}` for this conversion and install just the commitment domain writer (`batch.domainWriters[kv.CommitmentDomain] = commitmentRo.NewWriter()`). Mirrors `TestDumpStepRangeToPath`'s minimal batch shape — no full `NewTemporalMemBatch` constructor needed because the converter only exercises the wal->dump path.
- [x] Open source reader via aggregator: `reader := commitmentRo.dataReader(src.decompressor)`; `reader.Reset(0)`. Source decompressor obtained via the `visibleFile{}.src` type assertion (same pattern as `detectFileState`).
- [x] Stream loop with progress accounting. `totalForFile := at.KeyCountInFiles(...)`. Read (k, v); apply keyXform unless `bytes.Equal(k, commitmentdb.KeyCommitmentState)`; push transformed `(newK, v)` (or unchanged `(k, v)` for state) into the commitment wal via `PutWithPrev`. Per-key+value copies inserted before the wal call because both `seg.Reader.Next` and `keyXform` may return buffer-backed slices that get stomped on the next iteration. `vt` is **not** applied here; `dumpStepRangeToPath` applies it during collate.
- [x] 30s `time.Ticker` inside the loop logs `progress <ki>/<totalForFile> file=<name> (<file_pct>% in file) <progressPrefix>` where `<file_pct> = 100*ki/totalForFile`. `ctx.Done()` also short-circuits the loop with the context error.
- [x] After the stream completes: `commitmentRo.d.dumpStepRangeToPath(ctx, stepFrom, stepTo, batch, vt, dstDir, false)`. Produces `.kv` + per-config index files (`.kvi` for the default HashMap accessor; `.bt` + `.kvei` when BTree+Existence are enabled) in `dstDir` and does **not** integrate them into the aggregator's dirty files (`integrate=false`).
- [x] Compute `sizeDelta`, `deltaPct` via new helper `commitmentFileSizeDelta(origPath, newPath)` (same `os.Stat`-pair pattern as `squeeze.go:156-166`).
- [x] Log file-done line: `[commitment_convert] phase 1 file done <name> sizeDelta=<delta%> ki=<count> <progressPrefix>`.
- [x] Tests live in two files. **In-package detector/codec tests** stay in `db/state/commitment_convert_test.go` (package `state`). **Aggregator-level convert tests** live in `db/state/commitment_convert_blackbox_test.go` (package `state_test`) because the full-aggregator helper `testDbAggregatorWithFiles` imports `db/kv/temporal` — a cycle from inside `package state`. A small test-only bridge `db/state/commitment_convert_export_test.go` re-exports `convertCommitmentFile` as `ConvertCommitmentFileForTest` (and `errSkip` as `ErrSkipForTest`) so the black-box tests can call the private helper without leaking it from the production API. Four scenarios cover the per-axis behaviour: `TestConvertCommitmentFile_AlreadyTarget` (errSkip short-circuit), `TestConvertCommitmentFile_V1ToV2_KeysOnly` (file/index emission + non-state key V2-canonicality), `TestConvertCommitmentFile_V1ToV1_Squeeze` (value-axis only), and `TestConvertCommitmentFile_RoundTrip_V1V2` (V1→V2 forward, swap into SnapDomain via `agg.ReloadFiles()`, V2→V1 back, byte-equal `.kv` against the pre-pass snapshot). `OpenFolder` alone is **insufficient** for the round-trip because the aggregator caches mmap handles; `ReloadFiles` (close-dirty-then-rescan) is required to pick up the swapped file. Documented in the test comment.
- [x] Run `go test ./db/state/... -run TestConvertCommitmentFile -count=1 -failfast` — passes (4 tests, ~10s).
...
- [x] Implement `ConvertCommitmentFiles(ctx context.Context, at *AggregatorRoTx, opts ConvertOpts, logger log.Logger) error`.
- [x] **Pre-flight**: enumerate commitment files via `at.Files(kv.CommitmentDomain)`; if empty → log "no commitment files to convert" and return.
- [x] **Pre-flight**: refuse if `snapshots/backup/domains/` already exists with content (preserves a prior conversion's backup). Wipe `snapshots/rebuild/domain/` if leftover from a prior crashed run (log it).
- [x] **Phase 1 (convert all)**: mkdir `snapshots/rebuild/domain/`. For each file: build `progressPrefix` string (`(<idx+1>/<N> files, <overall_pct>% overall)`); call `convertCommitmentFile(..., dstDir=rebuild/domain/, progressPrefix, logger)`. On `errSkip` → log skip line, do not increment converted count; on real error → abort with file path and tell user to `rm -rf snapshots/rebuild/`. Accumulate `totalSizeDelta` and `processedFiles`.
- [x] **Early exit**: if `processedFiles == 0` after Phase 1 → clean up empty `rebuild/domain/`, log "no files needed conversion", return.
- [x] **Phase 1 wait gate**: phases 2–5 only begin after the Phase 1 loop completes for **every** file (no per-file interleaving with backup/promote). Log `[commitment_convert] phase 1 complete: converted <m>, skipped <n>, total <m+n>` at the boundary.
- [x] **Phase 2 (pre-swap check)**: enumerate visible commitment files and verify each that produced output has its full set of accessor siblings (`.kvi` for the default HashMap accessor; `.bt`/`.kvei` when the commitment domain config enables BTree+Existence). Abort with the missing path on the first mismatch.
- [x] **Phase 3 (backup)**: `mkdir snapshots/backup/domains/`. For each converted file: glob `<snapDomain>/*-commitment.<from>-<to>.*` (catches `.kv` + any accessor extension + any of the four `.torrent` siblings) and `os.Rename` each match into `snapshots/backup/domains/`. After this phase, no commitment files remain at the original paths.
- [x] **Phase 4 (promote)**: `os.Rename` every regular file in `snapshots/rebuild/domain/` (the converted `.kv` + per-config indexes) → `snapshots/domain/`. `dir.RemoveAll(rebuild/domain)` and the parent `rebuild/` if empty.
- [x] **Phase 5 (reopen + revert instruction)**: `agg.ReloadFiles()` (close dirty + rescan — `OpenFolder` alone is insufficient because the aggregator caches mmap handles on the now-moved originals; the Task 4 round-trip test discovered this same gotcha). No `BuildMissedAccessors` call. Emit the revert instruction. **Caveat**: `ReloadFiles` invalidates the `*AggregatorRoTx` passed in; the orchestrator's godoc and the CLI in Task 6 must treat `at` as dead after a successful return.
- [x] **Full-flow test (`TestConvertCommitmentFiles_FullFlow`)**: builds commitment files via `testDbAggregatorWithFiles` (V1+unsqueezed), runs the orchestrator with `--squeeze=true --nibbles.v2=false` (value-axis only — orchestrator mechanics without composing two codecs at once), asserts: `snapshots/backup/domains/` byte-equals the original commitment file map; `snapshots/rebuild/` is gone; new `.kv` + sibling `.kvi` land in `snapshots/domain/`; commitment root recomputed via `execctx.NewSharedDomains` + `domains.ComputeCommitment` matches the pre-conversion root.
- [x] **Composed round-trip test (`TestConvertCommitmentFiles_RoundTripComposed`)** — V1+unsqueezed → V2+squeezed → V1+unsqueezed via two orchestrator invocations (with `dir.RemoveAll(backupDir)` between passes because Phase-1 pre-flight refuses a non-empty backup). Compares the post-roundtrip `snapshots/domain/` map byte-for-byte against the starting map. Both axes round-trip in a single test.
- [x] **Edge-case tests**:
  - `TestConvertCommitmentFiles_BackupExists` — pre-populated `snapshots/backup/domains/` → abort with clear error naming the offending path.
  - `TestConvertCommitmentFiles_AllAlreadyTarget` — all files already in target state → no `rebuild/`, no `backup/`, snapshots untouched, clean return.
  - `TestConvertCommitmentFiles_LeftoverRebuild` — pre-seeded junk in `snapshots/rebuild/domain/` → wiped during pre-flight, conversion proceeds, junk file gone afterwards.
  - `TestConvertCommitmentFiles_EmptyDatadir` — `testDbAndAggregatorv3` produces no commitment files; orchestrator returns nil immediately, no `backup/` or `rebuild/` created.
  - Range mismatch is not separately tested at orchestrator level: `testDbAggregatorWithFiles` produces matched ranges by construction, and the underlying error path is already exercised in `convertCommitmentFile`'s code under Task 4 (the function wraps `errRangeMatch` with the offending step range before returning). Hand-constructing a mismatched datadir would be an order-of-magnitude more scaffolding than the assertion adds.
- [x] Run `go test ./db/state/... -run TestConvertCommitmentFiles -count=1 -failfast` — passes (6 tests, ~12s).
