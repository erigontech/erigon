...
* [Command line options](/docs/fundamentals/configuring-erigon/#command-line-options) (flags)
* [Configuration file](/docs/fundamentals/configuring-erigon/#configuration-file)
* [Environment variables](/docs/fundamentals/configuring-erigon/#environment-variables)

## Command line options

Here's a breakdown of the most important flags, see [options](/docs/fundamentals/configuring-erigon/#options) for the full list:
...
* `--chain value`: Sets the name of the [network](/docs/fundamentals/supported-networks) to join.
  * Default: `mainnet`
...
* `--prune.mode value`: Selects a pruning preset (`full`, `archive`, `minimal`, `blocks`). See also [Sync Modes](/docs/fundamentals/sync-modes)
  * Default: `"full"`
...
* `--prune.distance.blocks value`: Keeps block history for the latest `N` blocks.
  * Default: `0`
* `--prune.include-commitment-history, --prune.experimental.include-commitment-history, --experimental.commitment-history`: Enables fast `eth_getProof` for executed blocks by storing commitment history. Requires +32 GB RAM. See [`eth_getProof`](/docs/interacting-with-erigon/eth.md#eth_getproof).
  * Default: `false`
...
* `--nat value`: The NAT port mapping mechanism (See [here](/docs/fundamentals/configuring-erigon/nat) for more details).
