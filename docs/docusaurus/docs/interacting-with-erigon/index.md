...
The Erigon RPC Service, managed by Erigon's modular [RPC daemon](../modules/rpc-daemon), supports various API namespaces, which can be enabled or disabled using the `--http.api` flag. The available namespaces include:

* [`eth`](eth): Standard Ethereum API.
* [`erigon`](erigon): Erigon-specific extensions.
* [`engine`](engine): The JSON-RPC Interface for Execution and Consensus Layer Communication.
* [`web3`](web3): Web3 compatibility API.
* [`net`](net): Network information API.
* [`debug`](debug): Debugging and tracing API.
* [`trace`](trace): Transaction tracing API.
* [`txpool`](txpool): Transaction pool API.
* [`admin`](admin): Node administration API
* [`bor`](bor): Polygon Bor-specific API (when running on Polygon)
* [`ots`](ots): These methods are specifically tailored for use with Otterscan, an open-source, fast block explorer.
* [`internal`](internal): Erigon specific API for development and debugging purposes.
* [`gRPC`](grpc): API for lower-level data access.
