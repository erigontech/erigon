...
func (pi *PeerInfo) AddKnownWitness(hash libcommon.Hash) {
	pi.lock.Lock()
	defer pi.lock.Unlock()
...
func runWitPeer(
	...
	send func(msgId proto_sentry.MessageId, peerID [64]byte, b []byte),
	hasSubscribers func(msgId proto_sentry.MessageId) bool,
	getWitnessRequest func(hash libcommon.Hash, peerID [64]byte) bool,
	logger log.Logger,
) *p2p.PeerError {
...
func NewGrpcServer(ctx context.Context, dialCandidates func() enode.Iterator, readNodeInfo func() *eth.NodeInfo, cfg *p2p.Config, protocol uint, logger log.Logger) *GrpcServer {
	ss := &GrpcServer{
		...
		logger:                logger,
		activeWitnessRequests: make(map[libcommon.Hash]*WitnessRequest),
	}
...
type GrpcServer struct {
	...
	activeWitnessRequests map[libcommon.Hash]*WitnessRequest
...
func (ss *GrpcServer) getWitnessRequest(hash libcommon.Hash, peerID [64]byte) bool {
	ss.witnessRequestMutex.Lock()
	defer ss.witnessRequestMutex.Unlock()
