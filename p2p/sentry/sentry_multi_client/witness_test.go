...
func createTestMultiClient(t *testing.T) (*MultiClient, kv.TemporalRwDB) {
	...
	agg, err := state.NewAggregator2(context.Background(), dirs, 16, baseDB, logger)
	require.NoError(t, err)
	t.Cleanup(agg.Close)
	tdb := temporal.New(baseDB, agg)
	...
	return &MultiClient{
		...
		logger:        logger,
		blockReader:   freezeblocks.NewBlockReader(nil, nil, nil, nil),
	}, tdb
}
