...
func TestBlockRangeTrackerAccept(t *testing.T) {
	...
	if send, dueToUnwind := tracker.decide(first); !send || dueToUnwind {
		t.Fatal("expected initial announcement without forward gating")
	}
	tracker.record(first)

	if send, _ := tracker.decide(first); send {
	...
	if send, _ := tracker.decide(advanceBelowThreshold); send {
	...
	if send, dueToUnwind := tracker.decide(advanceThreshold); !send || dueToUnwind {
	...
	if send, dueToUnwind := tracker.decide(regression); !send || !dueToUnwind {
