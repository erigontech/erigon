...
func (p *BlockRawBodiesPacket) Unpack() ([][][]byte, [][]*types.Header, []types.Withdrawals, []types.BlockAccessList) {
	var (
		txSet         = make([][][]byte, len(*p))
		uncleSet      = make([][]*types.Header, len(*p))
		withdrawalSet = make([]types.Withdrawals, len(*p))
		balSet        = make([]types.BlockAccessList, len(*p))
	)
	for i, body := range *p {
		txSet[i], uncleSet[i], withdrawalSet[i], balSet[i] = body.Transactions, body.Uncles, body.Withdrawals, body.BlockAccessList
	}
	return txSet, uncleSet, withdrawalSet, balSet
}
