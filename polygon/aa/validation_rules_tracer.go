...
func (t *ValidationRulesTracer) OnOpcode(pc uint64, op byte, gas, cost uint64, scope tracing.OpContext, rData []byte, depth int, err error) {
	...
	// Track EXTCODE operations and validate EIP-7702 delegation
	...
	// Check for CALL operations and validate EIP-7702 delegation
	if opCode == vm.CALL || opCode == vm.CALLCODE || opCode == vm.DELEGATECALL || opCode == vm.STATICCALL {
		if len(scope.StackData()) > 0 {
			addr := libcommon.BytesToAddress(scope.StackData()[0].Bytes())
			if t.isDelegatedAccount(scope.Code()) && addr != t.senderAddress {
	...
	// Check for other banned opcodes
...
// isAssociatedStorage checks if a storage slot is associated with an address
func (t *ValidationRulesTracer) isAssociatedStorage(slot libcommon.Hash, addr libcommon.Address) bool {
	// The slot value is the address
	if bytes.Equal(slot.Bytes(), addr.Bytes()) {
		return true
	}

	// Case 2: The slot value was calculated as keccak(A||x)+n
	// We need to check if the slot could have been derived from the address
	// by checking if it's within the range of possible values
	for i := 0; i <= 128; i++ {
		// Create a buffer with address (20 bytes) and x (32 bytes)
		buf := make([]byte, 52)
		copy(buf, addr.Bytes())

		// Try all possible x values that could result in this slot
		// We can't know the exact x, but we can check if the slot
		// could be derived from the address with some x and n
		...
		// Check if this matches our slot
