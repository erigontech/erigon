...
func TestPromoteHashedStateClearState(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error while promoting state: %w", err)
	}
...
func TestUnwindHashed(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error while unwind state: %w", err)
	}
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := promoteHashedStateIncrementally("logPrefix", &StageState{BlockNumber: 1}, 1, 10, tx, StageHashStateCfg(db, t.TempDir()), ctx.Done()); !errors.Is(err, tc.errExp) {
				t.Errorf("error does not match expected error while shutdown promoteHashedStateIncrementally, got: %w, expected: %v", err, tc.errExp)
			}
		})
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := PromoteHashedStateCleanly("logPrefix", tx, StageHashStateCfg(db, t.TempDir()), ctx.Done()); !errors.Is(err, tc.errExp) {
				t.Errorf("error does not match expected error while shutdown promoteHashedStateCleanly , got: %w, expected: %v", err, tc.errExp)
			}
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err = unwindHashStateStageImpl("logPrefix", u, s, tx, cfg, ctx.Done()); !errors.Is(err, tc.errExp) {
				t.Errorf("error does not match expected error while shutdown unwindHashStateStageImpl, got: %w, expected: %v", err, tc.errExp)
			}
