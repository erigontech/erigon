...
func TestPromoteHashedStateClearState(t *testing.T) {
	...
	err = PromoteHashedStateCleanly("logPrefix", tx2.(ethdb.HasTx).Tx().(ethdb.RwTx), StageHashStateCfg(db2.RwKV(), getTmpDir()), nil)
	if err != nil {
...
func TestPromoteHashedStateIncremental(t *testing.T) {
	...
	cfg := StageHashStateCfg(db2.RwKV(), getTmpDir())
	err = PromoteHashedStateCleanly("logPrefix", tx2.(ethdb.HasTx).Tx().(ethdb.RwTx), cfg, nil)
	if err != nil {
		t.Errorf("error while promoting state: %v", err)
	}
	err = tx2.CommitAndBegin(context.Background())
	...
	err = promoteHashedStateIncrementally("logPrefix", &StageState{BlockNumber: 50}, 50, 101, tx2.(ethdb.HasTx).Tx().(ethdb.RwTx), cfg, nil)
	if err != nil {
		t.Errorf("error while promoting state: %v", err)
	}
	err = tx2.Commit()
...
func TestPromoteHashedStateIncrementalMixed(t *testing.T) {
	...
	err = promoteHashedStateIncrementally("logPrefix", &StageState{}, 50, 101, tx2.(ethdb.HasTx).Tx().(ethdb.RwTx), StageHashStateCfg(db2.RwKV(), getTmpDir()), nil)
	if err != nil {
		t.Errorf("error while promoting state: %v", err)
	}
...
func TestUnwindHashed(t *testing.T) {
	...
	err = unwindHashStateStageImpl("logPrefix", u, s, tx2.(ethdb.HasTx).Tx().(ethdb.RwTx), StageHashStateCfg(db2.RwKV(), getTmpDir()), nil)
	if err != nil {
...
func TestPromoteIncrementallyShutdown(t *testing.T) {
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := promoteHashedStateIncrementally("logPrefix", &StageState{BlockNumber: 1}, 1, 10, tx.(ethdb.HasTx).Tx().(ethdb.RwTx), StageHashStateCfg(db.RwKV(), getTmpDir()), ctx.Done()); !errors.Is(err, tc.errExp) {
...
func TestPromoteHashedStateCleanlyShutdown(t *testing.T) {
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := PromoteHashedStateCleanly("logPrefix", tx.(ethdb.HasTx).Tx().(ethdb.RwTx), StageHashStateCfg(db.RwKV(), getTmpDir()), ctx.Done()); !errors.Is(err, tc.errExp) {
...
func TestUnwindHashStateShutdown(t *testing.T) {
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			generateBlocks(t, 1, 10, plainWriterGen(tx), changeCodeWithIncarnations)
			cfg := StageHashStateCfg(db.RwKV(), getTmpDir())
			err = PromoteHashedStateCleanly("logPrefix", tx.(ethdb.HasTx).Tx().(ethdb.RwTx), cfg, nil)
			require.NoError(t, err)

			u := &UnwindState{UnwindPoint: 5}
			s := &StageState{BlockNumber: 10}
			if err = unwindHashStateStageImpl("logPrefix", u, s, tx.(ethdb.HasTx).Tx().(ethdb.RwTx), cfg, ctx.Done()); !errors.Is(err, tc.errExp) {
