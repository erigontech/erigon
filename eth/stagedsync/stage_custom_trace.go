...
func SpawnCustomTrace(cfg CustomTraceCfg, ctx context.Context, logger log.Logger) error {
	...
	for ; startBlock < endBlock; startBlock += 300_000 {
		if err := customTraceBatchProduce(ctx, cfg.execArgs, cfg.db, startBlock, startBlock+300_000, "custom_trace", logger); err != nil {
			return err
		}
	}
