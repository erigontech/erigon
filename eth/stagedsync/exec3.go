...
func dumpPlainStateDebug(tx kv.RwTx, doms *state2.SharedDomains) {
	...
	{
		it, err := tx.(state2.HasAggCtx).AggCtx().(*state2.FilesRoTx).DomainRangeLatest(tx, kv.CommitmentDomain, nil, nil, -1)
		if err != nil {
			panic(1)
		}
		for it.HasNext() {
...
func flushAndCheckCommitmentV3(ctx context.Context, header *types.Header, applyTx kv.RwTx, doms *state2.SharedDomains, cfg ExecuteBlockCfg, e *StageState, maxBlockNum uint64, parallel bool, logger log.Logger, u Unwinder, inMemExec bool) (bool, error) {
	...
	unwindToLimit, err := applyTx.(state2.HasAggCtx).AggCtx().(*state2.FilesRoTx).CanUnwindDomainsToBlockNum(applyTx)
	if err != nil {
		return false, err
	}
	...
	allowedUnwindTo, ok, err := applyTx.(state2.HasAggCtx).AggCtx().(*state2.FilesRoTx).CanUnwindBeforeBlockNum(unwindTo, applyTx)
	if err != nil {
		return false, err
	}
	if !ok {
