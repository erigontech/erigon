...
func TestBodiesUnwind(t *testing.T) {
	...
	{
		err = rawdb.MakeBodiesNonCanonical(tx, 5+1, false, m.Ctx, "test", logEvery, br.TxsV3Enabled()) // block 5 already canonical, start from next one
		require.NoError(err)
...

	{
		...
		err = rawdb.MakeBodiesNonCanonical(tx, 5+1, false, m.Ctx, "test", logEvery, br.TxsV3Enabled())
		require.NoError(err)
