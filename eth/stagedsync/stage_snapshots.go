...
func DownloadAndIndexSnapshotsIfNeed(s *StageState, ctx context.Context, tx kv.RwTx, cfg SnapshotsCfg, logger log.Logger) error {
	...
	if err := snapshotsync.WaitForDownloader(ctx, s.LogPrefix(), cfg.dirs, true, cfg.blobs, cfg.caplinState, cfg.prune, cstate, agg, tx, cfg.blockReader, cfg.blockReader.TxnumReader(ctx), &cfg.chainConfig, cfg.snapshotDownloader, cfg.syncConfig); err != nil {
		return err
	}
	...
	if err := snapshotsync.WaitForDownloader(ctx, s.LogPrefix(), cfg.dirs, false, cfg.blobs, cfg.caplinState, cfg.prune, cstate, agg, tx, cfg.blockReader, cfg.blockReader.TxnumReader(ctx), &cfg.chainConfig, cfg.snapshotDownloader, cfg.syncConfig); err != nil {
		return err
	}
