...
func SpawnMiningExecStage(s *StageState, tx kv.RwTx, cfg MiningExecCfg, quit <-chan struct{}) error {
	...
	if err := core.FinalizeBlockExecution(cfg.engine, stateReader, current.Header, current.Txs, []types.Transaction{}, current.Uncles, stateWriter, &cfg.chainConfig, ibs, nil, nil, nil); err != nil {
		return err
	}
