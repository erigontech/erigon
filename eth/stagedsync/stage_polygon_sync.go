...
func (s polygonSyncStageSbpsStore) GetLastEntityId(ctx context.Context) (uint64, bool, error) {
	entity, ok, err := s.GetLastEntity(ctx)
	if err != nil || !ok {
		return 0, ok, err
	}
...
func (s polygonSyncStageSbpsStore) GetLastEntity(ctx context.Context) (*heimdall.SpanBlockProducerSelection, bool, error) {
	type response struct {
		v   []byte
		ok  bool
		err error
...
func (s polygonSyncStageSbpsStore) GetEntity(ctx context.Context, id uint64) (*heimdall.SpanBlockProducerSelection, bool, error) {
	type response struct {
		v   []byte
		ok  bool
		err error
