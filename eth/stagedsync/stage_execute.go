...
func unwindExec3(u *UnwindState, s *StageState, txc wrap.TxContainer, ctx context.Context, accumulator *shards.Accumulator, logger log.Logger) (err error) {
	...
	unwindToLimit, err := txc.Tx.(libstate.HasAggCtx).AggCtx().(*libstate.FilesRoTx).CanUnwindDomainsToBlockNum(txc.Tx)
	if err != nil {
		return err
	}
...
func PruneExecutionStage(s *PruneState, tx kv.RwTx, cfg ExecuteBlockCfg, ctx context.Context, initialCycle bool) (err error) {
	...
	if cfg.historyV3 {
		...
		if _, err = tx.(*temporal.Tx).AggCtx().(*libstate.FilesRoTx).PruneSmallBatches(ctx, pruneTimeout, tx); err != nil { // prune part of retired data, before commit
			return err
		}
	} else {
