...
func SpawnExecuteBlocksStage(s *StageState, u Unwinder, txc wrap.TxContainer, toBlock uint64, ctx context.Context, cfg ExecuteBlockCfg, logger log.Logger) (err error) {
	...
	if cfg.historyV3 {
		if err = ExecBlockV3(s, u, txc, toBlock, ctx, cfg, logger); err != nil {
			return err
		}
		return nil
	}
	...
	if !s.CurrentSyncCycle().IsOnChainTip && cfg.silkworm == nil { // block read-ahead is not compatible w/ Silkworm one-shot block execution
