...
func TestUnwindSomeStagesBehindUnwindPoint(t *testing.T) {
	...
	stageState, err := state.StageState(stages.Headers, tx, nil, true, false)
	assert.NoError(t, err)
	assert.Equal(t, 1500, int(stageState.BlockNumber))

	stageState, err = state.StageState(stages.Bodies, tx, nil, true, false)
	assert.NoError(t, err)
	assert.Equal(t, 1000, int(stageState.BlockNumber))

	stageState, err = state.StageState(stages.Senders, tx, nil, true, false)
	assert.NoError(t, err)
	assert.Equal(t, 1500, int(stageState.BlockNumber))
}
...
func TestUnwind(t *testing.T) {
	...
	stageState, err := state.StageState(stages.Headers, tx, nil, true /* initialCycle */, false)
	assert.NoError(t, err)
	assert.Equal(t, 500, int(stageState.BlockNumber))

	stageState, err = state.StageState(stages.Bodies, tx, nil, true /* initialCycle */, false)
	assert.NoError(t, err)
	assert.Equal(t, 500, int(stageState.BlockNumber))

	stageState, err = state.StageState(stages.Senders, tx, nil, true /* initialCycle */, false)
	assert.NoError(t, err)
	assert.Equal(t, 500, int(stageState.BlockNumber))
