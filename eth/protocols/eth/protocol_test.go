...
func TestGetBlockHeadersDataEncodeDecode(t *testing.T) {
	...
	for i, tt := range tests {
		bytes, err := rlp2.EncodeToBytes(tt.packet)
		if err != nil && !tt.fail {
		...
		if !tt.fail {
			...
			if err := rlp2.DecodeBytes(bytes, packet); err != nil {
...
func TestEth66EmptyMessages(t *testing.T) {
	...
	for i, msg := range []interface{}{
		...
		BlockBodiesRLPPacket66{1111, BlockBodiesRLPPacket([]rlp2.RawValue{})},
		// Receipts
		GetReceiptsPacket66{1111, GetReceiptsPacket([]libcommon.Hash{})},
		ReceiptsPacket66{1111, ReceiptsPacket([][]*types.Receipt{})},
	} {
		if have, _ := rlp2.EncodeToBytes(msg); !bytes.Equal(have, want) {
...
func TestEth66Messages(t *testing.T) {
	...
	var (
		...
		blockBodyRlp rlp2.RawValue
		txs          []types.Transaction
		hashes       []libcommon.Hash
		receipts     []*types.Receipt
		receiptsRlp  rlp2.RawValue
	...
	blockBodyRlp, err = rlp2.EncodeToBytes(blockBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	// init the receipts
	{
		...
		rlpData, err := rlp2.EncodeToBytes(receipts)
		if err != nil {
			t.Fatal(err)
		}
		receiptsRlp = rlpData
	...
	for i, tc := range []struct {
		...
		{ // Identical to non-rlp-shortcut version
			BlockBodiesRLPPacket66{1111, BlockBodiesRLPPacket([]rlp2.RawValue{blockBodyRlp})},
			common.FromHex(""),
		},
		{
		...
		{
			ReceiptsRLPPacket66{1111, ReceiptsRLPPacket([]rlp2.RawValue{receiptsRlp})},
			common.FromHex(""),
		},
	} {
		if have, _ := rlp2.EncodeToBytes(tc.message); !bytes.Equal(have, tc.want) {
