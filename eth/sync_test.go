...
func testFastSyncDisabling(t *testing.T, protocol int) {
	...
	pmEmpty, _, clear := newTestProtocolManagerMust(t, downloader.FastSync, 0, nil, nil)
	defer clear()
	if atomic.LoadUint32(&pmEmpty.fastSync) == 0 {
		t.Fatalf("fast sync disabled on pristine blockchain")
	}
	// Create a full protocol manager, check that fast sync gets disabled
	pmFull, _, clearFull := newTestProtocolManagerMust(t, downloader.FastSync, 1024, nil, nil)
	defer clearFull()
