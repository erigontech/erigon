	...
	if config.SyncMode == downloader.StagedSync && config.SnapshotMode != (snapshotsync.SnapshotMode{}) && config.NetworkID == params.MainnetChainConfig.ChainID.Uint64() {
		if config.ExternalSnapshotDownloaderAddr != "" {
			...
			if err == nil {
				...
				snapshotKV := chainDb.KV()
				mp, err:=torrentClient.GetSnapshots(chainDb, config.NetworkID)
				if err!=nil {
					return nil, err
				}

				snapshotKV, err = snapshotsync.WrapBySnapshots2(snapshotKV, mp)
				if err != nil {
					return nil, err
				}
				chainDb.SetKV(snapshotKV)
				err = snapshotsync.PostProcessing(chainDb, config.SnapshotMode, mp)
				if err != nil {
					return nil, err
				}
			} else {
