...
func New(ctx context.Context, stack *node.Node, config *ethconfig.Config, logger log.Logger) (*Ethereum, error) {
	...
	mining := stagedsync.New(
		...
		stagedsync.MiningStages(backend.sentryCtx,
			...
			stagedsync.StageBorHeimdallCfg(backend.chainDB, snapDb, miner, *backend.chainConfig, heimdallClient, backend.blockReader, nil, nil, nil, recents, signatures, false, nil), stagedsync.StageExecuteBlocksCfg(backend.chainDB, config.Prune, config.BatchSize, chainConfig, backend.engine, &vm.Config{}, backend.notifications.Accumulator, config.StateStream, false, dirs, blockReader, backend.sentriesClient.Hd, config.Genesis, config.Sync, agg, stages2.SilkwormForExecutionStage(backend.silkworm, config)),
	...
	assembleBlockPOS := func(param *core.BlockBuilderParameters, interrupt *int32) (*types.BlockWithReceipts, error) {
		...
		proposingSync := stagedsync.New(
			...
			stagedsync.MiningStages(backend.sentryCtx,
				...
				stagedsync.StageExecuteBlocksCfg(backend.chainDB, config.Prune, config.BatchSize, chainConfig, backend.engine, &vm.Config{}, backend.notifications.Accumulator, config.StateStream, false, dirs, blockReader, backend.sentriesClient.Hd, config.Genesis, config.Sync, agg, stages2.SilkwormForExecutionStage(backend.silkworm, config)),
				stagedsync.StageSendersCfg(backend.chainDB, chainConfig, config.Sync, false, dirs.Tmp, config.Prune, blockReader, backend.sentriesClient.Hd, loopBreakCheck),
