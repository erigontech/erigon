...
func New(stack *node.Node, config *ethconfig.Config, logger log.Logger) (*Ethereum, error) {
	...
	mining := stagedsync.New(
		stagedsync.MiningStages(backend.sentryCtx,
			...
			stagedsync.StageBorHeimdallCfg(backend.chainDB, snapDb, miner, *backend.chainConfig, heimdallClient, backend.blockReader),
	...
	assembleBlockPOS := func(param *core.BlockBuilderParameters, interrupt *int32) (*types.BlockWithReceipts, error) {
		...
		proposingSync := stagedsync.New(
			stagedsync.MiningStages(backend.sentryCtx,
				...
				stagedsync.StageBorHeimdallCfg(backend.chainDB, snapDb, miningStatePos, *backend.chainConfig, heimdallClient, backend.blockReader),
