...
// the genesis block.
func newSnapshot(config *params.CliqueConfig, snapStorage *storage, number uint64, hash common.Hash, time uint64, signers []common.Address) *Snapshot {
	snap := &Snapshot{
		config:      config,
		Number:      number,
		Time:        time,
...
func (s *Snapshot) apply(r *lru.ARCCache, headers ...*types.Header) error {
	...
	s.Time = headers[len(headers)-1].Time
