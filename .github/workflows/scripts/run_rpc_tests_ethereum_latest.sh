...
"$(dirname "$0")/run_rpc_tests.sh" mainnet canepat/int_tests_eth_simulate_contract_logs "$DISABLED_TESTS" "$WORKSPACE" "$RESULT_DIR" "latest" "$REFERENCE_HOST" "do-not-compare-error-message" "$DUMP_RESPONSE"
