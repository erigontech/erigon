...
DISABLED_TEST_LIST=(
   ...
   debug_traceBlockByNumber/test_43.json # flucky test 
...
"$(dirname "$0")/run_rpc_tests.sh" mainnet v1.109.0 "$DISABLED_TESTS" "$WORKSPACE" "$RESULT_DIR" "latest" "$REFERENCE_HOST" "do-not-compare-error-message" "$DUMP_RESPONSE"
