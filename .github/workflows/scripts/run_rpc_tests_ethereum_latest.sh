...
"$(dirname "$0")/run_rpc_tests.sh" mainnet lupin012/add_test_with_prestate_callTracer "$DISABLED_TESTS" "$WORKSPACE" "$RESULT_DIR" "latest" "$REFERENCE_HOST" "do-not-compare-error-message" "$DUMP_RESPONSE"
