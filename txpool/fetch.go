...
func (f *Fetch) receiveMessage(ctx context.Context, sentryClient sentry.SentryClient) error {
	...
	stream, err := sentryClient.Messages(streamCtx, &sentry.MessagesRequest{Ids: []sentry.MessageId{
		sentry.MessageId_NEW_POOLED_TRANSACTION_HASHES,
		sentry.MessageId_GET_POOLED_TRANSACTIONS,
		sentry.MessageId_TRANSACTIONS,
		sentry.MessageId_POOLED_TRANSACTIONS,
...
func (f *Fetch) handleInboundMessage(ctx context.Context, req *sentry.InboundMessage, sentryClient sentry.SentryClient) (err error) {
	...
	switch req.Id {
	case sentry.MessageId_NEW_POOLED_TRANSACTION_HASHES:
		...
		if len(unknownHashes) > 0 {
			...
			case sentry.MessageId_NEW_POOLED_TRANSACTION_HASHES:
				...
				messageID = sentry.MessageId_GET_POOLED_TRANSACTIONS
			default:
	...
	case sentry.MessageId_GET_POOLED_TRANSACTIONS:
		...
		switch req.Id {
		case sentry.MessageId_GET_POOLED_TRANSACTIONS:
			messageID = sentry.MessageId_POOLED_TRANSACTIONS
	...
	case sentry.MessageId_POOLED_TRANSACTIONS, sentry.MessageId_TRANSACTIONS:
		txs := types2.TxSlots{}
		...
		switch req.Id {
		case sentry.MessageId_TRANSACTIONS:
			if err := f.threadSafeParsePooledTxn(func(parseContext *types2.TxParseContext) error {
		...
		case sentry.MessageId_POOLED_TRANSACTIONS:
			if err := f.threadSafeParsePooledTxn(func(parseContext *types2.TxParseContext) error {
