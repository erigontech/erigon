// Package bn254 implements a particular bilinear group at the 128-bit security
// level.
//
...
func (e *G1) Unmarshal(m []byte) ([]byte, error) {
	...
	const numBytes = 256 / 8
	if len(m) < 2*numBytes {
		return nil, errors.New("bn254: not enough data")
	}
	...
	if e.p.x == zero && e.p.y == zero {
		...
		if !e.p.IsOnCurve() {
			return nil, errors.New("bn254: malformed point")
		}
	}
