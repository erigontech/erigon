...
// Package iouring is a minimal hand-rolled io_uring facility for batched file
// reads — enough to warm the page cache at a controlled queue depth. Not a
// general async reactor: BatchReadWarm submits a batch and waits for all of it.
package iouring
...
// Req is a single read request for a multi-file batch.
type Req struct {
	Fd  int
	Off int64
	Len int
...
// BatchReadMulti is like BatchReadWarm but each request carries its own fd, so a
// single deep wave can span multiple files. scratch must have >= ring capacity buffers.
func (r *Ring) BatchReadMulti(reqs []Req, scratch [][]byte) error {
	n := len(reqs)
	done := 0
	for done < n {
		...
		for wave < int(space) && done+wave < n {
			req := reqs[done+wave]
			ln := req.Len
			if ln > len(scratch[wave]) {
				ln = len(scratch[wave])
			}
			slot := tail & r.sqMask
			r.fillSQE(slot, req.Fd, uint64(req.Off), scratch[wave][:ln], uint64(done+wave))
			r.sqArray[slot] = slot
			tail++
			wave++
		}
		atomic.StoreUint32(r.sqTail, tail)
		if _, _, errno := unix.Syscall6(sysEnter, uintptr(r.fd), uintptr(wave), uintptr(wave), enterGetevents, 0, 0); errno != 0 {
		...
		for reaped < wave {
			...
			if reaped < wave {
				if _, _, errno := unix.Syscall6(sysEnter, uintptr(r.fd), 0, uintptr(wave-reaped), enterGetevents, 0, 0); errno != 0 {
