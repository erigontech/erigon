...
type InsertBatch struct {
	...
	payloadBuf []*types.Block   // A buffer for storing execution payloads before they are inserted.
...
func NewInsertBatch(ec *ExecutionClient) *InsertBatch {
	return &InsertBatch{
		ec:         ec,
		payloadBuf: make([]*types.Block, 0, batchSize),
	}
}
...
// execution client.
func (b *InsertBatch) WriteExecutionPayload(payload *types.Block) error {
	b.mu.Lock()
	defer b.mu.Unlock()
