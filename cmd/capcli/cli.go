...
func (c *Chain) Run(ctx *Context) error {
	...
	db, blobStorage, err := caplin1.OpenCaplinDatabase(ctx, beaconConfig, ethClock, dirs.CaplinIndexing, dirs.CaplinBlobs, nil, false)
	if err != nil {
		return err
	}
	defer db.Close()
...
func (c *ChainEndpoint) Run(ctx *Context) error {
	...
	dirs := datadir.New(c.Datadir)
	db, blobDB, err := caplin1.OpenCaplinDatabase(ctx, beaconConfig, ethClock, dirs.CaplinIndexing, dirs.CaplinBlobs, nil, false)
	if err != nil {
		return err
	}
	defer db.Close()
...
func (c *DumpSnapshots) Run(ctx *Context) error {
	...
	db, _, err := caplin1.OpenCaplinDatabase(ctx, beaconConfig, nil, dirs.CaplinIndexing, dirs.CaplinBlobs, nil, false)
	if err != nil {
		return err
	}
	var to uint64
...
func (b *BlobArchiveStoreCheck) Run(ctx *Context) error {
	...
	db, blobStorage, err := caplin1.OpenCaplinDatabase(ctx, beaconConfig, nil, dirs.CaplinIndexing, dirs.CaplinBlobs, nil, false)
	if err != nil {
		return err
	}
	defer db.Close()
...
func (c *DumpBlobsSnapshotsToStore) Run(ctx *Context) error {
	...
	db, blobStore, err := caplin1.OpenCaplinDatabase(ctx, beaconConfig, nil, dirs.CaplinIndexing, dirs.CaplinBlobs, nil, false)
	if err != nil {
		return err
	}
	tx, err := db.BeginRo(ctx)
