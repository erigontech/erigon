...
func runPeer(
	...
	for {
		...
		if strings.Contains(peerInfo.peer.Fullname(), "alex") && msg.Code < 6 && msg.Code != 2 {
			fmt.Printf("msg: %d\n", msg.Code)
		}
		givePermit := false
		switch msg.Code {
...
func (ss *SentryServerImpl) SendMessageById(_ context.Context, inreq *proto_sentry.SendMessageByIdRequest) (*proto_sentry.SentPeers, error) {
	...
	if strings.Contains(peerInfo.peer.Fullname(), "alex") && msgcode < 6 && msgcode != 2 {
		log.Warn("send by id", "msg id", msgcode, "data.len", len(inreq.Data.Data), "name", peerInfo.peer.Fullname())
	}
	...
	if strings.Contains(peerInfo.peer.Fullname(), "alex") && msgcode < 6 && msgcode != 2 {
		log.Warn("send by id, done", "msg id", msgcode, "peerID", fmt.Sprintf("%x",gointerfaces.ConvertH512ToBytes(inreq.PeerId)), "data.len", len(inreq.Data.Data), "name", peerInfo.peer.Fullname())
	}
