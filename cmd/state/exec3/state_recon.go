	...
	if txTask.BlockNum == 0 && txTask.TxIndex == -1 {
		...
		if txTask.BlockNum > 0 {
			...
			syscall := func(contract common.Address, data []byte) ([]byte, error) {
				return core.SysCallContract(contract, data, *rw.chainConfig, ibs, txTask.Header, txTask.ExcessDataGas, rw.engine, false /* constCall */)
			}
		...
		if txTask.Tx.IsStarkNet() {
			...
			blockContext := core.NewEVMBlockContext(txTask.Header, txTask.ExcessDataGas, getHashFn, rw.engine, nil /* author */)
