...
func speculativeExecution(blockNum uint64) {
	...
	for !interrupt {
		...
		for _, tx := range block.Transactions() {
			...
			vmenv := vm.NewEVM(context, statedb1, chainConfig, vmConfig1, false /* skipAnalysis */)
		...
		for _, tx := range block.Transactions() {
			...
			context := core.NewEVMContext(msg, block.Header(), bc, nil)
			// Not yet the searched for transaction, execute on top of the current state
			vmenv := vm.NewEVM(context, statedb2, chainConfig, vmConfig2, false /* skipAnalysis */)
			if _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(tx.Gas())); err != nil {
				panic(fmt.Errorf("tx %x failed: %v", tx.Hash(), err))
			}
		}
		...
		for _, tx := range block.Transactions() {
			...
			context := core.NewEVMContext(msg, block.Header(), bc, nil)
			// Not yet the searched for transaction, execute on top of the current state
			vmenv := vm.NewEVM(context, statedb3, chainConfig, vmConfig3, false /* skipAnalysis */)
			if _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(tx.Gas())); err != nil {
				panic(fmt.Errorf("tx %x failed: %v", tx.Hash(), err))
			}
		...
		for _, tx := range block.Transactions() {
			...
			context := core.NewEVMContext(msg, block.Header(), bc, nil)
			// Not yet the searched for transaction, execute on top of the current state
			vmenv := vm.NewEVM(context, statedb4, chainConfig, vmConfig2, false /* skipAnalysis */)
			if _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(tx.Gas())); err != nil {
				panic(fmt.Errorf("tx %x failed: %v", tx.Hash(), err))
			}
		}
