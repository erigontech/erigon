...
func testDynamicTx(t *testing.T, ctx context.Context) {
	...
	t.Run("PingErigonRpc", func(t *testing.T) {
		require.Nil(t, admin.PingErigonRpc(withTestLogger(ctx, t)))
	})
	t.Run("CheckTxPoolContent", func(t *testing.T) {
		transactions.CheckTxPoolContent(withTestLogger(ctx, t), 0, 0, 0)
	})
	t.Run("SendTxWithDynamicFee", func(t *testing.T) {
		...
		_, err := transactions.SendTxWithDynamicFee(withTestLogger(ctx, t), recipientAddress, accounts.DevAddress, sendValue)
		require.Nil(t, err)
	})
	t.Run("AwaitBlocks", func(t *testing.T) {
		require.Nil(t, transactions.AwaitBlocks(withTestLogger(ctx, t), 2*time.Second))
	})
}
...
func TestCallContract(t *testing.T) {
	...
	t.Run("DeployAndCallLogSubscriber", func(t *testing.T) {
		_, err := contracts_steps.DeployAndCallLogSubscriber(withTestLogger(ctx, t), accounts.DevAddress)
		require.Nil(t, err)
	})
}
