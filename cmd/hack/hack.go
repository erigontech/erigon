...
func resetState(chaindata string) {
	...
	check(err)
	err = db.DeleteBucket(dbutils.PlainStateBucket)
	check(err)
	err = db.DeleteBucket(dbutils.PlainAccountChangeSetBucket)
	check(err)
	err = db.DeleteBucket(dbutils.PlainStorageChangeSetBucket)
	check(err)
