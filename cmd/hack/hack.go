...
func extractTrie(block int) {
	...
	tds, err := state.NewTrieDbState(baseBlock.Root(), stateDb, baseBlock.NumberU64(), bc.Config().WithEIPsEnabledCTX(context.Background(), baseBlock.Number()))
	check(err)
	startTime := time.Now()
	tds.Rebuild()
...
func testRewind(block, rewind int) {
	...
	db := ethDb.NewBatch()
	defer db.Rollback()
	tds, err := state.NewTrieDbState(baseBlock.Root(), db, baseBlockNr, bc.Config().WithEIPsEnabledCTX(context.Background(), baseBlock.Number()))
