...
func mgrSchedule(chaindata string, block uint64) {
	...
	bc, err := core.NewBlockChain(db, nil, params.MainnetChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil, dests)
	check(err)
	defer db.Close()
...
func execToBlock(chaindata string, block uint64, fromScratch bool) {
	...
	bcb, err := core.NewBlockChain(blockDb, nil, params.MainnetChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil, dests)
	check(err)
	defer blockDb.Close()
	if fromScratch {
		os.Remove("statedb")
	...
	check(err)
	bc, err := core.NewBlockChain(stateDB, nil, params.MainnetChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil, dests)
	check(err)
	tds, err := bc.GetTrieDbState()
	check(err)
...
func extractTrie(block int) {
	...
	defer stateDb.Close()
	dests := vm.NewDestsCache(100)
	bc, err := core.NewBlockChain(stateDb, nil, params.RopstenChainConfig, ethash.NewFaker(), vm.Config{}, nil, nil, dests)
	check(err)
