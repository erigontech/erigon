...
func (api *BaseAPI) getReceipts(ctx context.Context, tx kv.Tx, chainConfig *params.ChainConfig, block *types.Block, senders []common.Address) (types.Receipts, error) {
	...
	for i, txn := range block.Transactions() {
		...
		receipt, _, err := core.ApplyTransaction(chainConfig, core.GetHashFn(block.Header(), getHeader), ethashFaker, nil, gp, ibs, noopWriter, block.Header(), txn, usedGas, vm.Config{}, contractHasTEVM)
		if err != nil {
			return nil, err
		}
