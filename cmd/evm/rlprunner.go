...
var rlpTestCommand = cli.Command{
	Action:    rlpTestCmd,
	Name:      "rlptest",
	Usage:     "Executes RLP tests.",
	ArgsUsage: "<path>",
	Flags: []cli.Flag{
		&JSONOutputFlag,
		&RunFlag,
...
func rlpTestCmd(_ context.Context, ctx *cli.Command) error {
	path := ctx.Args().First()
	if path == "" {
...
func runRLPTest(path string, filter testFilter) ([]testResult, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}
	var tests map[string]testutil.RLPTest
