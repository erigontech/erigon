...
// scope is what the package makes available to the emitted code: its types, and the encoders
// blanking removed for the duration of this run.
type scope struct {
	pkg     *types.Package
...
func run(typeName, dir, out, computed string) error {
	...
	pkg, blanked, err := load(dir, path)
	if err != nil {
		return err
	}
	sc := scope{pkg: pkg.Types, blanked: blanked}
	...
	if err := writeFields(sc, &body, st, "x", map[string]struct{}{}); err != nil {
	...
	if computed != "" {
		if !declares(sc.pkg, obj.Type(), computed, 0) {
...
// sourceOfTruth names the type a generated file belongs to, and is what blanking reads it from.
const sourceOfTruth = "// The source of truth is "

const header = marker + ` DO NOT EDIT.
` + sourceOfTruth + `%[2]s's json and ethjson tags.
...
func load(dir, out string) (*packages.Package, map[string]bool, error) {
	name, err := packageName(dir)
	if err != nil {
		return nil, nil, err
	}
	overlay, blanked, err := blankGenerated(dir, out, name)
	if err != nil {
		return nil, nil, err
	}
	cfg := &packages.Config{
	...
	if len(pkgs) != 1 {
		return nil, nil, fmt.Errorf("%s holds %d packages, want 1", dir, len(pkgs))
	}
	...
	return pkgs[0], blanked, nil
}

// blankGenerated maps the file being written, and every sibling this tool wrote, to a bare
// package clause.
func blankGenerated(dir, out, name string) (map[string][]byte, map[string]bool, error) {
	...
	for _, e := range entries {
		...
		overlay[abs] = blank
		if t := typeInHeader(content); t != "" {
			blanked[t] = true
		}
	}
	return overlay, blanked, nil
}

// typeInHeader reads back the type a generated file belongs to. Its encoder is blanked with the
// file, so this is what still lets a field of that type be written by it.
func typeInHeader(content []byte) string {
	for _, line := range strings.Split(string(content), "\n") {
		if rest, ok := strings.CutPrefix(line, sourceOfTruth); ok {
			name, _, _ := strings.Cut(rest, "'s ")
			return name
		}
	}
	return ""
...
func writeFields(sc scope, w *bytes.Buffer, st *types.Struct, recv string, written map[string]struct{}) error {
	for i := range st.NumFields() {
		...
		if f.Embedded() {
			...
			if err := writeFields(sc, w, embedded, recv+"."+f.Name(), written); err != nil {
				return err
			}
			continue
		}
		...
		stmt, err := fieldStatement(sc, recv+"."+f.Name(), name, tag.Get("ethjson"), f.Type(), omitempty)
		if err != nil {
...
func fieldStatement(sc scope, ref, name, form string, t types.Type, omitempty bool) (string, error) {
	...
	case "bool":
		...
		present = ref
	case "objects":
		// A sibling's encoder is blanked for this run, so the type it belongs to is accepted
		// on the file's word that it has one. The receiver is a pointer, which an addressable
		// field satisfies either way.
		if !declares(sc.pkg, bare, method, 1) && !sc.willGenerate(bare) {
...
// willGenerate reports a type in this package whose encoder blanking removed.
func (sc scope) willGenerate(t types.Type) bool {
	named, ok := types.Unalias(t).(*types.Named)
	return ok && named.Obj().Pkg() == sc.pkg && sc.blanked[named.Obj().Name()]
}
