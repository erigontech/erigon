...
func run(typeName, dir, out string) error {
	...
	fmt.Fprintf(&file, header, pkg, typeName, body.String(), "\t"+strings.Join(imports, "\n\t"))
...
func parsePackage(dir string) (string, map[string]*ast.StructType, error) {
	...
	for pkgName, pkg := range pkgs {
		...
		for _, f := range pkg.Files {
			// Only package-level declarations: a type declared inside a function body, as
			// the generated MarshalJSON mirrors are, must not shadow the real one.
...
func writeFields(w *bytes.Buffer, st *ast.StructType, structs map[string]*ast.StructType, recv string, written map[string]struct{}, used map[string]bool) error {
	for _, f := range st.Fields.List {
		...
		for _, id := range f.Names {
			...
			if name == "" {
				name = id.Name // what encoding/json falls back to
			}
