...
var latestHash = rpc.BlockNumberOrHashWithHash(common.BytesToHash(latestTag.Bytes()), false)
...
func TestParityAPIImpl_ListStorageKeys_NoOffset(t *testing.T) {
	...
	result, err := api.ListStorageKeys(context.Background(), addr, 5, nil, latestHash)
	if err != nil {
...
func TestParityAPIImpl_ListStorageKeys_WithOffset_ExistingPrefix(t *testing.T) {
	...
	result, err := api.ListStorageKeys(context.Background(), addr, 5, &b, latestHash)
	if err != nil {
		t.Errorf("calling ListStorageKeys: %v", err)
	}
	assert.Equal(len(answers), len(result))
...
func TestParityAPIImpl_ListStorageKeys_WithOffset_NonExistingPrefix(t *testing.T) {
	...
	b := hexutil.Bytes(offset)
	result, err := api.ListStorageKeys(context.Background(), addr, 2, &b, latestHash)
	if err != nil {
		t.Errorf("calling ListStorageKeys: %v", err)
	}
	assert.Equal(len(answers), len(result))
...
func TestParityAPIImpl_ListStorageKeys_AccNotFound(t *testing.T) {
	...
	_, err := api.ListStorageKeys(context.Background(), addr, 2, nil, latestHash)
