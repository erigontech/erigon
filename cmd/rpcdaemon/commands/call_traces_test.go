...
func TestCallTraceOneByOne(t *testing.T) {
	...
	api := NewTraceAPI(
		NewBaseApi(nil, kvcache.New(kvcache.DefaultCoherentConfig), snapsync.NewBlockReader(), false),
...
func TestCallTraceUnwind(t *testing.T) {
	...
	api := NewTraceAPI(NewBaseApi(nil, kvcache.New(kvcache.DefaultCoherentConfig), snapsync.NewBlockReader(), false), m.DB, &httpcfg.HttpCfg{})
