...
func TestCallTraceOneByOne(t *testing.T) {
	...
	api := NewTraceAPI(
		NewBaseApi(nil, kvcache.New(kvcache.DefaultCoherentConfig), services.NewBlockReader(), false),
		m.DB, &cli.Flags{})
...
func TestCallTraceUnwind(t *testing.T) {
	...
	api := NewTraceAPI(NewBaseApi(nil, kvcache.New(kvcache.DefaultCoherentConfig), services.NewBlockReader(), false), m.DB, &cli.Flags{})
