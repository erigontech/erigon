...
func (api *APIImpl) Call(ctx context.Context, args ethapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *map[common.Address]ethapi.Account) (hexutil.Bytes, error) {
	...
	result, err := transactions.DoCall(ctx, args, ethdb.NewRoTxDb(dbtx), blockNrOrHash, overrides, api.GasCap, chainConfig, api.pending)
	if err != nil {
		return nil, err
	}
...
func (api *APIImpl) EstimateGas(ctx context.Context, args ethapi.CallArgs) (hexutil.Uint64, error) {
	...
	executable := func(gas uint64) (bool, *core.ExecutionResult, error) {
		...
		result, err := transactions.DoCall(ctx, args, ethdb.NewRoTxDb(dbtx), rpc.BlockNumberOrHash{BlockNumber: &lastBlockNum}, nil, api.GasCap, chainConfig, api.pending)
		if err != nil {
