...
func stageBorHeimdall(db kv.RwDB, ctx context.Context, unwindTypes []string, logger log.Logger) error {
	...
	return db.Update(ctx, func(tx kv.RwTx) error {
		...
		if unwind > 0 {
			...
			unwindState := sync.NewUnwindState(stages.BorHeimdall, stageState.BlockNumber-unwind, stageState.BlockNumber, false, false)
...
func stageBodies(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if err := db.Update(ctx, func(tx kv.RwTx) error {
		...
		if unwind > 0 {
			...
			u := sync.NewUnwindState(stages.Bodies, s.BlockNumber-unwind, s.BlockNumber, false, false)
...
func stageSenders(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if unwind > 0 {
		u := sync.NewUnwindState(stages.Senders, s.BlockNumber-unwind, s.BlockNumber, false, false)
		if err = stagedsync.UnwindSendersStage(u, tx, cfg, ctx); err != nil {
			return err
		}
	} else if pruneTo > 0 {
		p, err := sync.PruneStageState(stages.Senders, s.BlockNumber, tx, db, false, false)
		if err != nil {
			return err
		}
...
func stageExec(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if unwind > 0 {
		u := sync.NewUnwindState(stages.Execution, s.BlockNumber-unwind, s.BlockNumber, false, false)
	...
	if pruneTo > 0 {
		p, err := sync.PruneStageState(stages.Execution, s.BlockNumber, tx, db, false, false)
		if err != nil {
			return err
		}
...
func stageCustomTrace(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if unwind > 0 {
		u := sync.NewUnwindState(stages.CustomTrace, s.BlockNumber-unwind, s.BlockNumber, false, false)
	...
	if pruneTo > 0 {
		p, err := sync.PruneStageState(stages.CustomTrace, s.BlockNumber, tx, db, false, false)
		if err != nil {
			return err
		}
	...
	err := stagedsync.SpawnCustomTrace(s, txc, cfg, ctx, 0, logger)
	if err != nil {
		return err
	}
...
func stageTrie(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if unwind > 0 {
		u := sync.NewUnwindState(stages.IntermediateHashes, s.BlockNumber-unwind, s.BlockNumber, false, false)
		if err := stagedsync.UnwindIntermediateHashesStage(u, s, tx, cfg, ctx, logger); err != nil {
			return err
		}
	} else if pruneTo > 0 {
		p, err := sync.PruneStageState(stages.IntermediateHashes, s.BlockNumber, tx, db, false, false)
		if err != nil {
			return err
		}
...
func stageTxLookup(db kv.RwDB, ctx context.Context, logger log.Logger) error {
	...
	if unwind > 0 {
		u := sync.NewUnwindState(stages.TxLookup, s.BlockNumber-unwind, s.BlockNumber, false, false)
		err = stagedsync.UnwindTxLookup(u, s, tx, cfg, ctx, logger)
		if err != nil {
			return err
		}
	} else if pruneTo > 0 {
		p, err := sync.PruneStageState(stages.TxLookup, s.BlockNumber, tx, nil, false, false)
		if err != nil {
			return err
		}
...
func stage(st *stagedsync.Sync, tx kv.Tx, db kv.RoDB, stage stages.SyncStage) *stagedsync.StageState {
	res, err := st.StageState(stage, tx, db, false, false)
	if err != nil {
		panic(err)
	}
	return res
