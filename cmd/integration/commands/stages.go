...
func newSync(ctx context.Context, db kv.RwDB, miningConfig *params.MiningConfig, logger log.Logger) (consensus.Engine, *vm.Config, *stagedsync.Sync, *stagedsync.Sync, stagedsync.MiningState) {
	...
	sentryChief, err := sentry.NewChief(
		db,
		"",
		chainConfig,
		genesisBlock.Hash(),
	...
	stages := stages2.NewDefaultStages(context.Background(), db, snapDb, p2p.Config{}, &cfg, sentryChief, notifications, nil, blockReader, blockRetire, agg, nil, nil,
