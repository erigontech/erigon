...
func stageExec(db kv.TemporalRwDB, ctx context.Context, logger log.Logger) error {
	...
	if chainTipMode {
		...
		for bn := initialExecProgress; bn < block; bn++ {
			...
			if doms.SizeEstimate() < uint64(batchSize/2) {
				...
				case <-logEvery.C:
					took := time.Since(t)
					log.Info("[stage_exec] progress", "block_num", s.BlockNumber, "blk/sec", float64(bn-initialExecProgress)/took.Seconds(), "batch", datasize.ByteSize(doms.SizeEstimate()).String())
				default:
				}
	...
	log.Info("[stage_exec] starting", "LoopBlockLimit", syncCfg.LoopBlockLimit)
