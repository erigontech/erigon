...
func syncBySmallSteps(db kv.RwDB, miningConfig params.MiningConfig, ctx context.Context) error {
	...
	execCfg := stagedsync.StageExecuteBlocksCfg(db, pm, batchSize, changeSetHook, chainConfig, engine, vmConfig, nil, false, tmpDir, 1, getBlockReader(chainConfig))
...
func loopExec(db kv.RwDB, ctx context.Context, unwind uint64) error {
	...
	cfg := stagedsync.StageExecuteBlocksCfg(db, pm, batchSize, nil, chainConfig, engine, vmConfig, nil, false, tmpdir, 1, getBlockReader(chainConfig))
