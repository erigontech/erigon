...
func loopIh(db kv.RwDB, ctx context.Context, unwind uint64) error {
	...
	if err = stagedsync.UnwindIntermediateHashesStage(u, stage(sync, tx, nil, stages.IntermediateHashes), tx, stagedsync.StageTrieCfg(db, true, true, false, dirs.Tmp, getBlockReader(db), nil, nil /* snapshot downloader */), ctx); err != nil {
		return err
	}
	must(tx.Commit())
	tx, err = db.BeginRw(ctx)
