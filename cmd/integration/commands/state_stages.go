...
func syncBySmallSteps(db kv.RwDB, miningConfig params.MiningConfig, ctx context.Context, logger1 log.Logger) error {
	...
	execUntilFunc := func(execToBlock uint64) func(badBlockUnwind bool, stageState *stagedsync.StageState, unwinder stagedsync.Unwinder, txc wrap.TxContainer, logger log.Logger) error {
	...
	for (!backward && execAtBlock < stopAt) || (backward && execAtBlock > stopAt) {
		...
		if _, err := stateStages.Run(db, wrap.TxContainer{Tx: tx}, false /* firstCycle */, true); err != nil {
			return err
		}
		...
		if miner.MiningConfig.Enabled && nextBlock != nil && nextBlock.Coinbase() != (common2.Address{}) {
			...
			if _, err := miningStages.Run(db, wrap.TxContainer{Tx: tx}, false /* firstCycle */, true); err != nil {
				return err
			}
			tx.Rollback()
			tx, err = db.BeginRw(context.Background())
...
func loopIh(db kv.RwDB, ctx context.Context, unwind uint64, logger log.Logger) error {
	...
	if _, err = sync.Run(db, wrap.TxContainer{Tx: tx}, false /* firstCycle */, true); err != nil {
		return err
	}
...
func loopExec(db kv.RwDB, ctx context.Context, unwind uint64, logger log.Logger) error {
	...
	firstSyncCycle, isOnChainTip := false, true
	br, _ := blocksIO(db, logger)
	...
	for {
		...
		t := time.Now()
		if _, err = sync.Run(db, wrap.TxContainer{Tx: tx}, firstSyncCycle, isOnChainTip); err != nil {
			return err
		}
