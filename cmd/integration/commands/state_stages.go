...
func syncBySmallSteps(db kv.RwDB, miningConfig params.MiningConfig, ctx context.Context, logger1 log.Logger) error {
	...
	execCfg := stagedsync.StageExecuteBlocksCfg(db, pm, batchSize, chainConfig, engine, vmConfig, changesAcc, false, true, false, dirs, br, nil, genesis, syncCfg, agg, nil)
...
func loopExec(db kv.RwDB, ctx context.Context, unwind uint64, logger log.Logger) error {
	...
	cfg := stagedsync.StageExecuteBlocksCfg(db, pm, batchSize, chainConfig, engine, vmConfig, nil, false, true, false, dirs, br, nil, genesis, syncCfg, agg, nil)
