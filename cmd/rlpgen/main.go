...
const import2 = `import (
	"io"
	"github.com/erigontech/erigon-lib/rlp"
	"github.com/erigontech/erigon-lib/rlp2"
)
...
func main() {
	...
	fmt.Fprint(&header, headerMsg)
	fmt.Fprintf(&header, "package %s\n\n", ps[1].Types.Name())
	if requiresRLP2 {
		fmt.Fprint(&header, import2)
	} else {
		fmt.Fprint(&header, import1)
	}

	if *output == "-" {
		os.Stdout.Write(append(header.Bytes(), code.Bytes()...))
	} else if err := os.WriteFile(*output, append(header.Bytes(), code.Bytes()...), 0600); err != nil {
		_exit(err.Error())
	}
}
...
func process(pkgRLP *types.Package, pkgSrc *types.Package, typename *string, b *bytes.Buffer) error {
	...
	// 2. start EncodingSize method on a struct
	fmt.Fprintf(b, "func (obj *%s) EncodingSize2() (size int) {\n", typ.Obj().Name())

	// 3. add encoding size for every field in a struct
	addRlpEncodingSizes(b, typ)

	// 4. end EncodingSize method
	fmt.Fprintf(b, "    return\n}\n\n")

	// 5. start EncodeELP method
	fmt.Fprintf(b, "func (obj *%s) EncodeRLP2(w io.Writer) error {\n", typ.Obj().Name())

	// 6. add [32]byte
	fmt.Fprintf(b, "    var b [32]byte\n")

	// 7. encode fields of a struct
	addRlpEncodings(b, typ)

	// 8. end EncodeRLP method
...
func addRlpEncodings(b *bytes.Buffer, typ *types.Named) {
	fmt.Fprintf(b, "    if err := EncodeStructSizePrefix(obj.EncodingSize2(), w, b[:]); err != nil {\n")
