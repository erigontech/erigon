...
// Hash: da3a3d6c67cb5833a560a9ef835577a0c5d5f5fa4f0858a9587c82bd1929d237
// Version: 0.1.2
package lightrpc
...
func (l *LightClientBootstrap) UnmarshalSSZ(buf []byte) error {
	...
	// Offset (2) 'CurrentSyncCommitteeBranch'
	if o2 = ssz.ReadOffset(buf[24688:24692]); o2 > size {
		return ssz.ErrOffset
	}
	...
	// Field (2) 'CurrentSyncCommitteeBranch'
	{
		...
		l.CurrentSyncCommitteeBranch = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(l.CurrentSyncCommitteeBranch[ii]) == 0 {
				l.CurrentSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			l.CurrentSyncCommitteeBranch[ii] = append(l.CurrentSyncCommitteeBranch[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}
...
func (l *LightClientUpdate) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.FinalizedHeader.UnmarshalSSZ(buf[24692:24804]); err != nil {
		return err
	}

	// Offset (4) 'FinalityBranch'
	if o4 = ssz.ReadOffset(buf[24804:24808]); o4 > size || o2 > o4 {
		return ssz.ErrOffset
	}
	...
	if err = l.SyncAggregate.UnmarshalSSZ(buf[24808:24968]); err != nil {
		return err
	}

	// Field (6) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint64(buf[24968:24976])
	...
	// Field (2) 'NextSyncCommitteeBranch'
	{
		...
		l.NextSyncCommitteeBranch = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(l.NextSyncCommitteeBranch[ii]) == 0 {
				l.NextSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			l.NextSyncCommitteeBranch[ii] = append(l.NextSyncCommitteeBranch[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	...
	// Field (4) 'FinalityBranch'
	{
		...
		for ii := 0; ii < num; ii++ {
			if cap(l.FinalityBranch[ii]) == 0 {
				l.FinalityBranch[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			l.FinalityBranch[ii] = append(l.FinalityBranch[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}
...
func (l *LightClientFinalityUpdate) UnmarshalSSZ(buf []byte) error {
	...
	// Field (4) 'SignatureSlot'
	l.SignatureSlot = ssz.UnmarshallUint64(buf[388:396])
