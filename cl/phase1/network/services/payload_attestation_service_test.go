...
func TestPayloadAttestationServiceMultiplePendingForSameBlock(t *testing.T) {
	...
	// Add both as pending
	service.pending.jobs.Store(pendingPayloadAttestationKey{blockRoot, 1}, &pendingJob[*cltypes.PayloadAttestationMessage]{
		msg:          msg1,
		creationTime: time.Now(),
	})
	service.pending.jobs.Store(pendingPayloadAttestationKey{blockRoot, 2}, &pendingJob[*cltypes.PayloadAttestationMessage]{
		msg:          msg2,
		creationTime: time.Now(),
	})
