...
func (b *BlobHistoryDownloader) collectIncompleteBlocks(currentSlot, targetSlot, fuluRetentionFloor uint64) (batch []*cltypes.SignedBeaconBlock, visited uint64, err error) {
	tx, err := b.indiciesDB.BeginRo(b.ctx)
	if err != nil {
		return nil, 0, err
	}
