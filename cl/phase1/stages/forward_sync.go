	...
	for _, block := range blocks {
		...
		if block.Version() >= clparams.GloasVersion {
			if env, ok := envelopes[blockRoot]; ok {
				...
				if fceErr := cfg.forkChoice.OnExecutionPayload(ctx, env, false, canValidateGloasPayloads(cfg)); fceErr != nil && !errors.Is(fceErr, forkchoice.ErrExecutionPayloadAlreadyStored) {
...
func forwardSync(ctx context.Context, logger log.Logger, cfg *Cfg, args Args) error {
	...
	lastGloasVerification := time.Time{}
