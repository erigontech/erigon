		...
		Stages: map[string]clstages.Stage[*Cfg, Args]{
			...
			DownloadHistoricalBlocks: {
				...
				ActionFunc: func(ctx context.Context, logger log.Logger, cfg *Cfg, args Args) error {
					...
					if err := SpawnStageHistoryDownload(StageHistoryReconstruction(downloader, cfg.beaconDB, cfg.indiciesDB, cfg.executionClient, cfg.genesisCfg, cfg.beaconCfg, 0, startingRoot, startingSlot, cfg.dirs.Tmp, logger), ctx, logger); err != nil {
						downloaded = false
						return err
					}
					return nil
