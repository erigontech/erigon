...
func (p *PendingPartialWithdrawal) EncodeSSZ(buf []byte) ([]byte, error) {
	return ssz.MarshalSSZ(buf, &p.ValidatorIndex, &p.Amount, &p.WithdrawableEpoch)
}

func (p *PendingPartialWithdrawal) DecodeSSZ(buf []byte, version int) error {
	return ssz.UnmarshalSSZ(buf, version, &p.ValidatorIndex, &p.Amount, &p.WithdrawableEpoch)
}
