...
func runTest(t *testing.T, blocks []*cltypes.SignedBeaconBlock, preState, postState *state.CachingBeaconState) {
	...
	f := afero.NewMemMapFs()
	a := NewAntiquary(ctx, preState, vt, &clparams.MainnetBeaconConfig, datadir.New("/tmp"), nil, db, nil, reader, nil, log.New(), true, true, f, nil)
...
func TestStateAntiquaryCapella(t *testing.T) {
	if ethconfig.EnableHistoryV3InTest {
		t.Skip("TODO: fix me `makeslice: len out of range`")
	}
