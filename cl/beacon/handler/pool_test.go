...
func TestPoolVoluntaryExitsValidationResult(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "retained duplicate", validationErr: services.ErrIgnore, stored: voluntaryExit, status: http.StatusBadRequest},
		{name: "wrapped retained duplicate", validationErr: fmt.Errorf("validation: %w", services.ErrIgnore), stored: voluntaryExit, status: http.StatusBadRequest},
...
func TestPoolVoluntaryExitsDoesNotRepublishSuccessfulDuplicate(t *testing.T) {
	voluntaryExit := &cltypes.SignedVoluntaryExit{VoluntaryExit: &cltypes.VoluntaryExit{Epoch: 1, ValidatorIndex: 3}}
	_, _, _, _, _, handler, _, _, _, _ := setupTestingHandler(t, clparams.Phase0Version, log.Root(), false)
	ctrl := gomock.NewController(t)
	service := mock_services.NewMockVoluntaryExitService(ctrl)
	service.EXPECT().ProcessMessage(gomock.Any(), nil, gomock.Any()).Return(nil).Times(2)
	handler.voluntaryExitService = service
	encodedSSZ, err := voluntaryExit.EncodeSSZ(nil)
	require.NoError(t, err)
	gossipManager := gossip_mock.NewMockGossip(ctrl)
	gossipManager.EXPECT().Publish(gomock.Any(), gossip.TopicNameVoluntaryExit, encodedSSZ).Return(nil).Times(1)
	handler.gossipManager = gossipManager
	body, err := json.Marshal(voluntaryExit)
	require.NoError(t, err)
...
func TestPoolVoluntaryExitsPublishHasIndependentTimeout(t *testing.T) {
	voluntaryExit := &cltypes.SignedVoluntaryExit{VoluntaryExit: &cltypes.VoluntaryExit{Epoch: 1, ValidatorIndex: 3}}
	_, _, _, _, _, handler, _, _, _, _ := setupTestingHandler(t, clparams.Phase0Version, log.Root(), false)
	handler.voluntaryExitPublishTimeout = 10 * time.Millisecond
	ctrl := gomock.NewController(t)
	service := mock_services.NewMockVoluntaryExitService(ctrl)
	...
	require.NoError(t, err)
	gossipManager := gossip_mock.NewMockGossip(ctrl)
	gossipManager.EXPECT().Publish(gomock.Any(), gossip.TopicNameVoluntaryExit, encodedSSZ).DoAndReturn(func(ctx context.Context, _ string, _ []byte) error {
		<-ctx.Done()
		return ctx.Err()
	})
	handler.gossipManager = gossipManager
...
func TestPoolVoluntaryExitsPublishesAfterRequestCancellationDuringValidation(t *testing.T) {
	voluntaryExit := &cltypes.SignedVoluntaryExit{VoluntaryExit: &cltypes.VoluntaryExit{Epoch: 1, ValidatorIndex: 3}}
	_, _, _, _, _, handler, _, _, _, _ := setupTestingHandler(t, clparams.Phase0Version, log.Root(), false)
	ctrl := gomock.NewController(t)
	ctx, cancel := context.WithCancel(t.Context())
	service := mock_services.NewMockVoluntaryExitService(ctrl)
	service.EXPECT().ProcessMessage(gomock.Any(), nil, gomock.Any()).DoAndReturn(func(context.Context, *uint64, *services.SignedVoluntaryExitForGossip) error {
		cancel()
		return nil
	})
	handler.voluntaryExitService = service
	...
	gossipManager := gossip_mock.NewMockGossip(ctrl)
	gossipManager.EXPECT().Publish(gomock.Any(), gossip.TopicNameVoluntaryExit, encodedSSZ).DoAndReturn(func(context.Context, string, []byte) error {
		close(published)
		return nil
	})
	handler.gossipManager = gossipManager
	body, err := json.Marshal(voluntaryExit)
	require.NoError(t, err)
	req := httptest.NewRequestWithContext(ctx, http.MethodPost, "/eth/v1/beacon/pool/voluntary_exits", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	handler.mux.ServeHTTP(httptest.NewRecorder(), req)
	select {
	case <-published:
	case <-time.After(time.Second):
		t.Fatal("voluntary exit was stored but not published")
	}
}
...
func TestPoolVoluntaryExitsRetryFailedPublish(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "failed publish", epoch: 1000, publishErr: errors.New("temporary publish failure"), status: http.StatusOK, secondPublishes: 1, clockCalls: 2},
		{name: "alternate epoch", epoch: 999, status: http.StatusBadRequest, clockCalls: 1},
		{name: "absent pool entry", epoch: 1000, prune: true, status: http.StatusOK, clockCalls: 2},
		{name: "invalid signature", epoch: 1000, invalidSignature: true, status: http.StatusBadRequest, clockCalls: 1},
		{name: "future epoch", epoch: 1001, status: http.StatusBadRequest, clockCalls: 1},
		{name: "initiated exit", epoch: 1000, initiated: true, status: http.StatusBadRequest, clockCalls: 2},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, _, _, _, head, handler, opPool, syncedData, _, _ := setupTestingHandler(t, clparams.BellatrixVersion, log.Root(), true)
			cfg := handler.beaconChainCfg
			...
			validator := solid.NewValidatorFromParameters(publicKey, common.Hash{}, 0, false, 0, 0, cfg.FarFutureEpoch, cfg.FarFutureEpoch)
			head.ValidatorSet().Set(0, validator)
			require.NoError(t, syncedData.OnHeadState(head))
			ctrl := gomock.NewController(t)
			clock := eth_clock.NewMockEthereumClock(ctrl)
			clock.EXPECT().GetSlotTime(uint64(0)).Return(time.Unix(0, 0)).Times(tc.clockCalls)
			clock.EXPECT().GetSlotByTime(gomock.Any()).Return(1000 * cfg.SlotsPerEpoch).Times(tc.clockCalls)
			clock.EXPECT().GetEpochAtSlot(1000 * cfg.SlotsPerEpoch).Return(1000).Times(tc.clockCalls)
			handler.voluntaryExitService = services.NewVoluntaryExitService(opPool, beaconevents.NewEventEmitter(), syncedData, cfg, clock, services.NewBatchSignatureVerifier(t.Context(), nil))
			gossipManager := gossip_mock.NewMockGossip(ctrl)
			handler.gossipManager = gossipManager
			for attempt := range 2 {
				...
				if attempt == 1 {
					exit.VoluntaryExit.Epoch = tc.epoch
					status = tc.status
					publishErr = nil
					if tc.prune {
						require.True(t, opPool.VoluntaryExitsPool.DeleteIfExist(0))
					}
					if tc.initiated {
						validator.SetExitEpoch(1001)
						head.ValidatorSet().Set(0, validator)
						require.NoError(t, syncedData.OnHeadState(head))
					}
				}
				...
				if attempt == 1 && tc.invalidSignature {
					exit.Signature = common.Bytes96{}
				}
				encodedSSZ, err := exit.EncodeSSZ(nil)
				require.NoError(t, err)
				publishes := 1
				if attempt == 1 {
					publishes = tc.secondPublishes
				}
				gossipManager.EXPECT().Publish(gomock.Any(), gossip.TopicNameVoluntaryExit, encodedSSZ).Return(publishErr).Times(publishes)
				...
				response := httptest.NewRecorder()
				handler.mux.ServeHTTP(response, req)
				require.Equal(t, status, response.Code, "attempt %d: %s", attempt+1, response.Body.String())
				if attempt == 1 && tc.prune {
