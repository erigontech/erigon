...
import (
	...
	"github.com/erigontech/erigon/txnprovider/pool3/router"
	// "github.com/erigontech/erigon/txnprovider/txpool"
)

// Fetcher connects to sentry and implements eth/66 protocol regarding the transaction
...
func (f *Fetcher) ConnectSentries() {
...
func (f *Fetcher) receiveMessageLoop(sentryClient sentry.SentryClient) {
	for {
		select {
		case <-f.ctx.Done():
			return
...
func (f *Fetcher) receiveMessage(ctx context.Context, sentryClient sentry.SentryClient) error {
...
// func (f *Fetcher) handleInboundMessage(ctx context.Context, req *sentry.InboundMessage, sentryClient sentry.SentryClient) (err error) {
// 	defer func() {
...
// 		}
// 		hashes := make([]common.Hash, 32*hashCount)
// 		for i := 0; i < len(hashes); i += 32 {
// 			if _, pos, err = txpool.ParseHash(req.Data, pos, hashes[i:]); err != nil {
// 				return err
// 			}
// 		}
// 		unknownHashes := f.poolRouter.FilterKnownIdHashes(hashes)
// 		if err != nil {
// 			return err
// 		}
...
// 			if encodedRequest, err = txpool.EncodeGetPooledTransactions66(unknownHashes, uint64(1), nil); err != nil {
// 				return err
// 			}
...
func (f *Fetcher) receivePeerLoop(sentryClient sentry.SentryClient) {
	for {
		select {
		case <-f.ctx.Done():
			return
...
func (f *Fetcher) receivePeer(sentryClient sentry.SentryClient) error {
...
func (f *Fetcher) handleNewPeer(req *sentry.PeerEvent) error {
	// if req == nil {
	// 	return nil
	// }
	// switch req.EventId {
...
func (f *Fetcher) handleStateChanges(ctx context.Context, client StateChangesClient) error {
	streamCtx, cancel := context.WithCancel(ctx)
	defer cancel()
...
func (f *Fetcher) handleStateChangesRequest(ctx context.Context, req *remote.StateChangeBatch) error {
