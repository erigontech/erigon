...
func (s *TxnSlots) Valid() error {
	if len(s.Txns) != len(s.IsLocal) {
		return fmt.Errorf("TxnSlots: expect equal len of isLocal=%d and txs=%d", len(s.IsLocal), len(s.Txns))
	}
	if len(s.Txns) != s.Senders.Len() {
		return fmt.Errorf("TxnSlots: expect equal len of senders=%d and txs=%d", s.Senders.Len(), len(s.Txns))
	}
	return nil
}
